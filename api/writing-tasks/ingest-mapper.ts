/**
 * /api/writing-tasks — 외부 응답에서 적재 후보(task)를 방어적으로 추출하는 순수 매퍼.
 *
 * 상류(/api/writing/tasks)의 response 형태가 아직 미확정·가변(스웨거도 자체 모순:
 * tasks vs items, task54 vs Q54, task52 부재)이므로, 이 매퍼는 형태를 단정하지
 * 않는다. 안정적으로 신뢰하는 값은 ① 식별자 후보 ② 문항 번호 후보 둘뿐이고,
 * 나머지는 raw로 통째 보존한다(무손실). §7 승격용 필드 매핑은 라이브 캡처 후
 * 별도 단계(P11)에서 한다 — 여기서 §7 컬럼을 파괴적으로 고르지 않는다.
 *

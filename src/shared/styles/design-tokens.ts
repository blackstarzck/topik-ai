...
export const CSS_COLOR_VARIABLES: Record<string, string> = {
   ...
   * 링크 hover.
   *
   * 🚨 여기에는 antd 기본 파랑(`#4096ff`)이 남아 있었다 — `colorLink` 드리프트(#127)와 같은
   * 계열이고 hover 라서 더 나쁘다: 흰 배경 대비 **2.99:1** 로 WCAG AA(4.5:1)에 크게 못 미쳐
   * **평소엔 읽히는 링크가 마우스를 올리면 덜 읽혔다**(#127 이 고친 `#1677ff` 의 4.10:1 보다도
   * 낮다). 브랜드 파생 hover(`colorPrimaryHover` = 5.59:1, AA 통과)로 맞춘다.
   */
  'link-hover': designToken.colorPrimaryHover,

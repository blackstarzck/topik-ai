...
const statusDescriptionMap: Record<string, string> = {
  ...
  '검수 완료': '검수 기준을 통과해 노출 후보 전환이 가능한 상태입니다(approved).',
  '검수 필요': '검수가 끝나지 않았거나 재검수·수정이 필요한 상태입니다(needs_revision).',
  '사용 보류': '추가 확인이나 정책 판단이 필요해 사용을 보류한 상태입니다(on_hold).',
  '수정 필요': '재생성 또는 수동 수정이 필요하다고 판단한 상태입니다.',
  '노출 가능': '사용자 노출이 허용된 상태입니다(available — 검수 완료 문항만 전환 가능).',
  '노출 제외': '사용자 노출에서 제외된 상태입니다(excluded).',
  '내부 테스트': '내부 검증용 상태로 사용자 노출이 차단되어 있습니다(internal_test — 백필 기본값).',

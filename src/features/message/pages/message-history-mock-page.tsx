...
import { fetchHistoriesSafe, retryMessageHistorySafe } from '../api/messages-service';
import {
  downloadMessageHistoryCsv,
  getMessageHistoryModeLabel,
  getMessageRecipientStatusColor,
  messageHistoryStatusFilterValues,
  parseMessageHistoryChannel,
  parseMessageHistoryMode,
  type MessageHistoryDangerState,
  type MessageHistoryModeFilter,
  type MessageRecipientStatusFilter
...
// mock 모드 발송 이력 — Phase 4 분해로 message-history-page.tsx 에서 파일 분리(동작 동일).
export function MockMessageHistoryPage(): JSX.Element {
  const [searchParams, setSearchParams] = useSearchParams();
  const activeChannel = parseMessageHistoryChannel(searchParams.get('channel'));
  const searchField = searchParams.get('searchField') ?? 'all';
  const startDate = parseSearchDate(searchParams.get('startDate'));
  const endDate = parseSearchDate(searchParams.get('endDate'));
  const modeFilter = parseMessageHistoryMode(searchParams.get('mode'));
  ...
  const [recipientStatusFilter, setMessageRecipientStatusFilter] =
    useState<MessageRecipientStatusFilter>('all');
  const [dangerState, setDangerState] = useState<MessageHistoryDangerState>(null);
  ...
  const handleModeFilterChange = useCallback(
    (nextMode: MessageHistoryModeFilter) => {
  ...
  const handleExportCsv = useCallback(() => {
    const rows = visibleRows.map((history) => {
      return [
        ...
        getMessageHistoryModeLabel(history.mode),
      ...
      fixDrawerTableFirstColumn<MessageHistory['recipients'][number]>([
        ...
        },
        {
          ...
          render: (status: MessageRecipientStatus) => (
            <Tag color={getMessageRecipientStatusColor(status)}>{status}</Tag>
          ),
          sorter: createTextSorter((record) => record.status)
        },
        {
      ...
      >
        ...
        <AdminDataTable<MessageHistory>
          ...
          onRow={(record) => ({
            onClick: () => {
              setRecipientKeyword('');
              setMessageRecipientStatusFilter('all');
              setDetailRow(record);
            },
            ...
            <DetailDrawerSection title="기본 정보">
              <Descriptions
                ...
                items={[
                  ...
                  { key: 'mode', label: '유형', children: getMessageHistoryModeLabel(detailRow.mode) },
              ...
              >
                {recipientStatusOptions.map((option) => (
                  <Checkbox
                    ...
                    onChange={() => setMessageRecipientStatusFilter(option.key)}

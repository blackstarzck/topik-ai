...
const initialGroups: SystemMetadataGroup[] = [
  {
    ...
    groupName: '회원 상태',
    description: 'Users 회원 목록과 상세 탭에서 공통으로 쓰는 회원 상태 코드 테이블입니다.',
    managerType: '코드 테이블',
    ownerModule: 'Users',
    ownerRole: 'OPS_ADMIN',
    status: '활성',
    ...
    linkedUserSurfaces: ['회원 가입 완료 화면', '마이페이지 계정 상태 뱃지'],
    schemaCandidateNotes: [
      'user_status_codes',
      '회원 상태는 감사 로그/검색 필터와 동일한 code table candidate'
    ],
    itemCodePrefix: 'USR_STATUS',
    items: [
      createItem({
  ...
  },
  {
    ...
    description: '대상 그룹과 자동 발송 룰에서 공통 사용하는 세그먼트 필드 메타데이터입니다.',
    managerType: '세그먼트 필드',
    ownerModule: 'Message',
    ownerRole: 'OPS_ADMIN',
    status: '활성',
    syncStatus: '검토 필요',
    exposureStatus: '내부 전용',
    ...
    itemCodePrefix: 'SEGMENT',
    items: [
      createItem({
        itemId: 'META-ITEM-004',
        code: 'shoppingGrade',
        label: '쇼핑 등급',
        description: '정기 쿠폰과 메시지 세그먼트가 공통 참조하는 등급 필드',
        status: '활성',
        sortOrder: 1,
        isDefault: false,
        exposureStatus: '내부 전용',
  ...
  },
  {
    ...
    schemaCandidateNotes: ['operation_faq_curations', '노출 위치는 code table candidate'],
  ...
  },
  {
    ...
    groupName: '쿠폰 적용 범위',
    description: '쿠폰과 정기 쿠폰 템플릿이 공통으로 쓰는 적용 범위 옵션입니다.',
    managerType: '선택 옵션',
    ...
    linkedUserSurfaces: [],
    schemaCandidateNotes: ['coupon-form-schema.ts', '상품/카테고리 참조는 후속 API 연동 필요'],
  ...
  },
  {
    ...
    description: '관리자 권한 세트와 역할 설명에 연결되는 운영 메타데이터입니다.',
    managerType: '코드 테이블',
    ownerModule: 'System',
    ...
    linkedUserSurfaces: [],
    schemaCandidateNotes: ['role_catalog', '권한 승인 절차 확정 전까지 내부 메타 후보 유지'],
    ...
    items: [
      createItem({
        ...
        code: 'OPS_ADMIN',
        label: '운영 관리자',
        description: '운영, 커뮤니티, 메시지, 커머스 권한 묶음',
        status: '활성',
        sortOrder: 1,
        isDefault: false,
        exposureStatus: '내부 전용',
  ...
  },
  {
    ...
    groupName: '배지 등급',
    description: '콘텐츠 배지 등급과 색상 세트를 관리할 후보 메타 그룹입니다.',
    managerType: '코드 테이블',
    ownerModule: 'Content',
    ownerRole: 'CONTENT_MANAGER',
    status: '비활성',
    syncStatus: '초안',
    exposureStatus: '노출 예정',
    linkedAdminPages: ['/content/badges'],
    linkedUserSurfaces: ['배지 획득 화면'],
    schemaCandidateNotes: ['badge_grade_codes', 'Content 페이지 구현 전까지 초안 유지'],
...
export const useSystemMetadataStore = create<SystemMetadataStore>((set, get) => ({
  ...
  saveGroup: (payload) => {
    ...
    const historyEntry = createHistoryEntry({
      ...
      action: existingGroup ? '그룹 수정' : '그룹 생성',
    ...
    const nextGroup: SystemMetadataGroup = {
      ...
      status: existingGroup?.status ?? '활성',
      ...
      lastReviewedAt:
        payload.syncStatus === '초안' ? existingGroup?.lastReviewedAt ?? now : now
    };
  ...
  toggleGroupStatus: (payload) => {
    ...
    const historyEntry = createHistoryEntry({
      historyId: createHistoryId(groups),
      action: payload.nextStatus === '활성' ? '그룹 활성화' : '그룹 비활성화',
      reason: payload.reason,
      changedBy: payload.changedBy,
      createdAt: now
    });
  ...
  saveItem: (payload) => {
    ...
    const historyEntry = createHistoryEntry({
      historyId: createHistoryId(groups),
      action: existingItem ? '항목 수정' : '항목 추가',
      reason: payload.reason,
      changedBy: payload.changedBy,
      createdAt: now
    });
  ...
  toggleItemStatus: (payload) => {
    ...
    const historyEntry = createHistoryEntry({
      historyId: createHistoryId(groups),
      action: payload.nextStatus === '활성' ? '항목 활성화' : '항목 비활성화',
      reason: payload.reason,
      changedBy: payload.changedBy,
      createdAt: now
    });

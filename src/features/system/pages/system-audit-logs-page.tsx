...
export default function SystemAuditLogsPage(): JSX.Element {
  ...
  const mergedRows = useMemo(() => {
    ...
    return [...couponRows, ...permissionRows, ...staticRows].sort(
      (left, right) => right.createdAt.localeCompare(left.createdAt),
    );
  }, [couponAudits, permissionAudits]);
      ...
      >
        <Paragraph type="secondary" style={{ marginBottom: 16 }}>
          ...
          {targetIdFilter ? (
            <>
              {" "}
              현재는 <Text strong>{targetIdFilter}</Text>에 대한 이력만 보고
              있습니다.
            </>
          ) : null}
          {commerceAuditCount > 0
            ? " 커머스 환불/결제와 쿠폰 운영 이력도 같은 표에서 함께 추적됩니다."
            : null}
        </Paragraph>

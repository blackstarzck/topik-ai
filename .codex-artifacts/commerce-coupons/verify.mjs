...
const svc = readFileSync('src/features/commerce/api/supabase-commerce-coupons-service.ts', 'utf8');
for (const [tbl, cst] of [['commerce_coupons', 'COUPON_COLUMNS'], ['commerce_coupon_subscription_templates', 'TEMPLATE_COLUMNS']]) {
...
const fns = (await q("select proname from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and proname like '%commerce_coupon%' order by 1")).map((f) => f.proname);

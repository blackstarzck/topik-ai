...
const tabItems: TabsProps['items'] = [
  { key: 'master', label: 'FAQ 留덉뒪?? },
  { key: 'curation', label: '?몄텧 愿由? },
  { key: 'metrics', label: '吏??蹂닿린' }
];
...
function buildFaqFormValues(faq?: OperationFaq): FaqFormValues {
  return {
    ...
    category: faq?.category ?? '怨꾩젙',
    status: faq?.status ?? '怨듦컻'
  };
}
...
export default function OperationFaqPage(): JSX.Element {
  ...
  const publicFaqCount = faqsState.data.filter((faq) => faq.status === '怨듦컻').length;
    ...
    () => [
      {
        key: 'all-faqs',
        label: '전체 FAQ',
        value: `${totalFaqCount.toLocaleString()}嫄?
      },
      {
        key: 'public-faqs',
        label: '怨듦컻 FAQ',
        value: `${publicFaqCount.toLocaleString()}嫄?
      },
      {
        key: 'active-curations',
        label: '활성 ?몄텧',
        value: `${activeCurationCount.toLocaleString()}嫄?
      },
      {
        key: 'faq-views',
        label: '?꾩쟻 議고쉶??,
  ...
  const handleSaveFaq = useCallback(async () => {
    ...
    if (!result.ok) {
      notificationApi.error({
        message:
          faqEditorState?.type === 'edit' ? 'FAQ ?섏젙 실패' : 'FAQ ?깅줉 실패',
        description: (
    ...
    notificationApi.success({
      message:
        faqEditorState?.type === 'edit' ? 'FAQ ?섏젙 완료' : 'FAQ ?깅줉 완료',
      description: (
        <Space direction="vertical">
          <Text>대상?좏삎: {getTargetTypeLabel('OperationFaq')}</Text>
          <Text>대상ID: {result.data.id}</Text>
          <Text>
            議곗튂: {faqEditorState?.type === 'edit' ? 'FAQ ?먮Ц ?섏젙' : 'FAQ ?좉퇋 ?깅줉'}
          </Text>
  ...
  const handleSaveCuration = useCallback(async () => {
    ...
    notificationApi.success({
      ...
      description: (
        <Space direction="vertical">
          <Text>대상?좏삎: {getTargetTypeLabel('OperationFaqCuration')}</Text>
          <Text>대상ID: {result.data.id}</Text>
          <Text>
            議곗튂:{' '}
            {curationEditorState?.type === 'edit'
  ...
  const faqColumns = useMemo<TableColumnsType<OperationFaq>>(
    () => [
      ...
      {
        title: createStatusColumnTitle('상태', ['怨듦컻', '鍮꾧났媛?]),
        dataIndex: 'status',
        width: 110,
      ...
      },
      {
        ...
        render: (_, record) => (
          <Space direction="vertical" size={0}>
            <Text>{record.faq?.question ?? '??젣?섏뿀嫄곕굹 ?숆린?붾릺吏 ?딆? FAQ'}</Text>
  ...
  const metricColumns = useMemo<TableColumnsType<FaqMetricRow>>(
    () => [
      ...
      },
      {
        ...
        dataIndex: 'faq',
        width: 320,
        render: (_, record) => record.faq?.question ?? '??젣??FAQ'
      },
      {
        title: '移댄뀒怨좊━',
        dataIndex: 'faq',
  ...
  const faqAlert =
    faqsState.status === 'error' ? (
      <Alert
        ...
        message="FAQ 紐⑸줉??遺덈윭?ㅼ? 紐삵뻽?듬땲??"
        description={
          <Space direction="vertical">
            <Text>{faqsState.errorMessage ?? '?쇱떆?곸씤 ?ㅻ쪟媛 諛쒖깮?덉뒿?덈떎.'}</Text>
            {faqsState.errorCode ? (
              <Text type="secondary">?ㅻ쪟 肄붾뱶: {faqsState.errorCode}</Text>
            ) : null}
            {faqsState.data.length > 0 ? (
              <Text type="secondary">
                留덉?留?성공 상태瑜??좎???梨?운영 ?먮쫫??怨꾩냽?????덉뒿?덈떎.
              </Text>
            ) : null}
          </Space>
        }
      ...
      <Alert
        ...
        showIcon
        style={{ marginBottom: 12 }}
        message="FAQ 紐⑸줉???덈줈 遺덈윭?ㅻ뒗 以묒엯?덈떎."
        description="留덉?留?성공 상태瑜??좎???梨?理쒖떊 ?곗씠?곕? ?숆린?뷀븯怨??덉뒿?덈떎."
      />
    ) : faqsState.status === 'empty' ? (
      <Alert
        type="info"
        showIcon
        style={{ marginBottom: 12 }}
        message="?깅줉??FAQ媛 ?놁뒿?덈떎."
        description="FAQ ?깅줉 踰꾪듉?쇰줈 泥?FAQ瑜?異붽??섏꽭??"
      />
      ...
      <Alert
        ...
        showIcon
        style={{ marginBottom: 12 }}
        message="寃??議곌굔??留욌뒗 FAQ媛 ?놁뒿?덈떎."
        description="寃?됱뼱, 移댄뀒怨좊━, 怨듦컻 상태, ?좎쭨 議곌굔???ㅼ떆 議곗젙?섏꽭??"
      />
    ) : null;
  ...
  const curationAlert =
    curationsState.status === 'error' ? (
      <Alert
        ...
        showIcon
        style={{ marginBottom: 12 }}
        message="FAQ ?몄텧 紐⑸줉??遺덈윭?ㅼ? 紐삵뻽?듬땲??"
        description={
          <Space direction="vertical">
            <Text>{curationsState.errorMessage ?? '?쇱떆?곸씤 ?ㅻ쪟媛 諛쒖깮?덉뒿?덈떎.'}</Text>
            {curationsState.errorCode ? (
              <Text type="secondary">?ㅻ쪟 肄붾뱶: {curationsState.errorCode}</Text>
            ) : null}
          </Space>
        }
        action={
      ...
      <Alert
        ...
        showIcon
        style={{ marginBottom: 12 }}
        message="FAQ ?몄텧 ?뺣낫瑜??덈줈 遺덈윭?ㅻ뒗 以묒엯?덈떎."
        description="留덉?留?성공 상태瑜??좎???梨?理쒖떊 ?몄텧 洹쒖튃??諛섏쁺?섍퀬 ?덉뒿?덈떎."
      />
    ) : curationsState.status === 'empty' ? (
      <Alert
        type="info"
        showIcon
        style={{ marginBottom: 12 }}
        message="?깅줉??FAQ ?몄텧 洹쒖튃???놁뒿?덈떎."
        description="?몄텧 異붽? 踰꾪듉?쇰줈 ??怨좉컼?쇳꽣 대상FAQ瑜?愿由ы븯?몄슂."
      />
      ...
      <Alert
        ...
        showIcon
        style={{ marginBottom: 12 }}
        message="寃??議곌굔??留욌뒗 ?몄텧 洹쒖튃???놁뒿?덈떎."
        description="?몄텧 ?꾩튂, ?ㅼ젙 諛⑹떇, ?몄텧 상태 議곌굔???ㅼ떆 議곗젙?섏꽭??"
      />
    ) : null;
  ...
  const metricAlert =
    metricsState.status === 'error' ? (
      <Alert
        ...
        showIcon
        style={{ marginBottom: 12 }}
        message="FAQ 吏?쒕? 遺덈윭?ㅼ? 紐삵뻽?듬땲??"
        description={
          <Space direction="vertical">
            <Text>{metricsState.errorMessage ?? '?쇱떆?곸씤 ?ㅻ쪟媛 諛쒖깮?덉뒿?덈떎.'}</Text>
            {metricsState.errorCode ? (
              <Text type="secondary">?ㅻ쪟 肄붾뱶: {metricsState.errorCode}</Text>
            ) : null}
          </Space>
        }
        action={
      ...
      <Alert
        ...
        showIcon
        style={{ marginBottom: 12 }}
        message="FAQ 吏?쒕? ?덈줈 遺덈윭?ㅻ뒗 以묒엯?덈떎."
        description="留덉?留?성공 상태瑜??좎???梨?理쒖떊 議고쉶/寃??吏?쒕? 媛깆떊?섍퀬 ?덉뒿?덈떎."
      />
      ...
      <Alert
        ...
        showIcon
        style={{ marginBottom: 12 }}
        message="?쒖떆??FAQ 吏?쒓? ?놁뒿?덈떎."
        description="FAQ媛 ?깅줉?섎㈃ 議고쉶/寃??吏???ㅻ깄?룹씠 ?④퍡 ?앹꽦?⑸땲??"
      />
      ...
      <Alert
        ...
        showIcon
        style={{ marginBottom: 12 }}
        message="寃??議곌굔??留욌뒗 FAQ 吏?쒓? ?놁뒿?덈떎."
        description="FAQ ID, 吏덈Ц, 寃???ㅼ썙??湲곗??쇰줈 ?ㅼ떆 寃?됲븯?몄슂."
      />
    ) : null;
  ...
  const faqDrawerInfoItems = selectedFaq
    ? [
        ...
        { key: 'category', label: '移댄뀒怨좊━', children: selectedFaq.category },
        {
          key: 'status',
          label: '怨듦컻 상태',
          children: <StatusBadge status={selectedFaq.status} />
        },
        { key: 'createdAt', label: '?깅줉??, children: selectedFaq.createdAt },
        { key: 'updatedAt', label: '理쒖쥌 ?섏젙', children: selectedFaq.updatedAt },
        { key: 'updatedBy', label: '?섏젙??, children: selectedFaq.updatedBy }
      ]
    : [];
  ...
  const faqMetricItems = selectedFaqMetric
    ? [
        {
          key: 'viewCount',
          label: '議고쉶??,
          children: `${selectedFaqMetric.viewCount.toLocaleString()}??
        },
        {
          key: 'searchHitCount',
          label: '寃???좎엯',
          children: `${selectedFaqMetric.searchHitCount.toLocaleString()}??
        },
        {
          key: 'helpful',
          label: '?꾩???,
          children: `${selectedFaqMetric.helpfulCount.toLocaleString()}嫄?
        },
        {
          key: 'notHelpful',
          label: '?꾩? ????,
          children: `${selectedFaqMetric.notHelpfulCount.toLocaleString()}嫄?
        },
        {
          key: 'ratio',
          label: '?꾩???鍮꾩쑉',
    ...
    : [{ key: 'empty', label: '吏??상태', children: '?꾩쭅 吏묎퀎??吏?쒓? ?놁뒿?덈떎.' }];

  const curationDrawerItems = selectedCuration
    ? [
        { key: 'curationId', label: '?몄텧 ID', children: selectedCuration.id },
        {
          key: 'surface',
          label: '?몄텧 ?꾩튂',
        ...
        },
        {
          key: 'displayRank',
          label: '?몄텧 ?쒖꽌',
          children: `${selectedCuration.displayRank}??
        },
        {
          key: 'curationMode',
          label: '?ㅼ젙 諛⑹떇',
        ...
        { key: 'updatedAt', label: '理쒖쥌 ?섏젙', children: selectedCuration.updatedAt },
        { key: 'updatedBy', label: '?섏젙??, children: selectedCuration.updatedBy }
      ]
    : [];
  ...
  const curationLinkedFaqItems = selectedCuration
    ? [
        { key: 'faqId', label: 'FAQ ID', children: selectedCuration.faqId },
        {
          key: 'question',
          label: '吏덈Ц',
          children: selectedCuration.faq?.question ?? '??젣?섏뿀嫄곕굹 ?숆린?붾릺吏 ?딆? FAQ'
        },
        {
          key: 'category',
          label: '移댄뀒怨좊━',
  ...
  return (
    <div>
          ...
          <div className="admin-list-card-toolbar-stack">
            ...
            {activeTab === 'master' ? (
              <SearchBar
                ...
                searchFieldOptions={[
                  ...
                  { label: '?듬?', value: 'answer' },
                  { label: '寃???ㅼ썙??, value: 'searchKeywords' }
                ]}
                keyword={keyword}
                ...
                detailContent={
                  <Space direction="vertical" size={16} style={{ width: '100%' }}>
                    <SearchBarDetailField label="理쒖쥌 ?섏젙??>
                      <SearchBarDateRange
                ...
                summary={<Text type="secondary">珥?{visibleFaqs.length.toLocaleString()}嫄?/Text>}
                actions={
                  <Space>
            ...
            {activeTab === 'curation' ? (
              <SearchBar
                ...
                searchFieldOptions={[
                  { label: '전체', value: 'all' },
                  { label: '?몄텧 ID', value: 'id' },
                  { label: 'FAQ ID', value: 'faqId' },
                  { label: '吏덈Ц', value: 'question' },
                  { label: '?몄텧 ?꾩튂', value: 'surface' }
                ]}
                ...
                summary={
                  <Text type="secondary">珥?{visibleCurations.length.toLocaleString()}嫄?/Text>
                }
                actions={
                  <Space>
                    <Button icon={<ReloadOutlined />} onClick={handleReload}>
            ...
            {activeTab === 'metrics' ? (
              <SearchBar
                ...
                summary={<Text type="secondary">珥?{visibleMetrics.length.toLocaleString()}嫄?/Text>}
                actions={
                  <Button icon={<ReloadOutlined />} onClick={handleReload}>
                    ?덈줈怨좎묠
                  </Button>
      ...
      >
        ...
        {activeTab === 'master' ? (
          <AdminDataTable<OperationFaq>
            ...
            locale={{ emptyText: '?깅줉??FAQ媛 ?놁뒿?덈떎.' }}
            scroll={{ x: 1180 }}
        ...
        {activeTab === 'curation' ? (
          <AdminDataTable<FaqCurationRow>
            ...
            locale={{ emptyText: '?깅줉??FAQ ?몄텧 洹쒖튃???놁뒿?덈떎.' }}
            scroll={{ x: 1320 }}
        ...
        {activeTab === 'metrics' ? (
          <AdminDataTable<FaqMetricRow>
            ...
            locale={{ emptyText: '?쒖떆??FAQ 吏?쒓? ?놁뒿?덈떎.' }}
            scroll={{ x: 1280 }}
      ...
      <Modal
        ...
        title={faqEditorState?.type === 'edit' ? 'FAQ ?섏젙' : 'FAQ ?깅줉'}
        okText="대상
        cancelText="취소"
      ...
      >
        <Form form={faqForm} layout="vertical">
          <Form.Item
            label="吏덈Ц"
            name="question"
            rules={[{ required: true, message: '吏덈Ц???낅젰?섏꽭??' }]}
          >
            <Input placeholder="?ъ슜?먯뿉寃??몄텧??吏덈Ц???낅젰?섏꽭??" />
          </Form.Item>
          <Form.Item
            label="移댄뀒怨좊━"
            name="category"
          ...
          <Form.Item
            ...
            extra="?쇳몴 ?먮뒗 以꾨컮轅?湲곗??쇰줈 遺꾨━?섏뼱 ??λ맗?덈떎."
          >
            <Input.TextArea rows={3} placeholder="?? 결제 ?ㅻ쪟, 移대뱶 결제, 환불" />
          </Form.Item>
          <Form.Item
            label="?듬?"
            name="answer"
            rules={[{ required: true, message: '?듬????낅젰?섏꽭??' }]}
          >
            <Input.TextArea rows={7} placeholder="FAQ ?듬????낅젰?섏꽭??" />
          </Form.Item>
          <Form.Item
            label="怨듦컻 상태"
            name="status"
            rules={[{ required: true, message: '怨듦컻 상태瑜??좏깮?섏꽭??' }]}
      ...
      <Modal
        ...
        title={curationEditorState?.type === 'edit' ? 'FAQ ?몄텧 ?섏젙' : 'FAQ ?몄텧 異붽?'}
        okText="대상
        cancelText="취소"
      ...
      >
        <Form form={curationForm} layout="vertical">
          <Form.Item
            label="?곌껐 FAQ"
            name="faqId"
            rules={[{ required: true, message: '?곌껐??FAQ瑜??좏깮?섏꽭??' }]}
          >
            <Select
              showSearch
              optionFilterProp="label"
              placeholder="FAQ瑜??좏깮?섏꽭??"
              options={faqsState.data.map((faq) => ({
                label: `${faq.id} 쨌 ${faq.question}`,
                value: faq.id
              }))}
            />
          </Form.Item>
          <Form.Item
            label="?몄텧 ?꾩튂"
            name="surface"
            rules={[{ required: true, message: '?몄텧 ?꾩튂瑜??좏깮?섏꽭??' }]}
          >
          ...
          <Form.Item
            label="?몄텧 ?쒖꽌"
            name="displayRank"
            rules={[{ required: true, message: '?몄텧 ?쒖꽌瑜??낅젰?섏꽭??' }]}
          >
          ...
          <Form.Item
            label="?ㅼ젙 諛⑹떇"
            name="curationMode"
            rules={[{ required: true, message: '?ㅼ젙 諛⑹떇???좏깮?섏꽭??' }]}
          >
          ...
          <Form.Item
            label="?몄텧 상태"
            name="exposureStatus"
            rules={[{ required: true, message: '?몄텧 상태瑜??좏깮?섏꽭??' }]}
          >
            <Select options={faqCurationStatusOptions} />
          </Form.Item>
          <Form.Item label="?몄텧 湲곌컙" name="pinnedDateRange" style={{ marginBottom: 0 }}>
          ...
          title={
            dangerState.type === 'deleteFaq'
              ...
              : dangerState.type === 'toggleFaqStatus'
                ? `FAQ ${dangerState.nextStatus === '怨듦컻' ? '怨듦컻' : '鍮꾧났媛?} ?꾪솚`
                : dangerState.type === 'deleteCuration'
                  ? 'FAQ ?몄텧 ??젣'
                  : `FAQ ?몄텧 ${dangerState.nextStatus === 'active' ? '?ш컻' : '?쇱떆以묒?'}`
          }
          description={
            dangerState.type === 'deleteFaq'
              ? 'FAQ瑜???젣?섎㈃ ?곌껐???몄텧 洹쒖튃怨?吏???곌껐???④퍡 ?뺣━?⑸땲??'
              : dangerState.type === 'toggleFaqStatus'
                ? 'FAQ 怨듦컻 상태瑜?蹂寃쏀븯硫?사용자?꾩?留먭낵 ?곌껐???몄텧 洹쒖튃??즉시 ?곹뼢???앷퉩?덈떎.'
                : dangerState.type === 'deleteCuration'
                  ? '대상FAQ ?몄텧 洹쒖튃????젣?섎㈃ ?대떦 ?붾㈃?먯꽌 ???댁긽 ?먮룞?쇰줈 ?몄텧?섏? ?딆뒿?덈떎.'
                  : 'FAQ ?몄텧 상태瑜?蹂寃쏀븯硫?대상FAQ ?곸뿭??즉시 ?몄텧 ?щ?媛 ?щ씪吏묐땲??'
          }
          targetType={
      ...
      <DetailDrawer
        ...
        title={selectedFaq ? `FAQ 상세 쨌 ${selectedFaq.id}` : 'FAQ 상세'}
        width={760}
        destroyOnHidden
        ...
        footerEnd={
          selectedFaq ? (
            <Space wrap>
              <Button onClick={() => openFaqEditModal(selectedFaq)}>FAQ ?섏젙</Button>
              ...
              <Button
                onClick={() =>
                  setDangerState({
                    type: 'toggleFaqStatus',
                    faq: selectedFaq,
                    nextStatus: selectedFaq.status === '怨듦컻' ? '鍮꾧났媛? : '怨듦컻'
                  })
                }
              >
                {selectedFaq.status === '怨듦컻' ? '鍮꾧났媛??꾪솚' : '怨듦컻 ?꾪솚'}
              </Button>
              <Button
                danger
      ...
      >
        {selectedFaq ? (
          <DetailDrawerBody>
            {selectedFaq.status === '鍮꾧났媛? && selectedFaqCurations.length > 0 ? (
              <Alert
                type="warning"
                showIcon
                message="鍮꾧났媛?FAQ???곌껐???몄텧 洹쒖튃???덉뒿?덈떎."
                description="鍮꾧났媛??꾪솚 ??愿???몄텧 洹쒖튃? ?먮룞?쇰줈 대기상태濡??꾪솚?⑸땲??"
              />
            ) : null}

            <DetailDrawerSection title="FAQ ?뺣낫">
              <Descriptions bordered size="small" column={1} items={faqDrawerInfoItems} />
            </DetailDrawerSection>

            <DetailDrawerSection title="吏덈Ц / ?듬?">
              <Descriptions
                bordered
                size="small"
                column={1}
                items={[
                  { key: 'question', label: '吏덈Ц', children: selectedFaq.question },
                  {
                    key: 'searchKeywords',
                    label: '寃???ㅼ썙??,
            ...
            >
              {selectedFaqCurations.length === 0 ? (
                <Alert
                  type="info"
                  showIcon
                  message="?곌껐???몄텧 洹쒖튃???놁뒿?덈떎."
                  description="??異붿쿇 FAQ??怨좉컼?쇳꽣 대상FAQ濡??몄텧?섎젮硫?洹쒖튃??異붽??섏꽭??"
                />
              ) : (
                <Descriptions
                  bordered
                  ...
                  items={selectedFaqCurations.map((curation) => ({
                    ...
                    children: (
                      <Space wrap>
                        ...
                        <Text type="secondary">{curation.displayRank}??/Text>
            ...
            <DetailDrawerSection title="吏???붿빟">
      ...
      <DetailDrawer
        ...
        title={selectedCuration ? `FAQ ?몄텧 상세 쨌 ${selectedCuration.id}` : 'FAQ ?몄텧 상세'}
        width={720}
        destroyOnHidden
      ...
      >
        {selectedCuration ? (
          <DetailDrawerBody>
            <DetailDrawerSection title="?몄텧 洹쒖튃">
            ...
            <DetailDrawerSection title="吏??李멸퀬">
              <Descriptions
                bordered
                size="small"
                column={1}

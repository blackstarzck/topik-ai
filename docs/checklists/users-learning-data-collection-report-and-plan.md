...
| 결정 항목 | 기본 추천 | 이유 |
| --- | --- | --- |
| TOPIK 쓰기 소요 시간 저장 위치 | writing 전용 metrics 계약 | `problem_attempts`는 objective attempt 성격이 강하고 writing 제출과 의미가 다르다. |
...
| 활성 사용자 기준 | 로그인 기준이 아니라 학습 이벤트 기준 | 학습 분석의 목적은 실제 학습 행동 측정이다. |

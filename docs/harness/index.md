...
- `npm run harness:check`
  ...
  - 중복 코드 임계 검사 (`npm run check:duplication` — jscpd, `package.json` `jscpd` 키가 설정 SoT. 임계 3.6% — Phase 3b 확산 3차 후 실측 3.51% 의 래칫. 대규모 중복 제거 후 같은 PR 에서 하향한다 — 배경은 `docs/specs/admin-page-gap-register.md` §3.12)
  ...
  - typecheck (`tsc -b --noEmit`. 루트 `tsconfig.json` 이 solution-style 이라 `-b` 없이는 **아무 파일도 검사하지 않는다** — 배경은 `docs/specs/admin-page-gap-register.md` §3.9. 검사 대상은 `tsconfig.app.json`(src)·`tsconfig.node.json`(vite.config·api)·`tsconfig.tests.json`(tests+import 소스) 세 프로젝트다. `tests` 편입은 §3.9.2 — 참조 대신 include 로 넣어야 하는 이유가 거기 있다)

...
- 레벨 테스트 화면의 운영 목적, 데이터 블록, 조치 흐름을 같은 기준으로 정리합니다.
...
- 레벨 테스트 화면의 핵심 운영 데이터를 검색하고 검수합니다.
...
- 시나리오 1: 운영자가 레벨 테스트 화면에서 검색/필터를 적용하고 대상 레코드나 편집 대상을 선택합니다.
...
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 테스트 정보: 테스트명, 대상 레벨, 문항 수, 상태, 운영 정보: 결과 구간, 추천 콘텐츠 연결, 최근 수정 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| --- | --- | --- | --- | --- | --- |
| 생성 | 수정 | Assessment + levelTestId | 사유 권장 | 생성 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Assessment&targetId={levelTestId} |
| 수정 | 수정 | Assessment + levelTestId | 사유 권장 | 수정 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Assessment&targetId={levelTestId} |
| 결과 규칙 설정 | 조회 | Assessment + levelTestId | 불필요 | 결과 규칙 설정 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 발행 | 파괴적 | Assessment + levelTestId | 확인 + 사유 필수 | 발행 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Assessment&targetId={levelTestId} |
...
| --- | --- | --- | --- |
| Content > 콘텐츠 관리 | 결과 구간 추천 콘텐츠 연결 | 콘텐츠 연결 | 선행 관계 |
...
| --- | --- | --- | --- |
| 레벨 테스트 시작/결과 화면 | 노출 예정 | 추천 콘텐츠 분기점이 될 가능성 있음 | 레벨 테스트 세트 |

# Content > 객관식 선택 상세 IA

## 1. 문서 목적

- 객관식 선택 화면의 운영 목적, 데이터 블록, 조치 흐름을 같은 기준으로 정리합니다.
...
- 객관식 선택 화면의 핵심 운영 데이터를 검색하고 검수합니다.
...
- 시나리오 1: 운영자가 객관식 선택 화면에서 검색/필터를 적용하고 대상 레코드나 편집 대상을 선택합니다.
...
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 문제 정보: 문제 묶음명, 보기 수, 난이도, 상태, 운영 정보: 정답, 연결 단어장, 최근 수정 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| --- | --- | --- | --- | --- | --- |
| 생성 | 수정 | Content + multipleChoiceId | 사유 권장 | 생성 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Content&targetId={multipleChoiceId} |
| 편집 | 수정 | Content + multipleChoiceId | 사유 권장 | 편집 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Content&targetId={multipleChoiceId} |
| 정답 검증 | 조회 | Content + multipleChoiceId | 불필요 | 정답 검증 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 미리보기 | 조회 | Content + multipleChoiceId | 불필요 | 미리보기 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 발행 | 파괴적 | Content + multipleChoiceId | 확인 + 사유 필수 | 발행 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Content&targetId={multipleChoiceId} |
...
| --- | --- | --- | --- |
| Content > 단어장 | 연결 단어장 원천 참조 | 연결 단어장 링크 | 선행 관계 |
...
| --- | --- | --- | --- |
| 객관식 학습/퀴즈 화면 | 노출 예정 | 단어장 하위 콘텐츠 타입 | 객관식 콘텐츠 |

...
- EPS TOPIK 화면의 운영 목적, 데이터 블록, 조치 흐름을 같은 기준으로 정리합니다.
...
- EPS TOPIK 화면의 핵심 운영 데이터를 검색하고 검수합니다.
...
- 시나리오 1: 운영자가 EPS TOPIK 화면에서 검색/필터를 적용하고 대상 레코드나 편집 대상을 선택합니다.
- 시나리오 2: 운영자가 세트 생성 흐름으로 세부 정보를 확인하고 필요한 조치를 실행합니다.
...
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 세트 정보: 회차, 세트명, 문항 수, 상태, 운영 정보: 배점 규칙, 공개 여부, 연결 문항 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| --- | --- | --- | --- | --- | --- |
| 세트 생성 | 수정 | Assessment + examSetId | 사유 권장 | 세트 생성 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Assessment&targetId={examSetId} |
| 편성 | 조회 | Assessment + examSetId | 불필요 | 편성 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 미리보기 | 조회 | Assessment + examSetId | 불필요 | 미리보기 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 발행 | 파괴적 | Assessment + examSetId | 확인 + 사유 필수 | 발행 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Assessment&targetId={examSetId} |
...
| --- | --- | --- | --- |
| Assessment > TOPIK 쓰기 문제은행 | 세트 편성용 문항 풀 참조 | 문항 연결 | 선행 관계 |
...
| --- | --- | --- | --- |
| EPS TOPIK 응시/결과 화면 | 노출 예정 | 시험 운영 UX의 핵심 데이터 | 시험 세트 |

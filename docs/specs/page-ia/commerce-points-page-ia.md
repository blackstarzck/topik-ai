...
- 포인트 관리 화면의 운영 목적, 데이터 블록, 조치 흐름을 같은 기준으로 정리합니다.
...
- 포인트 관리 화면의 핵심 운영 데이터를 검색하고 검수합니다.
...
- 시나리오 1: 운영자가 포인트 관리 화면에서 검색/필터를 적용하고 대상 레코드나 편집 대상을 선택합니다.
- 시나리오 2: 운영자가 수동 적립 흐름으로 세부 정보를 확인하고 필요한 조치를 실행합니다.
...
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 정책 정보: 포인트 정책명, 적립 규칙, 차감 규칙, 상태, 이력 정보: 사용자별 잔액, 최근 적립/차감, 만료 예정 수량 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| --- | --- | --- | --- | --- | --- |
| 수동 적립 | 조회 | Commerce + pointPolicyId | 불필요 | 수동 적립 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 수동 차감 | 파괴적 | Commerce + pointPolicyId | 확인 + 사유 필수 | 수동 차감 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Commerce&targetId={pointPolicyId} |
| 정책 수정 | 수정 | Commerce + pointPolicyId | 사유 권장 | 정책 수정 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Commerce&targetId={pointPolicyId} |
| 내역 내보내기 | 조회 | Commerce + pointPolicyId | 불필요 | 내역 내보내기 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
...
| --- | --- | --- | --- |
| Users > 추천인 관리 | 추천 보상 포인트 연동 가능 | 보상 정책 참조 | 선행 관계 |
| Content > 학습 미션 | 미션 보상 포인트 연동 가능 | 보상 연결 설정 | 선행 관계 |
...
| --- | --- | --- | --- |
| 포인트 지갑, 결제 사용, 리워드 내역 | 노출 예정 | 쿠폰/추천인/미션과 함께 연결될 가능성 큼 | 포인트 정책/이력 |

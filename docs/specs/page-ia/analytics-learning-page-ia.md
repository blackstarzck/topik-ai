...
| 문제 유형별 비교 | 학습자, 제출자, 제출 수, 완료율, 평균 환산 점수, 조회율, 풀이 시간, 재제출률, PDF 내보내기 수. 표와 안내 문구를 포함한 콘텐츠 자연 높이를 사용하며 같은 행의 점수 분포 카드 높이에 강제로 맞추지 않음 | 정렬 |
| 문제 유형별 점수 분포 | `0–40`, `41–60`, `61–80`, `81–100`. 차트/표의 콘텐츠 자연 높이를 사용하고, 부모 분석 행은 두 카드 중 더 큰 높이를 포함한 뒤 다음 섹션을 배치함. 1024px 이하에서는 두 카드를 세로로 전환 | 차트/표 전환 |
| 주제별 성과 | 전폭 행: 단일 테이블 행 안에서 좌측 정보(`topic_main`, `topic_detail`, 문제 유형 51~54, 평균 환산 점수, 제출 수, 직전 기간 변화)와 우측 제출 수 바 차트를 같은 문제 유형×주제 단위로 1:1 대응. 화면은 행별 제출 수 내림차순으로 정렬하고, 막대 길이는 현재 결과의 최대 제출 수를 기준으로 표시 | 정렬 |
| PDF 사용 분석 | 한 카드 안의 2열 구조. 좌측 `전체 구성 비율`은 전체·직접 귀속·혼합·미분류 요약과 문제 유형·혼합·미분류의 전체 구성 파이 차트를 표시하되 별도 범례 카드는 두지 않음. 데스크톱 좌측 섹션은 제목 아래 KPI와 차트 영역을 같은 높이로 나누고 각 콘텐츠를 세로 중앙 정렬해 파이를 하단 절반에 배치함. 우측 `문제 유형별 구성과 주제 상세`는 문제 유형 부모 행과 직접 귀속 주제 자식 행을 합친 Ant Design 확장형 구성표를 표시함. 부모 행의 대주제는 고유 대주제 수만 단위 없는 숫자로 표시함. 자식 행은 완료 수 내림차순을 유지하되 `1위`, `2위` 같은 순위 라벨을 노출하지 않음. 헤더·본문·hover에는 행 배경색을 사용하지 않음. 데스크톱에서는 두 내부 섹션의 상·하단을 같은 높이로 정렬하고, 1024px 이하에서는 세로 전환 후 콘텐츠 자연 높이를 사용 | Ant Design 기본 확장 버튼으로 문제 유형별 주제 상세 펼치기·접기, 혼합·미분류 확인 |
...
- PDF 구성과 주제 상세는 별도 목록·순위표로 나누지 않습니다. 한 카드에서 좌측 `전체 구성 비율`에 요약과 전체 구성 파이 차트를 표시하되 파이 하단의 별도 범례 카드는 두지 않고, 우측 `문제 유형별 구성과 주제 상세`에 Ant Design Table의 확장형 계층표를 나란히 표시합니다. 데스크톱에서는 두 내부 섹션의 상·하단을 같은 높이로 정렬하고, 좌측 제목 아래 공간은 KPI와 차트가 각각 절반을 차지하도록 나눠 두 콘텐츠를 세로 중앙 정렬합니다. 계층표는 헤더·부모·자식·혼합·미분류·hover 상태에 배경색을 사용하지 않으며, 1024px 이하에서는 두 섹션을 세로로 전환하고 강제 높이를 해제합니다. 51~54번 부모 행의 대주제 셀은 자식 행의 고유 `topicMain` 수만 `개`, `주제` 같은 단위 없이 숫자로 표시하고, 기본 확장 버튼으로 펼치면 해당 문제 유형에 직접 귀속된 `topicMain × topicDetail` 자식 행이 건수 내림차순으로 나타납니다. 자식 행에는 별도 순위 라벨을 표시하지 않습니다.
- 혼합·미분류는 특정 주제로 배분하지 않고 `주제 분석 불가`인 독립 부모 행으로 표시합니다. 문제 유형만 확인되고 주제가 연결되지 않은 직접 귀속 이벤트는 자식 행에서 `주제 미연결`로 표시합니다. 건수가 0인 문제 유형도 숨기지 않고 0건 상태로 유지합니다.
...
- 페이지: `src/features/analytics/pages/analytics-learning-page.tsx` (권한 게이트·조회 상태·URL 동기화·CSV·조건 적용 판정 소유).
- 분해 구현 파일(Phase 4 — 라우트·IA·동작 무변경): `src/features/analytics/model/analytics-learning-page-schema.ts` (KPI 정의 카피·기간 옵션·색상·포맷터·조건 태그·PDF 위계/조각 순수 변환), `src/features/analytics/ui/analytics-learning-kpi.tsx` (KPI 카드·추세·지표 설명 툴팁·카드 빌더), `src/features/analytics/ui/analytics-learning-charts.tsx` (주제 제출 바·PDF 구성 파이·PDF 위계 바), `src/features/analytics/ui/analytics-learning-columns.tsx` (테이블 컬럼 3종 팩토리), `src/features/analytics/ui/analytics-learning-condition-drawer.tsx` (분석 조건 Drawer — 초안 상태는 페이지 소유, 파생 표시값은 Drawer 내부 계산).

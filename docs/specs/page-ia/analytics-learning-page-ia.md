...
| 문제 유형별 비교 | 학습자, 제출자, 제출 수, 완료율, 평균 환산 점수, 조회율, 풀이 시간, 재제출률, PDF 내보내기 수. 데스크톱 2열에서는 점수 분포 카드와 같은 높이를 공유하고, 표 viewport와 가로 스크롤 영역은 안내 문구를 제외한 카드 본문의 남는 높이를 채우되 행 높이는 늘리지 않음 | 정렬 |
| 문제 유형별 점수 분포 | `0–40`, `41–60`, `61–80`, `81–100`. 1024px 이하에서 문제 유형별 비교와 세로로 전환되면 각 카드·표는 콘텐츠 자연 높이를 사용 | 차트/표 전환 |
...
| PDF 사용 분석 | 한 카드 안의 2열 구조. 좌측 `전체 구성 비율`은 전체·직접 귀속·혼합·미분류 요약과 문제 유형·혼합·미분류의 전체 구성 파이 차트를 표시하되 별도 범례 카드는 두지 않음. 우측 `문제 유형별 구성과 주제 상세`는 문제 유형 부모 행과 직접 귀속 주제 자식 행을 합친 Ant Design 확장형 구성표를 표시하며 헤더·본문·hover에 행 배경색을 사용하지 않음. 데스크톱에서는 두 내부 섹션의 상·하단을 같은 높이로 정렬하고, 1024px 이하에서는 세로 전환 후 콘텐츠 자연 높이를 사용 | Ant Design 기본 확장 버튼으로 문제 유형별 주제 상세 펼치기·접기, 혼합·미분류 확인 |

...
- 학습 규모·피드백 품질·점수 분포·취약 차원·주제 성과·PDF 내보내기 사용을 같은 범위에서 비교합니다.
- 지표 정의, 계산식, 표본, 커버리지와 직전 동일 기간 비교 기준을 화면에서 확인합니다.

### 비목표

- 개인 학습자 단위 상세는 `Users > 회원 상세 > 학습 현황`이 담당합니다.
- 이 화면은 조회·공유·CSV 내보내기만 제공하며 데이터 변경과 감사 로그 생성은 하지 않습니다.
...
| 영역 | 주요 내용 | 액션 |
| --- | --- | --- |
| 페이지 헤더 | 브레드크럼, 제목·설명, 데이터 갱신 시각 | 지표 사전, 분석 공유, CSV 내보내기, 분석 조건 열기 |
| 적용 조건 요약 | 기간, 문제 유형, 주제·세부 조건, 이전 기간 비교 | 조건 초기화, 조건 변경 |
| KPI 8종 | 학습 활성 사용자, 제출 수, 피드백 완료율, 평균 환산 점수, 피드백 조회율, 평균 풀이 시간, 처리 시간 중앙값, PDF 내보내기 완료 수 | 정보 아이콘으로 해당 지표 정의 열기 |
| 문제 유형별 비교 | 학습자, 제출자, 제출 수, 완료율, 평균 환산 점수, 조회율, 풀이 시간, 재제출률, PDF 내보내기 수 | 정렬 |
| 문제 유형별 점수 분포 | `0–40`, `41–60`, `61–80`, `81–100` | 차트/표 전환 |
| 취약 평가 영역 | 실제 `feedback_dimension_scores` 표준 차원, 점수, 표본 | 문제 유형별 비교 |
| 주제별 성과 | `topic_main`, `topic_detail`, 평균 환산 점수, 제출 수, 직전 기간 변화 | 정렬 |
...
- 각 지표 사전은 정의, 계산식과 분자·분모, 포함·제외 조건, 현재 조건, 표본 수, 커버리지, 비교 기준과 왜곡 가능성을 제공합니다.

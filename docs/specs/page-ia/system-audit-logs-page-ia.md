...
- 감사 로그 화면의 운영 목적, 데이터 블록, 조치 흐름을 같은 기준으로 정리합니다.
...
- 감사 로그 화면의 핵심 운영 데이터를 검색하고 검수합니다.
...
- 시나리오 1: 운영자가 감사 로그 화면에서 검색/필터를 적용하고 대상 레코드나 편집 대상을 선택합니다.
...
| --- | --- | --- | --- | --- | --- |
| 상단 요약 | 운영 규모와 우선순위 파악 | 현재 결과 수, 권한 변경 로그 수, 오늘 생성 로그 수 | 없음 | 후속 화면 우선순위 결정 | 직접 또는 간접 영향 |
| 검색/필터 | 탐색 범위 축소 | 검색어, 시각 | 조건 변경, 초기화 | 후속 상세 대상 축소 | 직접 영향 없음 |
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 로그 ID, 대상 유형, 대상 ID, 조치, 수행자, 사유, 시각 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| --- | --- | --- | --- | --- | --- |
| 상세 보기 | 조회 | System + auditLogId | 불필요 | 상세 보기 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 원본 페이지 이동 | 조회 | System + auditLogId | 불필요 | 원본 페이지 이동 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 필터 초기화 | 조회 | System + auditLogId | 불필요 | 필터 초기화 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |

## 8. 상태값/정책/운영 규칙

| 항목 | 현재 상태 | 관리자 페이지 영향 | 사용자 화면 영향 | 추후 결정 필요 내용 |
| --- | --- | --- | --- | --- |
| 상태값/운영 규칙 | 확정 | 감사 로그는 append-only 증적 데이터로 간주하고 수정/삭제 없이 조회만 허용합니다. | 사용자 화면 문구와 상태 기준의 운영 원천이 됩니다. | 상태 세트 변경 시 연관 문서와 화면을 함께 갱신해야 합니다. |
...
| --- | --- | --- | --- |
| 없음 | 내부 전용 | 운영 증적 데이터. Users/Community/Commerce/Message/System 원본 화면과 `Target Type + Target ID` 링크로 상호 추적된다 | 관리자 조치 로그 |
...
- 사용자 대상 링크는 `Users > 회원 상세`로 이동하는 단일 동선을 사용합니다.
- 감사 로그 상세 Modal에서도 `Users` 대상 식별자는 `이름 (ID)` 형식을 유지합니다.

...
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 그룹명, 설명, 정의 방식, 조건 요약, 예상 대상 수, 상태, 작업, 적용 채널, 운영 상태, 세그먼트 조건, 정적 대상 목록 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| 재계산 | 조회 | Message + groupId | 불필요 | 재계산 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |

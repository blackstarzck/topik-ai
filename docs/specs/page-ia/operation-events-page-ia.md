...
- 시나리오 1: 운영자가 이벤트 화면에서 검색/필터를 적용하고 대상 레코드나 편집 대상을 선택합니다.
...
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 이벤트 기본 정보: 이벤트 ID, 이벤트명, 유형, 기간, 상태, 운영 정보: 노출 위치, 참여자 수, 보상 정책 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| --- | --- | --- | --- | --- | --- |
| 생성 | 수정 | Operation + eventId | 사유 권장 | 생성 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Operation&targetId={eventId} |
| 수정 | 수정 | Operation + eventId | 사유 권장 | 수정 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Operation&targetId={eventId} |
| 게시 예약 | 조회 | Operation + eventId | 불필요 | 게시 예약 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
...
| 참여 현황 확인 | 조회 | Operation + eventId | 불필요 | 참여 현황 확인 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
...
| --- | --- | --- | --- |
| Commerce > 쿠폰 관리 | 이벤트 보상 연동 가능 | 보상 정책 참조 | 선행/후행 관계 |
| Message > 메일 / 푸시 | 이벤트 안내 메시지 연계 | 템플릿 연동 | 후행 관계 |
...
| --- | --- | --- | --- |
| 이벤트 목록, 상세, 프로모션 랜딩 | 노출 예정 | 메뉴 자리만 선반영된 상태 | 이벤트 메타/콘텐츠 |

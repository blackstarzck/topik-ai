...
- 시나리오 4: 운영자가 목록 또는 상세 Drawer에서 `게시 예약`, `종료`를 수행하고 결과를 검수합니다.
- 시나리오 5: 운영자가 상세 Drawer에서 참여 현황을 검수하고, `미리보기`로 별도 본문 Modal을 열어 랜딩 콘텐츠를 확인합니다.
- 시나리오 6: 운영자가 조치 후 `Target Type`, `Target ID` 기준으로 감사 로그를 확인하고 후속 화면 검수를 이어갑니다.
...
| 본문 목록 | 핵심 데이터 비교와 대상 선택 | 이벤트 ID, 이벤트명, 유형, 기간, 노출 상태, 참여자 수, 보상 정책 요약 | 행 클릭 상세, 미리보기 | 관련 화면과 연결 | 직접 또는 간접 영향 |
| 상세 Drawer | 세부 정보와 조치 근거 확인 | 기본 정보, 이벤트 요약, 노출 설정, 참여 조건, 참여 현황, 보상 정책, 메시지/쿠폰 연동, 운영 메모 | 미리보기, 수정, 게시 예약, 종료, 감사 로그 확인 | 감사 로그와 연결 | 직접 또는 간접 영향 |
| 등록 상세 페이지 | 작성과 저장 책임 분리 | 기본 정보, 이벤트 본문, 노출 설정, 참여 조건, 보상 설정, 선택형 SEO/공유 메타, 관리자 메모 | 신규는 임시 저장, 수정은 저장, 게시 예약, 미리보기 | 관련 연동 설정 편집 | 직접 영향 |
...
- 섹션 5. 참여 현황
  - 현재 참여자 수, 참여 제한, 운영 검수용 참여 규모 확인
- 섹션 6. 보상 정책
...
- 섹션 8. 운영 메모/변경 이력
  - 관리자 메모, 최근 수정 정보
- 이벤트 본문은 Drawer 안에서 직접 렌더링하지 않고, `미리보기` 액션으로 별도 Modal에서 확인합니다.
- 미리보기 Modal은 공지사항/메시지와 같은 공용 본문 미리보기 레이아웃을 사용하고, 푸터에 `본문 수정하기`, `닫기` 버튼을 둡니다.
...
- 섹션 3. 노출 설정
  ...
  - 현재 구현은 `Upload + drag sorting uploadList`로 배너 이미지를 첨부하고, 첫 번째 이미지를 대표 배너로 사용합니다.
  - DB/API 스키마는 URL 단일 필드가 아니라 정렬 가능한 `bannerImages[]` 또는 asset list를 SoT로 두고, 대표 배너 파생 규칙을 함께 유지합니다.
- 섹션 4. 참여 조건
  - 대상 그룹, 참여 제한, 안내 문구
  - 현재 구현은 `Message > 대상 그룹`의 `MessageGroup`을 `select`로 선택합니다.
  - `targetGroupId`를 기준값으로 저장하고, 그룹명은 선택 결과에서 파생해 보여주거나 snapshot으로만 유지합니다.
- 섹션 5. 보상 설정
  - 보상 유형, 보상 정책, 지급 조건
  - 현재 구현은 메시지 템플릿과 보상 정책을 각각 `select` 기반 참조형 입력으로 사용합니다.
  - `rewardPolicyId`, `messageTemplateId`를 기준값으로 저장하고, 이름 필드는 파생 표시값 또는 snapshot 후보로만 취급합니다.
- 섹션 6. 노출/SEO 설정
  - `slug`
  - `공유 제목`
  - `공유 설명`

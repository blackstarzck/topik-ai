...
- 단어장 화면의 운영 목적, 데이터 블록, 조치 흐름을 같은 기준으로 정리합니다.
...
- 시나리오 1: 운영자가 단어장 화면에서 검색/필터를 적용하고 대상 레코드나 편집 대상을 선택합니다.
...
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 단어장 정보: 단어장명, 레벨, 언어, 상태, 운영 정보: 단어 수, 연결 콘텐츠, 최근 수정 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| --- | --- | --- | --- | --- | --- |
| 생성 | 수정 | Content + vocabularyId | 사유 권장 | 생성 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Content&targetId={vocabularyId} |
| 편집 | 수정 | Content + vocabularyId | 사유 권장 | 편집 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Content&targetId={vocabularyId} |
| 대량 등록 | 수정 | Content + vocabularyId | 사유 권장 | 대량 등록 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Content&targetId={vocabularyId} |
| 미리보기 | 조회 | Content + vocabularyId | 불필요 | 미리보기 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 발행 | 파괴적 | Content + vocabularyId | 확인 + 사유 필수 | 발행 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Content&targetId={vocabularyId} |
...
| --- | --- | --- | --- |
| Content > 소나기 | 하위 콘텐츠 타입 연결 | 연결 콘텐츠 탭 | 선행 관계 |
| Content > 객관식 선택 | 하위 콘텐츠 타입 연결 | 연결 콘텐츠 탭 | 선행 관계 |
...
| --- | --- | --- | --- |
| 단어장 학습 화면 | 노출 예정 | 학습 콘텐츠 편집 축 | 단어장 카테고리/항목 |

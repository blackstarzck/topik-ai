...
- 이커머스 관리 화면의 운영 목적, 데이터 블록, 조치 흐름을 같은 기준으로 정리합니다.
...
- 이커머스 관리 화면의 핵심 운영 데이터를 검색하고 검수합니다.
...
- 시나리오 1: 운영자가 이커머스 관리 화면에서 검색/필터를 적용하고 대상 레코드나 편집 대상을 선택합니다.
- 시나리오 2: 운영자가 상품 등록 흐름으로 세부 정보를 확인하고 필요한 조치를 실행합니다.
...
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 상품 정보: 상품 ID, 상품명, 유형, 가격, 상태, 운영 정보: 노출 상태, 판매량, 연결 쿠폰/포인트 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| --- | --- | --- | --- | --- | --- |
| 상품 등록 | 수정 | Commerce + productId | 사유 권장 | 상품 등록 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Commerce&targetId={productId} |
| 수정 | 수정 | Commerce + productId | 사유 권장 | 수정 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Commerce&targetId={productId} |
| 노출 제어 | 조회 | Commerce + productId | 불필요 | 노출 제어 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 판매 중지 | 조회 | Commerce + productId | 불필요 | 판매 중지 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
...
| --- | --- | --- | --- |
| Commerce > 쿠폰 관리 | 상품별 할인 정책 연결 | 상품-쿠폰 연결 | 선행 관계 |
| Commerce > 포인트 관리 | 포인트 적립/사용 정책 연결 | 상품-포인트 연결 | 선행 관계 |
...
| --- | --- | --- | --- |
| 상품 목록, 패키지 상세, 체크아웃 | 노출 예정 | Commerce B2C 허브 데이터 | 상품/패키지 카탈로그 |

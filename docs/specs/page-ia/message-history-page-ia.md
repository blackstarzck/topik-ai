...
| --- | --- | --- | --- | --- | --- |
| 상단 요약 | 운영 규모와 우선순위 파악 | 총 발송 건수, 성공 건수, 실패 건수, 재시도 필요 건수 | 없음 | 후속 화면 우선순위 결정 | 직접 또는 간접 영향 |
| 검색/필터 | 탐색 범위 축소 | 채널 탭, `전체/자동/수동` 단일 선택 체크 필터, 검색 대상, 검색어, 발송일 | 조건 변경, 초기화 | 후속 상세 대상 축소 | 직접 영향 없음 |
...
- 본문 레이아웃 메모: 채널 탭 아래 한 줄에서 `SearchBar` 검색 입력부, `전체/자동/수동` 단일 선택 체크 필터, `총 건수`와 `CSV` 내보내기를 순서대로 배치합니다. 카드 body는 안내 문구 -> 테이블 순서를 유지합니다.

...
| 검색/필터 | 탐색 범위 축소 | 채널: 메일/푸시, 발송 방식, 검색 대상, 검색어, 발송일 | 조건 변경, 초기화 | 후속 상세 대상 축소 | 직접 영향 없음 |
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 템플릿명, 발송 그룹, 수신자 수, 발송 성공, 발송 실패, 상태, 실행 시각 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
| 상세 영역 | 세부 정보와 조치 근거 확인 | 기본 정보, 수신자 샘플 테이블, 수신 상태 검색, 상세 Drawer, 수신자 샘플 테이블, CSV 내보내기 | 조회/저장/상태 변경 | 감사 로그와 연결 | 직접 또는 간접 영향 |
...
| --- | --- | --- | --- | --- | --- |
| 상세 보기 | 조회 | Message + historyId | 불필요 | 상세 보기 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
...
| 복제 발송 | 수정 | Message + historyId | 사유 권장 | 복제 발송 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Message&targetId={historyId} |
...
- 수신자 샘플 보존 기간과 CSV 내보내기 감사 여부 미정
## 15. 사용자 표시 규칙

- 상세 Drawer의 `수신자 샘플` 테이블은 사용자 셀을 raw ID로 두지 않고 `이름 (ID)` 형식의 파란 링크로 표시합니다.

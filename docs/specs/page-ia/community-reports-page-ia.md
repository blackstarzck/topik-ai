...
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 신고 ID, 대상 게시글, 대상 사용자 ID, 신고자, 신고 사유, 신고일, 처리 상태 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| 처리 완료 | 조회 | Community + reportId | 불필요 | 처리 완료 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |

...
- 신고 관리 화면의 운영 목적, 데이터 블록, 조치 흐름을 같은 기준으로 정리합니다.
...
- 신고 관리 화면의 핵심 운영 데이터를 검색하고 검수합니다.
...
- 시나리오 1: 운영자가 신고 관리 화면에서 검색/필터를 적용하고 대상 레코드나 편집 대상을 선택합니다.
...
| 검색/필터 | 탐색 범위 축소 | 검색어, 신고일 | 조건 변경, 초기화 | 후속 상세 대상 축소 | 직접 영향 없음 |
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 신고 ID, 대상 게시글, 대상 사용자 ID, 신고자, 신고 사유, 신고일, 처리 상태 | 행 클릭/조치 실행 | 관련 화면과 연결 | 간접 영향 |
| 상세 영역 | 세부 정보와 조치 근거 확인 | 상세 이동 또는 패널 | 조회/조치 결과 확인 | 감사 로그와 연결 | 직접 또는 간접 영향 |
...
| --- | --- | --- | --- | --- |
| 상태값/운영 규칙 | 확정 | Community 모듈의 현재 코드/문서 기준 상태값과 운영 규칙을 유지하고 변경 시 연관 화면과 문서를 함께 갱신해야 합니다. | 사용자 화면 문구와 상태 기준의 운영 원천이 됩니다. | 상태 세트 변경 시 연관 문서와 화면을 함께 갱신해야 합니다. |
...
| --- | --- | --- | --- |
| Community > 게시글 관리 | 대상 게시글 원본 검수 | 게시글 ID 링크 | 후행 관계 |
| Users > 회원 상세 | 대상 사용자 제재 이력 검수 | 사용자 ID 링크 | 후행 관계 |
...
- 목록 상태 메모: `처리 상태` 컬럼은 `처리 대기/처리 완료` 배지로 표시하고, 더보기 메뉴에는 `게시글 숨김`, `사용자 정지`만 유지합니다. `처리 완료`는 별도 조치가 아니라 실제 신고 조치 결과로만 반영합니다.
- 권한/로그 처리 메모: 파괴적 액션에는 확인 단계와 사유 입력, Target Type, Target ID, 감사 로그 확인 경로를 함께 둡니다.

## 14. 오픈 이슈

- 신고 우선순위 점수와 자동 조치 기준 미정
## 15. 사용자 표시 규칙

...
- 쿠폰 관리 화면의 운영 목적, 데이터 블록, 조치 흐름을 같은 기준으로 정리합니다.
...
- 쿠폰 관리 화면의 핵심 운영 데이터를 검색하고 검수합니다.
...
- 시나리오 1: 운영자가 쿠폰 관리 화면에서 검색/필터를 적용하고 대상 레코드나 편집 대상을 선택합니다.
...
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 쿠폰 기본 정보: 쿠폰명, 코드, 할인 유형, 할인값, 상태, 운영 정보: 유효기간, 발급 수, 사용 수, 대상 범위 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| --- | --- | --- | --- | --- | --- |
| 생성 | 수정 | Commerce + couponId | 사유 권장 | 생성 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Commerce&targetId={couponId} |
| 수정 | 수정 | Commerce + couponId | 사유 권장 | 수정 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Commerce&targetId={couponId} |
| 발급 | 조회 | Commerce + couponId | 불필요 | 발급 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 비활성화 | 파괴적 | Commerce + couponId | 확인 + 사유 필수 | 비활성화 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Commerce&targetId={couponId} |
| 사용 현황 조회 | 조회 | Commerce + couponId | 불필요 | 사용 현황 조회 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
...
| --- | --- | --- | --- |
| Operation > 이벤트 | 이벤트 보상/프로모션 쿠폰 연동 | 보상 정책 참조 | 선행/후행 관계 |
| Commerce > 이커머스 관리 | 적용 상품 연결 | 상품 연결 설정 | 선행 관계 |
...
| --- | --- | --- | --- |
| 쿠폰함, 결제 할인 적용, 프로모션 배너 | 노출 예정 | 정책 정의 후 B2C 할인 UX와 연결 | 쿠폰 정책 |

...
- 소나기 화면의 운영 목적, 데이터 블록, 조치 흐름을 같은 기준으로 정리합니다.
...
- 시나리오 1: 운영자가 소나기 화면에서 검색/필터를 적용하고 대상 레코드나 편집 대상을 선택합니다.
- 시나리오 2: 운영자가 템플릿 편집 흐름으로 세부 정보를 확인하고 필요한 조치를 실행합니다.
...
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 콘텐츠 정보: 콘텐츠명, 템플릿, 상태, 운영 정보: 연결 단어장, 최근 수정, 노출 위치 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| --- | --- | --- | --- | --- | --- |
| 템플릿 편집 | 수정 | Content + sonagiId | 사유 권장 | 템플릿 편집 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Content&targetId={sonagiId} |
| 미리보기 | 조회 | Content + sonagiId | 불필요 | 미리보기 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 연결 관리 | 조회 | Content + sonagiId | 불필요 | 연결 관리 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 발행 | 파괴적 | Content + sonagiId | 확인 + 사유 필수 | 발행 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Content&targetId={sonagiId} |
...
| --- | --- | --- | --- |
| 소나기 학습 화면 | 노출 예정 | 단어장 하위 콘텐츠 타입 | 소나기 콘텐츠 |

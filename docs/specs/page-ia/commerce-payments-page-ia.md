...
- 결제 내역 화면의 운영 목적, 데이터 블록, 조치 흐름을 같은 기준으로 정리합니다.
...
- 결제 내역 화면의 핵심 운영 데이터를 검색하고 검수합니다.
...
- 시나리오 1: 운영자가 결제 내역 화면에서 검색/필터를 적용하고 대상 레코드나 편집 대상을 선택합니다.
- 시나리오 2: 운영자가 회원 상세 이동 흐름으로 세부 정보를 확인하고 필요한 조치를 실행합니다.
...
| --- | --- | --- | --- | --- | --- |
| 상단 요약 | 운영 규모와 우선순위 파악 | 전체 결제 건수, 결제 완료 금액, 환불 관련 건수 | 없음 | 후속 화면 우선순위 결정 | 직접 또는 간접 영향 |
| 검색/필터 | 탐색 범위 축소 | 검색어(결제 ID/회원 ID/닉네임/상품명), 결제일 | 조건 변경, 초기화 | 후속 상세 대상 축소 | 직접 영향 없음 |
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 결제 ID, 회원, 상품, 금액, 결제 수단, 결제일, 상태 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| --- | --- | --- | --- | --- | --- |
| 회원 상세 이동 | 조회 | Commerce + paymentId | 불필요 | 회원 상세 이동 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 환불 관리 연계 | 파괴적 | Commerce + paymentId | 확인 + 사유 필수 | 환불 관리 연계 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Commerce&targetId={paymentId} |
| 상세 보기 | 조회 | Commerce + paymentId | 불필요 | 상세 보기 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
...
| --- | --- | --- | --- |
| Commerce > 환불 관리 | 동일 거래 원본 환불 상태 검증 | 결제 ID 링크 | 선행/후행 관계 |
| Users > 회원 상세 | 회원 단위 결제 이력 검수 | 회원 ID 링크 | 후행 관계 |
...
| --- | --- | --- | --- |
| 결제 완료, 구독 상태, 마이페이지 결제 내역 | 운영상 추정 | 환불 관리와 동일 원본을 공유하며, 환불 승인 시 사용자 결제 상태와 회원 상세 결제 탭에 함께 반영되는 원천 데이터 | 결제 거래 목록 |

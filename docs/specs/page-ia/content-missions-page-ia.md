...
- 학습 미션 화면의 운영 목적, 데이터 블록, 조치 흐름을 같은 기준으로 정리합니다.
...
- 학습 미션 화면의 핵심 운영 데이터를 검색하고 검수합니다.
...
- 시나리오 1: 운영자가 학습 미션 화면에서 검색/필터를 적용하고 대상 레코드나 편집 대상을 선택합니다.
...
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 미션 정보: 미션명, 목표, 기간, 상태, 운영 정보: 보상, 참여율, 완료율 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| --- | --- | --- | --- | --- | --- |
| 생성 | 수정 | Content + missionId | 사유 권장 | 생성 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Content&targetId={missionId} |
| 수정 | 수정 | Content + missionId | 사유 권장 | 수정 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Content&targetId={missionId} |
| 대상 설정 | 조회 | Content + missionId | 불필요 | 대상 설정 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 보상 연결 | 조회 | Content + missionId | 불필요 | 보상 연결 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 수동 지급/회수 | 파괴적 | Content + missionId | 확인 + 사유 필수 | 수동 지급/회수 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Content&targetId={missionId} |
...
| --- | --- | --- | --- |
| Content > 배지 | 미션 보상 배지 연결 | 보상 연결 설정 | 선행 관계 |
| Commerce > 포인트 관리 | 미션 보상 포인트 정책 연결 | 보상 정책 참조 | 선행 관계 |
...
| --- | --- | --- | --- |
| 미션 화면, 리워드 영역 | 노출 예정 | 배지/포인트와 연결 가능 | 미션 정의 |

...
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 게시글 ID, 제목, 작성자, 게시판, 작성일, 조회수, 신고 수, 상태 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| 게시글 숨김 | 파괴적 | Community + postId | 정책 코드 선택 + 확인 + 사유 필수 | 게시글 숨김 완료 후 대상 식별 정보, 정책 코드, 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Community&targetId={postId} |
...
| 조치 사유 표준화 | 확정 | 게시글 숨김/삭제 시 운영자는 정책 코드 Select와 자유 사유를 함께 입력합니다. | B2C에는 직접 노출되지 않지만 운영 판단 근거와 통계 기준의 원천이 됩니다. | 정책 코드 세트 변경 시 문서와 액션 UI를 함께 갱신해야 합니다. |
...
- 상세 Drawer 레이아웃 메모: 헤더 오른쪽은 상태 메타만 두고, 푸터 왼쪽은 `감사 로그 확인`, 푸터 오른쪽은 게시글 조치 버튼을 배치합니다. 공통 기준은 `docs/guidelines/admin-detail-drawer-guidelines.md`를 따릅니다.

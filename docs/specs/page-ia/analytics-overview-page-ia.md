...
- 통계 개요 화면의 운영 목적, 데이터 블록, 조치 흐름을 같은 기준으로 정리합니다.
...
- 통계 개요 화면의 핵심 운영 데이터를 검색하고 검수합니다.
...
- 시나리오 1: 운영자가 통계 개요 화면에서 검색/필터를 적용하고 대상 레코드나 편집 대상을 선택합니다.
- 시나리오 2: 운영자가 모듈별 상세 통계 이동 흐름으로 세부 정보를 확인하고 필요한 조치를 실행합니다.
...
| --- | --- | --- | --- | --- | --- |
| 상단 요약 | 운영 규모와 우선순위 파악 | 사용자 지표, 메시지 지표, 커뮤니티 지표, 매출 지표 | 없음 | 후속 화면 우선순위 결정 | 직접 또는 간접 영향 |
| 검색/필터 | 탐색 범위 축소 | 기간 | 조건 변경, 초기화 | 후속 상세 대상 축소 | 직접 영향 없음 |
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 모듈별 핵심 지표 테이블, 이상 징후 Alert | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| --- | --- | --- | --- | --- | --- |
| 모듈별 상세 통계 이동 | 조회 | Analytics + metricKey | 불필요 | 모듈별 상세 통계 이동 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 대시보드 이동 | 조회 | Analytics + metricKey | 불필요 | 대시보드 이동 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |

## 8. 상태값/정책/운영 규칙

| 항목 | 현재 상태 | 관리자 페이지 영향 | 사용자 화면 영향 | 추후 결정 필요 내용 |
| --- | --- | --- | --- | --- |
| 상태값/운영 규칙 | 확정 | Analytics 모듈의 현재 코드/문서 기준 상태값과 운영 규칙을 유지하고 변경 시 연관 화면과 문서를 함께 갱신해야 합니다. | 사용자 화면 문구와 상태 기준의 운영 원천이 됩니다. | 상태 세트 변경 시 연관 문서와 화면을 함께 갱신해야 합니다. |
...
| --- | --- | --- | --- |
| Dashboard > 대시보드 | 운영 큐와 분석 지표를 상호 보완해 확인 | 대시보드 이동 링크 | 양방향 관계 |
| Commerce > 결제 내역 | 매출/환불 지표 원본 검증 | 커머스 상세 이동 | 후행 관계 |
| Message > 발송 이력 | 메시지 성공률 원본 검증 | 메시지 상세 이동 | 후행 관계 |
...
| --- | --- | --- | --- |
| 일부 B2C 직접 노출 가능성 낮음 | 내부 전용 | 현재는 운영 판단용 지표이며, 결제·환불·메시지 운영 원본과 연결된 요약 값을 보여준다 | 집계 지표 |
...
- 예상 feature 파일: src/features/analytics/pages/*

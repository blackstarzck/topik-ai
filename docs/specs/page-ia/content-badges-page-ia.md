...
- 배지 화면의 운영 목적, 데이터 블록, 조치 흐름을 같은 기준으로 정리합니다.
...
- 시나리오 1: 운영자가 배지 화면에서 검색/필터를 적용하고 대상 레코드나 편집 대상을 선택합니다.
...
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 배지 정보: 배지명, 아이콘, 상태, 노출 순서, 운영 정보: 획득 조건, 획득자 수, 연결 미션 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| --- | --- | --- | --- | --- | --- |
| 생성 | 수정 | Content + badgeId | 사유 권장 | 생성 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Content&targetId={badgeId} |
| 수정 | 수정 | Content + badgeId | 사유 권장 | 수정 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Content&targetId={badgeId} |
| 노출 제어 | 조회 | Content + badgeId | 불필요 | 노출 제어 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 수동 지급/회수 | 파괴적 | Content + badgeId | 확인 + 사유 필수 | 수동 지급/회수 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Content&targetId={badgeId} |
...
| --- | --- | --- | --- |
| Content > 학습 미션 | 미션 보상 배지 연결 | 보상 연결 설정 | 선행 관계 |
...
| --- | --- | --- | --- |
| 마이페이지 성취, 학습 프로필 | 노출 예정 | 보상/동기부여 UI와 직접 연결 | 배지 정의 |

# Assessment > 문제은행 상세 IA

## 1. 문서 목적

- 문제은행 화면의 운영 목적, 데이터 블록, 조치 흐름을 같은 기준으로 정리합니다.
...
- 문제은행 화면의 핵심 운영 데이터를 검색하고 검수합니다.
...
- 시나리오 1: 운영자가 문제은행 화면에서 검색/필터를 적용하고 대상 레코드나 편집 대상을 선택합니다.
...
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 문항 정보: 문항 ID, 유형, 난이도, 상태, 출처, 운영 정보: 사용 횟수, 검수 상태, 버전 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| --- | --- | --- | --- | --- | --- |
| 생성 | 수정 | Assessment + questionId | 사유 권장 | 생성 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Assessment&targetId={questionId} |
| 수정 | 수정 | Assessment + questionId | 사유 권장 | 수정 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Assessment&targetId={questionId} |
| 검수 | 수정 | Assessment + questionId | 사유 권장 | 검수 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Assessment&targetId={questionId} |
| 복제 | 수정 | Assessment + questionId | 사유 권장 | 복제 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Assessment&targetId={questionId} |
| 시험 연결 | 조회 | Assessment + questionId | 불필요 | 시험 연결 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
...
| --- | --- | --- | --- |
| Assessment > EPS TOPIK | 문항 풀을 시험 세트에 배정 | 문항 연결 | 선행 관계 |
| Assessment > 레벨 테스트 | 테스트 세트 원천 문항 | 문항 연결 | 선행 관계 |
...
| --- | --- | --- | --- |
| 시험/문제 풀이 화면 데이터 원천 | 노출 예정 | 직접 테이블 노출이 아니라 시험 흐름으로 소비될 가능성 큼 | 문항 풀/카테고리 |

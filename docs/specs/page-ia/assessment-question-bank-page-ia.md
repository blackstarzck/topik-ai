...
> **2026-08-01 철회 — 이 페이지에는 기관 배정 진입점이 없다.** 2026-06-26 에 문항 중심 기관 노출 설정(테이블 `기관 노출` 컬럼, 더보기 `기관 노출 설정`, 일괄 `기관 배정`/`배정 해제`)을 제공하기로 문서화했으나 **구현된 적이 없고**(서비스 계층 wrapper 3종만 존재, 호출자 0), 오너 결정으로 만들지 않는다. 기관별 배정은 `Users > 기관 코드`의 `노출 문항` 모달(기관 중심) 단일 경로로만 관리한다. 아래 규칙은 그 기관 중심 경로에 적용되며, 문항 축 UI 를 나중에 되살릴 경우의 참고 계약으로만 남긴다. 단, `service_status`가 전역 최상위 노출 조건이므로 `available`이 아닌 문항(`excluded`, `internal_test`)은 새 기관 매핑 추가가 RPC에서 `blocked` 처리된다. 기존 매핑은 삭제하지 않고 `전역 미노출`으로 표시하며 제거/해제는 허용한다. 셀 라벨은 매핑 0건이면 `미배정`, N건이면 `기관 N곳 배정`이다 — 학습자 규칙이 기관 할당제이므로 `미배정` 문항은 기관 소속 학습자에게 보이지 않고, 배정된 문항도 무소속 학습자에게는 계속 보인다(구 라벨 `전체 공개`/`기관 한정`은 양방향 모두 거짓이라 2026-07-30 폐기).
...
- 표시 축은 **주제(`topic_main`/`topic_detail`)·난이도(1~6)·유형·노출 상태(`service_status`)·태그**다(기관 배정은 이 페이지 축이 아니다 — 2026-08-01 확정). 문항 품질·상태 표현은 태그로만 한다.
...
- 노출 상태·태그 조치가 필요한 문항을 이 페이지의 더보기 메뉴와 일괄 조치 바에서 바로 처리한다. 기관 배정 조치는 이 페이지에 없다.
...
- 태그 부여/제거와 `service_status` 노출 통제는 통합된 이 화면의 관리 책임이다. 기관별 배정은 이 화면 책임이 아니다 — 기관별 배정은 `Users > 기관 코드`의 `노출 문항` 모달(기관 중심) 단일 경로로만 관리한다.
...
- 시나리오 3: 운영자가 목록에서 태그 부여/제거 또는 노출 상태 전환이 필요하다고 판단하면 더보기 메뉴 또는 선택 행 일괄 조치 바에서 처리한다. 2depth 상세는 공급 메타데이터와 현재 문항 상태를 조회 전용으로 확인하는 화면이다.
...
| 목록 테이블 | 수신 문항 비교·열람 | 문항 번호, 문항 ID, 주제, 난이도, TOPIK 급수, **노출 상태**(`service_status` Tag), 태그, 최근 수정, `버전` 수정 횟수 (구 검수 상태 컬럼은 재정의 P3에서 제거 완료) | 문항 ID/더보기로 상세 진입, 노출 상태/태그 조치, 기관 노출 설정, 수정 이력 행 확장 |
| 확장 변경 이력 | 승격된 과거 내용 버전 역추적 | 버전 ID, 원본 생성 시각, **원본 수정 시각**(헤더 안내 툴팁), 최초 수신, 마지막 수신, **수신 횟수**(헤더 안내 툴팁), content hash, payload hash | 행 클릭/키보드 Enter로 과거 상세 진입, 헤더 안내 아이콘 hover/focus |
...
- ~~`institutionExposure`~~ — 이 페이지에 노출하지 않는다(2026-08-01 확정). 기관별 배정 현황은 `Users > 기관 코드` 목록의 `노출 모드` 컬럼과 `노출 문항` 모달에서 본다.
- `recommendationKeys`
- `updatedAt`
...
- 목록 화면의 쓰기 액션은 태그 부여/제거와 노출 상태 전환이다(기관 배정 제외 — 2026-08-01 확정). 모두 `AssessmentQuestion + questionId` 감사 로그 계약을 따른다(`docs/specs/admin-action-log.md`).
- 기관 배정 감사 액션 `question_institutions_changed`/`question_institutions_cleared` 는 Target 이 `AssessmentQuestion + questionId` 이지만 **이 페이지가 아니라** `Users > 기관 코드` 경로에서 발생한다. 결과 shape 는 결과 shape는 `{total, changed, unchanged, blocked, failed, details, batch_id}`다.

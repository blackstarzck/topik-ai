...
- 콘텐츠 관리 화면의 운영 목적, 데이터 블록, 조치 흐름을 같은 기준으로 정리합니다.
...
- 콘텐츠 관리 화면의 핵심 운영 데이터를 검색하고 검수합니다.
...
- 시나리오 1: 운영자가 콘텐츠 관리 화면에서 검색/필터를 적용하고 대상 레코드나 편집 대상을 선택합니다.
...
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | 콘텐츠 정보: 콘텐츠 ID, 제목, 유형, 카테고리, 상태, 운영 정보: 노출 위치, 난이도, 최근 수정 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
| --- | --- | --- | --- | --- | --- |
| 생성 | 수정 | Content + contentId | 사유 권장 | 생성 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Content&targetId={contentId} |
| 수정 | 수정 | Content + contentId | 사유 권장 | 수정 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Content&targetId={contentId} |
| 태그 관리 | 조회 | Content + contentId | 불필요 | 태그 관리 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 미리보기 | 조회 | Content + contentId | 불필요 | 미리보기 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 노출 제어 | 조회 | Content + contentId | 불필요 | 노출 제어 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
...
| --- | --- | --- | --- |
| Content > 단어장 | 연결 자산과 하위 콘텐츠 검수 | 연결 자산 탭 | 선행/후행 관계 |
| Content > 학습 미션 | 미션 연결 콘텐츠 검수 | 연결 자산 링크 | 후행 관계 |
...
| --- | --- | --- | --- |
| 학습 콘텐츠 목록, 상세 | 노출 예정 | Content 허브 데이터 | 콘텐츠 카탈로그 |

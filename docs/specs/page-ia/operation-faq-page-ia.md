...
- FAQ 화면의 운영 목적, 데이터 블록, 조치 흐름을 같은 기준으로 정리합니다.
...
- 시나리오 1: 운영자가 FAQ 화면에서 검색/필터를 적용하고 대상 레코드나 편집 대상을 선택합니다.
- 시나리오 2: 운영자가 FAQ 등록 흐름으로 세부 정보를 확인하고 필요한 조치를 실행합니다.
...
| --- | --- | --- | --- | --- | --- |
| 상단 요약 | 운영 규모와 우선순위 파악 | 전체 FAQ 수, 공개 FAQ 수, 비공개 FAQ 수 | 없음 | 후속 화면 우선순위 결정 | 직접 또는 간접 영향 |
| 검색/필터 | 탐색 범위 축소 | 본문 상단 SearchBar, 우측 끝 `FAQ 등록` 버튼, 검색어, 최종 수정일 | 조건 변경, 초기화, 등록 | 후속 상세 대상 축소 | 직접 영향 없음 |
| 본문 영역 | 핵심 데이터 비교와 대상 선택 | FAQ ID, 질문, 카테고리, 공개 상태, 최종 수정일 | 행 클릭/편집 | 관련 화면과 연결 | 간접 영향 |
...
### 검색/선택 데이터
- 본문 상단 SearchBar 우측 끝 `FAQ 등록` 버튼
- 검색어
- 최종 수정일
...
| --- | --- | --- | --- | --- | --- |
| FAQ 등록 | 수정 | Operation + faqId | 사유 권장 | FAQ 등록 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Operation&targetId={faqId} |
| 수정 | 수정 | Operation + faqId | 사유 권장 | 수정 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Operation&targetId={faqId} |
| 공개/비공개 | 조회 | Operation + faqId | 불필요 | 공개/비공개 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
| 삭제 | 파괴적 | Operation + faqId | 확인 + 사유 필수 | 삭제 완료 후 대상 식별 정보와 후속 확인 경로를 안내합니다. | /system/audit-logs?targetType=Operation&targetId={faqId} |
| 상세 보기 | 조회 | Operation + faqId | 불필요 | 상세 보기 결과 패널을 열거나 관련 화면으로 이동합니다. | 조회 액션이므로 별도 감사 로그는 필요하지 않거나 원본 화면 흐름을 사용합니다. |
...
| --- | --- | --- | --- |
| 고객센터 FAQ, 도움말 | 운영상 추정 | 검색/카테고리/공개 상태 기준으로 B2C 도움말 노출을 제어하는 원천 데이터 | FAQ 콘텐츠 |

...
| --- | --- | --- | --- | --- |
| 2026-04-02 | `Assessment > TOPIK 쓰기 문제은행` | JSON SoT 기준 재정의와 synthetic 필드 제거 | `docs/specs/page-ia/assessment-question-bank-page-ia.md`, `docs/specs/admin-page-tables.md`, `docs/specs/admin-data-contract.md`, `docs/specs/admin-action-log.md`, `docs/architecture/admin-data-source-transition.md`, `docs/specs/admin-data-usage-map.md`, `docs/specs/admin-page-gap-register.md` | 외부 JSON 파일을 repo fixture로 가져와 목록/상세/감사 로그 계약을 `id`, `meta`, `review_workflow`, `prompt_text`, `review_passed` 중심으로 다시 정의했다. `문항 관리` 탭, 문제 번호 필터, 공개/자동 점검/검수 상태 같은 synthetic 운영 메타와 `AQ-`형 legacy fixture를 SoT에서 제거하고, `review_memo 저장 + 검수 통과/미통과` 흐름만 남겼다. |
| 2026-04-02 | `Assessment > TOPIK 쓰기 문제은행` | 문항 관리 공개 상태 Switch 전환, Drawer 제거, 목록 로딩 UX 정렬 | `docs/specs/page-ia/assessment-question-bank-page-ia.md`, `docs/specs/admin-page-tables.md`, `docs/specs/admin-data-contract.md`, `docs/specs/admin-action-log.md`, `docs/specs/admin-page-gap-register.md`, `docs/architecture/admin-data-source-transition.md`, `docs/specs/admin-data-usage-map.md` | 문항 관리 탭의 `운영 상태`/상세 Drawer 흐름을 제거하고, `검수 완료` 문항만 `공개/비공개` Switch로 제어하는 계약으로 바꿨다. 검수 큐와 문항 관리 테이블에서 액션 컬럼을 제거하고, 문항 미리보기 hover Popover 하단의 `검수하기` CTA를 2depth 진입 기본 동선으로 고정했다. 동시에 초기 pending을 다른 1depth 목록과 같은 `AdminDataTable` loading 오버레이 + empty 조합으로 맞췄다. |

...
| `POL-017` | TOPIK 쓰기 문항 수신·관리 운영정책 (2026-06-11 인바운드 전환 재정의 — 구 "검수·배포" 모델 폐기) | 운영/콘텐츠 | 문서 추적 | `Assessment > TOPIK 쓰기 문항 목록`, `Assessment > TOPIK 쓰기 문항 관리`, `System > 감사 로그` | `docs/architecture/metadata-tag-schema-transition-decision-record.md`(§0: 수신(외부 공급 API, 미개발) → 적재(Supabase) → 관리 포인트(태그)+노출 통제(`service_status`) → v13 read-only), `docs/architecture/admin-data-source-transition.md` §10.3, `docs/specs/page-ia/assessment-question-bank-page-ia.md`, `docs/specs/page-ia/assessment-question-manage-page-ia.md` |

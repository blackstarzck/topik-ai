...
| `POL-018` | TOPIK 쓰기 태그 운영·노출 제외 기준 (2026-06-11 개정 — 검수 결합 기준 ① 삭제) | 운영/콘텐츠 | 문서 추적 | `Assessment > TOPIK 쓰기 문항 관리`, `Assessment > TOPIK 쓰기 문항 목록`, `System > 감사 로그` | `docs/architecture/metadata-tag-schema-transition-decision-record.md`(D-6: `service_status` 단일 노출 상태, 노출 제외 기준 — ②운영주의 태그 활성 시 사유 필수 ③반복과다 `excluded` 권고, 노출상태 태그 그룹 시드 제외), `docs/metadata-tag-schema-rule.md` §2.2 |

...
| `POL-017` | TOPIK 쓰기 문항 수신·관리·버전 운영정책 (2026-07-15 버전 노출 규칙 확정 — 구 "검수·배포" 모델 폐기 유지) | 운영/콘텐츠 | 정책 확정·구현 추적 | `Assessment > TOPIK 쓰기 문항 목록`, `Assessment > 가져온 문항(인박스)`, `System > 감사 로그`, v13 쓰기 풀이·임시저장·북마크·결과 | `docs/architecture/metadata-tag-schema-transition-decision-record.md`(§0: 인바운드 수신 모델), `docs/architecture/writing-question-version-policy.md`(문항 버전·사용자 상태별 노출 SoT), `docs/architecture/admin-data-source-transition.md` §10.3, `docs/specs/page-ia/assessment-question-bank-page-ia.md`, `docs/specs/page-ia/assessment-question-bank-imported-page-ia.md` |
| `POL-018` | TOPIK 쓰기 태그 운영·노출 제외 기준 (2026-06-11 개정 — 검수 결합 기준 ① 삭제) | 운영/콘텐츠 | 코드 반영 (P4 관리 포인트 개방, 2026-06-11 — ② `available` 전환 모달의 운영주의 활성 태그 검사·경고+사유 필수, ③ 반복방지 활성 과다(임계 2) `excluded` 권고 표시, '서비스_노출상태' 그룹 facade 필터+RPC 부여 차단) | `Assessment > TOPIK 쓰기 문항 관리`, `Assessment > TOPIK 쓰기 문항 목록`, `System > 감사 로그` | `docs/architecture/metadata-tag-schema-transition-decision-record.md`(D-6: `service_status` 단일 노출 상태, 노출 제외 기준 — ②운영주의 태그 활성 시 사유 필수 ③반복과다 `excluded` 권고, 노출상태 태그 그룹 시드 제외), `docs/metadata-tag-schema-rule.md` §2.2, `src/features/assessment/pages/assessment-question-manage-page.tsx`, `src/features/assessment/ui/question-tag-edit-modal.tsx` |

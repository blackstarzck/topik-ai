...
- 문제 유형별 비교 컬럼: 문제 유형, 학습자, 제출자, 제출 수, 완료율, 평균 환산 점수, 조회율, 풀이 시간, 재제출률, PDF 내보내기 수. 데스크톱 2열에서는 점수 분포 카드와 같은 높이를 공유하고 표 viewport·가로 스크롤 영역이 안내 문구를 제외한 카드 본문의 남는 높이를 채우되, 데이터 행 높이는 늘리지 않습니다. 1024px 이하 세로 전환 시에는 콘텐츠 자연 높이로 복원합니다.
- 보조 블록: 4구간 점수 분포(차트/표), 주제별 성과(문제 유형×주제별 정보와 제출 수 막대를 단일 행에서 1:1 대응하고 행별 제출 수 내림차순으로 표시하며 현재 결과의 최대 제출 수를 막대 기준으로 사용), PDF 사용 분석(한 카드 안에서 좌측 `전체 구성 비율`의 전체/직접 귀속/혼합/미분류 요약·전체 구성 파이 차트와 우측 `문제 유형별 구성과 주제 상세`의 Ant Design 확장형 계층표를 병렬 배치하고 데스크톱에서는 두 내부 섹션의 상·하단을 같은 높이로 정렬하며 1024px 이하에서 세로 전환 후 자연 높이 사용). 파이 하단의 별도 범례 카드는 두지 않습니다. PDF 계층형 구성표 컬럼은 `구성`, `대주제`, `세부 주제`, `내보내기 완료 수(전체 이벤트 기준 건수·비율)`이며 헤더·본문·hover에 행 배경색을 사용하지 않습니다. Ant Design 기본 확장 버튼으로 51~54번 부모 행을 펼치면 주제 자식 행이 완료 수 내림차순으로 나타납니다. 혼합·미분류는 `주제 분석 불가`로 유지하고 특정 주제로 배분하지 않습니다. 취약 평가 차원 블록은 2026-07-15 오너 지시로 제거.

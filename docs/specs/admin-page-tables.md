...
- 구현 컬럼: 코드, 이름, 종류, 상태, 노출 모드, 가입 수, 생성일, 액션
  - `노출 모드` 셀: `제한 없음` 또는 `배정분만` Tag + 배정 건수 종속 표기(`제한 없음` 이면 `배정 N건 보존` — 모드 복귀 시 살아난다는 의미). 기관 축 설정이며 문항 축 라벨(`미배정`/`기관 N곳 배정`)과 다른 축이다. 컬럼 헤더에 값별 설명을 병기한다. 전환은 상세 `노출 문항` 탭의 **노출 설정 Drawer** 안 라디오 2안(사유 필수)이고, 배정 0건 + 회원 1명 이상으로 `배정분만` 전환은 차단한다. 생성 **페이지**에는 실제 기본값인 `배정분만`을 읽기 전용으로 안내한다.
- 구현 필터/정렬: 종류/상태/노출 모드 컬럼 필터, 코드/이름/가입 수/생성일 정렬
- 구현 액션: `코드 생성`(→ `/create` 페이지), 액션 셀 `더보기` 메뉴의 `회원 관리`, `노출 문항`, `수정`(각각 상세 `?tab=` 딥링크), 하단 분리 `삭제`
- 레이아웃 메모: 별도 SearchBar 없이 `AdminListCard.toolbar` 오른쪽에 총 건수/활성 건수/누적 가입 수와 `코드 생성` 버튼을 같은 줄에 배치하며, `코드 생성` 버튼은 `large` 크기를 사용합니다.
- 입력 화면: 생성은 전용 페이지(`/users/institution-codes/create`), 수정은 상세 `기본 정보` 탭입니다(2026-08-03 모달 → 페이지 전환). 둘 다 `Descriptions` 기반 입력 테이블을 쓰고 필수 필드 label 에 빨간 `*` 를 표시하며, 수정은 사유 입력이 필수입니다.
- 상세 탭 구조(2026-08-06): 각 탭은 **툴바(요약 지표 + 주요 액션 + 설정 Drawer 트리거) + 본문(현황)** 입니다. 설정 편집은 Drawer 3종(회원 초대 / 회원 정책 / 노출 설정)에 모으고, 판단에 필요한 값과 현황 경보는 툴바·본문에 남깁니다. 배경과 실측치는 IA `§13 오픈 이슈` 의 "탭 배치 원칙"에 있습니다.
- 파괴적 액션: `삭제`는 shared `TableActionMenu` footer의 danger 버튼으로 최하단에 배치하고, footer 상단 구분선으로 일반 액션과 분리합니다. 클릭 후 `ConfirmAction`으로 확인 + 사유 입력을 요구합니다. 가입 수가 1명 이상인 코드는 삭제 전에 상세 `회원` 탭에서 소속 해제가 필요하며, 서버 RPC도 삭제를 차단합니다. 삭제 성공 시 문항 배정과 기관 노출 모드 원장을 함께 제거해 같은 코드를 재생성해도 이전 설정이 되살아나지 않습니다.
...
- 본문 탭: 정책(설정형), 초기화(목록 운영형) (`/operation/pdf-quota`, `?tab=resets`)
...
- 주요 액션: `상세 보기`, `노출 가능`, `노출 제외`, `내부 테스트`, `태그 편집`. **기관 배정 진입점은 이 페이지에 없다** — 문항 중심 진입점은 만들지 않기로 확정했다(2026-08-01 오너 결정). 기관별 배정은 `Users > 기관 코드`의 `노출 문항` 모달(기관 중심) 단일 경로로만 관리한다. 아래 기관 배정 규칙은 그 기관 중심 경로에 적용된다. 기관 신규 추가는 `service_status='available'` 문항만 허용하고, 기존 기관 매핑 제거는 문항 상태와 무관하게 허용합니다.
...
- 2026-06-26 갱신(**2026-08-01 철회**): 당시 `/assessment/question-bank`에 기관 컬럼과 배정 진입점을 다시 제공하기로 문서화했으나 구현된 적이 없고, 문항 중심 진입점은 만들지 않기로 확정했다(2026-08-01 오너 결정). 기관별 배정은 `Users > 기관 코드`의 `노출 문항` 모달(기관 중심) 단일 경로로만 관리한다. 같은 `topik_writing_question_institution_exposure` 매핑을 `Users > 기관 코드`에서도 기관 중심으로 관리합니다.
- 현재 상태: 구현됨 (`topik_writing` canonical 4테이블 + 추천 뷰가 유일한 운영 조회 source. `VITE_QUESTION_BANK_SOURCE`와 `public.problems` read adapter는 제거됐고 JSON fixture/store fallback 없음)
...
- 컬럼: 문항 번호, 문항 ID, 주제, 난이도, TOPIK 급수, 노출 상태, 태그(활성 태그 수 + `태그 편집` 버튼), 기관 노출, 최근 수정, 더보기 (구 `검수 상태`·`사용 현황` 컬럼은 재정의 P3에서 제거 완료)
  ...
  - ~~`기관 노출` 셀/모달~~ **미제공(2026-08-01 확정)** — 이 페이지에는 기관 배정 진입점이 없다. 기관별 배정은 `Users > 기관 코드`의 `노출 문항` 모달 단일 경로다. 아래 라벨 계약은 문항 축 UI 를 나중에 되살릴 경우의 참고로만 남긴다: 매핑이 없으면 `미배정`, 매핑이 있으면 `기관 N곳 배정`으로 표시한다. 라벨 축은 "누가 보는가"가 아니라 "몇 개 기관에 배정했나"다 — 학습자 규칙은 기관 할당제이므로 `미배정` 문항은 기관 소속 학습자에게 보이지 않고, `기관 N곳 배정` 문항도 무소속 학습자에게는 계속 보인다. 구 라벨 `전체 공개`/`기관 한정`은 두 방향 모두 거짓이어서 폐기했다(2026-07-30). 컬럼 헤더에는 규칙 한 줄을 함께 노출한다: "소속 없는 학습자는 노출 허용한 문항을 모두 봅니다. 기관 소속 학습자는 그 기관에 배정한 문항만 봅니다." `service_status!='available'` 문항은 `전역 미노출` 태그를 함께 표시한다(기관 단위가 아니라 서비스 전역 상태를 가리키므로 `현재 미노출`에서 개명). 단건 모달과 일괄 지정은 신규 기관 추가 시 전역 노출 상태를 서버 RPC에서 검증하며, `excluded`/`internal_test` 문항은 `blocked` 결과로 안내한다. 제거/전체 해제는 stale 매핑 정리를 위해 상태와 무관하게 허용한다.
- 행 클릭: 별도 동작 없음
- write 경로: 신규 RPC 단일 경로 — `admin_update_topik_question`(service_status)/`admin_assign_question_tag`/`admin_remove_question_tag`/`admin_set_writing_question_institutions`/`admin_clear_writing_question_institutions`. 직접 테이블 write는 RLS 전면 차단(쓰기 정책 0건 — P4-4 네거티브 검증). 구 "준비 중" 안내 Alert는 P4 개방으로 제거됨.
- 주요 액션: `노출 가능`, `노출 제외`, `내부 테스트`(감사 `service_status_changed` — 사유 필수), `태그 부여`(`tag_assigned`), `태그 제거`(`tag_removed` — 사유 없음), `기관 노출 설정`/`기관 배정`/`배정 해제`(`question_institutions_changed`/`question_institutions_cleared` — 사유 필수) — 모두 `AssessmentQuestion + questionId` 감사 기록.
...
- 통합 메모: 조회와 관리 포인트는 `/assessment/question-bank` 단일 페이지(#19)로 통합되어 있으며, 이 #19-1 절은 구 문서 호환용 역사 항목입니다.
...
- 문제 유형별 비교 컬럼: 문제 유형, 학습자, 제출자, 제출 수, 완료율, 평균 환산 점수, 조회율, 풀이 시간, 재제출률, PDF 내보내기 수. 비교 표와 점수 분포 차트/표는 각각 콘텐츠 자연 높이를 사용하고 서로 같은 높이로 강제하지 않습니다. 부모 분석 행은 두 카드 중 더 큰 높이를 포함해 다음 섹션과 겹치지 않게 하며, 1024px 이하에서는 두 카드를 세로로 전환합니다.
- 보조 블록: 4구간 점수 분포(차트/표), 주제별 성과(문제 유형×주제별 정보와 제출 수 막대를 단일 행에서 1:1 대응하고 행별 제출 수 내림차순으로 표시하며 현재 결과의 최대 제출 수를 막대 기준으로 사용), PDF 사용 분석(한 카드 안에서 좌측 `전체 구성 비율`의 전체/직접 귀속/혼합/미분류 요약·전체 구성 파이 차트와 우측 `문제 유형별 구성과 주제 상세`의 Ant Design 확장형 계층표를 병렬 배치하고 데스크톱에서는 두 내부 섹션의 상·하단을 같은 높이로 정렬하며 1024px 이하에서 세로 전환 후 자연 높이 사용). 데스크톱의 `전체 구성 비율`은 제목 아래 KPI·차트 영역을 같은 높이로 나누고 각 콘텐츠를 세로 중앙 정렬하며, 파이 하단의 별도 범례 카드는 두지 않습니다. PDF 계층형 구성표 컬럼은 `구성`, `대주제`, `세부 주제`, `내보내기 완료 수(전체 이벤트 기준 건수·비율)`이며 헤더·본문·hover에 행 배경색을 사용하지 않습니다. 51~54번 부모 행의 대주제는 자식 행의 고유 대주제 수만 단위 없는 숫자로 표시합니다. Ant Design 기본 확장 버튼으로 부모 행을 펼치면 주제 자식 행이 완료 수 내림차순으로 나타나며 `1위`, `2위` 같은 순위 라벨은 표시하지 않습니다. 혼합·미분류는 `주제 분석 불가`로 유지하고 특정 주제로 배분하지 않습니다. 취약 평가 차원 블록은 2026-07-15 오너 지시로 제거.
- 액션: 분석 조건 적용·초기화, 현재 결과 CSV 내보내기, 데이터 오류·coverage 계약 오류 재시도. 페이지 헤더에 지표 사전·분석 공유 버튼은 두지 않으며 데이터 변경과 감사 로그는 없습니다.
- 연결 상태: summary의 현재/직전 제출·이벤트 메타데이터 대상/연결 수를 비교해 100% 미만인 범위만 별도 Alert로 표시합니다. 연결 필드 누락 또는 비정상 cardinality는 통계 계약 오류로 표시합니다.
...
## 40) 시스템 > 메타데이터 관리 > TOPIK 쓰기 마스터 카탈로그 (P5-1 조회 + P5-3 태그 토글)

- 현재 상태: 구현됨 (`master-catalog-section.tsx` — 운영 설정 카탈로그 카드 아래 별도 `AdminListCard`)
- 데이터 SoT: Supabase `topik_writing_topic_master` / `topik_writing_tag_master` **전수 조회**(비활성 포함, facade `fetchQuestionBankTopicMasterCatalogSafe` / `fetchQuestionBankTagMasterCatalogSafe`). 모크 그룹 store(편집 가능 인메모리)와 SoT가 다르므로 별도 섹션으로 분리합니다.
- 탭 구성: `주제 마스터`(기본) / `태그 마스터`
- 주제 마스터 컬럼: 정렬, 종합 주제(필터+정렬), 세부 내용, 상태(활성/비활성 필터), 출처, 메모 — 전면 조회 전용
- 태그 마스터 컬럼: 태그 코드, 태그명, 그룹(필터+정렬), 상태(활성/비활성 스위치 + 필터), 설명, 사용 규칙, 예시 문항, 최근 수정

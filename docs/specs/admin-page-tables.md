...
- 구현 컬럼: 코드, 이름, 종류, 상태, 노출 모드, 가입 수, 생성일, 액션
  - `노출 모드` 셀: `제한 없음` 또는 `배정분만` Tag + 배정 건수 종속 표기(`제한 없음` 이면 `배정 N건 보존` — 모드 복귀 시 살아난다는 의미). 기관 축 설정이며 문항 축 라벨(`미배정`/`기관 N곳 배정`)과 다른 축이다. 컬럼 헤더에 값별 설명을 병기한다. 전환은 `수정` 모달의 라디오 2안(사유 필수)이고, 배정 0건 + 회원 1명 이상으로 `배정분만` 전환은 차단한다. 생성 모달에는 읽기 전용 안내만 둔다.
...
- 주요 액션: `상세 보기`, `노출 가능`, `노출 제외`, `내부 테스트`, `태그 편집`, `기관 노출 설정`, 일괄 `기관 배정`/`배정 해제`. 기관 신규 추가는 `service_status='available'` 문항만 허용하고, 기존 기관 매핑 제거는 문항 상태와 무관하게 허용합니다.
...
- 2026-06-26 갱신: `/assessment/question-bank`에 기관 컬럼과 기관 노출 설정/기관 배정/배정 해제 진입점을 다시 제공했습니다. 같은 `topik_writing_question_institution_exposure` 매핑을 `Users > 기관 코드`에서도 기관 중심으로 관리합니다.
- 현재 상태: 구현됨 (`topik_writing` canonical 4테이블 + 추천 뷰가 유일한 운영 조회 source. `VITE_QUESTION_BANK_SOURCE`와 `public.problems` read adapter는 제거됐고 JSON fixture/store fallback 없음)

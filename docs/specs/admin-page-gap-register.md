...
- 확산 3차(2026-08-20): 이월 계열 ④⑧ 해소, **2파일 전환 + 서비스 계약 1건 정규화**.
  ...
  - 잔여(4차 이후): ⑤institution-codes·institution-code-detail(fail-open 부가 조회 8개 — 부가 조회를 각각 `keepDataOnError: false` 훅으로 나누고 `reload` 를 묶는 설계까지 확인했으나 범위상 분리) · ~~⑨billing-payments~~(**4차에서 해소**) · ~~⑩users-page~~(**5차에서 해소**) + C/D 복합(멀티 effect·폴링).

...
- `Resolved`(2026-07-15): PDF 사용 분석이 51~54번 합계만 제공해 어떤 주제의 내보내기 완료가 많은지 비교할 수 없던 gap을 `pdf_usage.perTopic`과 문제 유형×대주제×세부 주제별 순위 테이블로 해소했다. 직접 귀속 이벤트만 집계하고 혼합·미분류는 특정 주제로 배분하지 않는다.

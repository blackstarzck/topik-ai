...
#### 4.9.1 통계 권한 경계의 잔여 우회 경로 (2026-08-05 실측)

- 배경: 관리자 콘솔은 관리 주체가 불명확하던 시기에 "마스터 관리자" 단일 개념으로 개발됐다. 2026-08-05 학습 분석·분석 개요 RPC 4종을 `analytics.read` 서버 강제로 전환했으나(마이그 `20260805130000`), 같은 데이터에 도달하는 다른 경로는 여전히 관리자 공통 게이트다.
- 🚨 `누락`(우선순위 높음, v13 조율 필요): 학습 원본 테이블 직접 조회가 `analytics.read` 없이 가능하다. dev 실측(2026-08-05, 트랜잭션 롤백): `analytics.read` grant 없는 활성 `content_admin`이 `authenticated` 롤로 `public.writing_submissions` 282행, `public.writing_feedback` 249행을 직접 SELECT 했다(두 테이블 모두 RLS enable+force인데 admin select 정책이 `private.is_admin` 기준). RPC 를 잠가도 원본에서 통계를 재구성할 수 있다. 이 RLS 정책은 v13 소유 도메인이라 이 저장소 단독으로 바꿀 수 없고, "답안·회원 개인정보 노출 범위 불변경" 원칙과도 충돌하므로 오너 결정(2026-08-05)에 따라 후속으로 분리했다. 착수 시 소유권 조율과 `content_admin` 실사용 영향 검토가 선행 조건이다(현재 관리자 계정은 dev·운영 각 `platform_admin` 1개라 즉시 위험은 없다).
- `누락`: `public.get_admin_dashboard_stats()`가 `private.is_admin` 기준이다. 메뉴는 이미 `dashboard.read`로 게이팅되므로 화면과 DB 판정이 어긋난다. 별개 권한 도메인이라 이번 범위에서 제외했다.
- `누락`: `public.get_admin_backup_summary()`가 `private.is_admin` 기준인데 같은 화면의 `get_admin_backup_runs`는 `system.backups.read`를 요구한다(혼재 표면). 요약만 권한 없이 읽힌다.
- `누락`: 마이그레이션 러너가 비-LIFO down 을 막지 않는다(`scripts/db/migrate-core.mjs`). 문자열 수술 마이그레이션은 앞선 정의를 전제하므로, 과거 마이그레이션의 down 을 먼저 실행하면 fail-closed 되거나 추적기와 실제 정의가 어긋난다. `20260805130000` 은 헤더와 PR 본문에 순서 주의를 명시하는 방식으로만 대응했다.

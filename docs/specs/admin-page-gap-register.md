...
- 확산 2차 후 잔여(3차 대상): ④message-channel(응답 3상태 분배 — templates/groups 의 조치 후 변이 여부 정독 필요) ⑤institution-codes·**institution-code-detail(5-fetch 확인)** — fail-open 부가 조회 결합은 부가 상태를 훅 밖 잔여 effect 로 나눌지 설계 필요 ⑧system-admins(서비스가 `{ok, data, error: string}` 비표준 반환 — SafeResult 정규화 별도 작업) ⑨billing-payments(refunds 분기가 에러를 삼킴 — 표준화하면 동작 개선이라 오너 승인 필요) ⑩users-page(fetch 가 쓰지 않는 `query.page/pageSize` 재조회 deps 잔존 — instructor·referrals 와 동일 패턴이나 오너 확인(2026-08-19)이 두 페이지 특정이라 확장 적용은 확인 필요) + C/D 복합 페이지(멀티 effect·폴링). 나머지는 Phase 4 분해와 함께 자연 전환.
...
- 남은 것(별도 판단 필요)
  - **차트 축·눈금 라벨이 `fontSize={9}`** 4곳(`source-data-chart.tsx`). 가시 텍스트 14px 규칙의 예외로 볼지, 차트 레이아웃을 다시 잡아 올릴지 판단이 필요하다. 지금은 근거와 함께 게이트 예외로 뒀다.
  - **`colorInfo` 는 antd 기본 파랑 그대로**다(정보 Alert 아이콘 — 프리뷰 실측 잔존 18개가 전부 이것). 브랜드색과 같게 하면 정보 Alert 이 주요 액션과 구분되지 않으므로 시맨틱 상태색으로 남겼다.
  - **`/analytics/learning` 만 페이지 단위 `fontSize: 16`** 을 쓴다(2026-07-14 조치의 잔재). 전역이 14 라서 이 화면만 본문이 크다 — 전역과 맞출지 결정이 필요하다.
  - **`global.css` 에 색 리터럴이 106줄** 남아 있다. CSS 는 TS 모듈을 import 할 수 없어 CSS 변수 브리지가 선행돼야 하는 별도 작업이다.

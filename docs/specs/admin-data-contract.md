...
- `Users > 회원 목록`
  ...
  - 전화번호 계약: 목록 `UserSummary.phoneMasked`는 `phone_masked`를 사용하고 원문 전화번호를 표시하지 않는다. 상세 `UserSummary.phone`은 `get_admin_user(target_id uuid)`의 단건 조회에서만 `profiles.phone` 원문을 받아 프로필 탭에 표시하며, 값이 없으면 `phoneMasked`로 fallback한다.

...
- 포인트 수동 적립/차감/회수
- 포인트 소멸 보류/해제/실행
- TOPIK 쓰기 문항의 노출 상태(`service_status`) 변경, 태그 부여/제거, 기관 노출 설정/해제, 수신·적재·승격(`question_received`)
...
- 회원 정지/해제 로그는 `Target Type = User`(`admin_audit_logs.target_table='User'`), `Target ID = userId`를 사용하며, `/users` 또는 `/users/{userId}` 기준 원본 화면과 `/system/audit-logs?targetType=Users&targetId={userId}` 후속 검증 경로로 역추적할 수 있어야 합니다. 읽기 RPC와 UI는 과거 `User` URL도 수용하되 화면 projection은 `Users`로 통일합니다.
...
- 회원 정보 내보내기 로그는 `Target Type = User`(`admin_audit_logs.target_table='User'`), `Target ID = batch:{uuid}`를 사용합니다. 원본 화면은 `/users`, 후속 검증 경로는 `/system/audit-logs?targetType=Users&targetId=batch:{uuid}`입니다.
...
- 기관 코드 조치 로그는 `Target Type = InstitutionCode`(`admin_audit_logs.target_table='InstitutionCode'`), `Target ID = code`를 사용하며, `/users/institution-codes?selected={code}` 원본 화면과 `/system/audit-logs?targetType=InstitutionCode&targetId={code}` 후속 검증 경로로 역추적할 수 있어야 합니다.
  ...
  - 액션 `institution_exposure_mode_changed`(기관 노출 모드 변경) — 기록 주체 `admin_set_institution_exposure_mode(p_code,p_mode,p_reason)`. Target 은 동일하게 `InstitutionCode + code`. 사유 필수이며 값이 그대로면 감사 행을 남기지 않는다. `diff.exposure_mode.{from,to}` + `payload.{reason, code, assignment_count, member_count, pending_invitation_count}` — 폭발 반경(그 기관 학습자 전원의 가시 문항 집합)을 사후 검증할 수 있도록 배정·회원·대기초대 수를 함께 남긴다. `institution_code_updated` 에 얹지 않은 이유는 그 액션의 diff 계약이 `label/kind/status` 3필드로 고정돼 있고, 라벨 오타 수정과 노출 범위 변경이 같은 액션이면 감사 필터가 불가능해지기 때문이다.
  - 액션 `institution_contract_created` / `institution_contract_updated` / `institution_contract_deleted`(기관 계약 기간) — 기록 주체 `admin_create_institution_contract(p_code,p_starts_on,p_ends_on,p_reason,p_note,p_doc_url)`, `admin_update_institution_contract(p_contract_id,p_starts_on,p_ends_on,p_reason,p_note,p_doc_url)`, `admin_delete_institution_contract(p_contract_id,p_reason)`. Target 은 `InstitutionCode + code`(계약 id 는 payload 에). 사유 필수. 생성/삭제는 `diff.contract.{from,to}` 에 기간 전후를, 수정은 `diff.starts_on/ends_on.{from,to}` 를 남깁니다. 삭제 payload 에는 삭제 전 전량 값 + `had_active_contract` + `member_count`/`pending_invitation_count` 를 남깁니다 — 하드 삭제를 허용하는 대신 복구 근거를 감사에 보존하는 계약입니다(오너 결정). 계약 기간 겹침은 exclusion 제약이 거부하므로 감사에는 성공 건만 남습니다.
  - 액션 `institution_contract_auto_hide_changed`(만료 시 자동 비노출 옵션) — 기록 주체 `admin_set_institution_auto_hide_on_expiry(p_code,p_enabled,p_reason)`. `diff.auto_hide_on_expiry.{from,to}` + `payload.{reason, code, has_active_contract, writing_hidden_now, member_count, pending_invitation_count}`. `writing_hidden_now` 는 이 변경으로 **지금 즉시** 그 기관 학습자에게 쓰기 문항이 사라지는지를 뜻하며, 옵션 ON 과 계약 무효가 동시에 성립할 때만 true 입니다.
  - 액션 `institution_auto_assign_changed`(신규 문항 자동 배정 옵션) — 기록 주체 `admin_set_institution_auto_assign(p_code,p_enabled,p_reason)`. `diff.auto_assign_new_questions.{from,to}` + `payload.{reason, code, assignment_count, retroactive:false}`. `retroactive:false` 는 "옵션을 켠 시점에 이미 노출 중인 문항은 소급 배정하지 않는다"는 계약을 감사에 못박은 것입니다.
  - 액션 `institution_settings_changed`(기관 운영 설정) — 기록 주체 `admin_update_institution_settings(p_code,p_max_members,p_default_invite_expiry_days,p_block_intake_on_expiry,p_contact_name,p_contact_email,p_reason)`. `diff` 에는 `max_members`/`default_invite_expiry_days`/`block_intake_on_expiry` 전후 값만 남기고, **담당자 이름·이메일 값은 diff·payload 어디에도 기록하지 않습니다**(개인정보). 변경 사실은 `payload.changed_fields` 의 필드명으로만 남기며 `payload.contact_value_logged=false` 가 그 계약을 명시합니다. 실제 변경이 없으면 감사 행을 남기지 않습니다.
  - 기록 주체: `admin_create_institution_code(p_code,p_label,p_kind,p_note)`, `admin_update_institution_code(p_code,p_label,p_kind,p_status,p_note,p_reason)`, `admin_delete_institution_code(p_code,p_reason)` 단일 write 경로. 세 RPC 모두 admin 권한과 `users.institution-codes.manage` permission을 요구하며, 수정/삭제는 reason 필수입니다. 권한 검사는 `public.admin_has_permission(uuid,text)` 를 씁니다 — `private` 변형은 존재하지 않으며, 그것을 부르던 `admin_set_institution_exposure_mode` 는 2026-08-04 에 수리했습니다(그 전에는 모드 변경이 42883 으로 실패했습니다).
  - 삭제 정책: `profiles.affiliation_code = code` 회원이 1명 이상이면 삭제를 차단하고, 회원이 없을 때만 `institution_codes` 행을 제거합니다. pending 초대는 먼저 취소하며, 존재하는 `topik_writing_question_institution_exposure` 매핑, `topik_writing_institution_exposure_mode` 원장, `topik_writing_institution_contracts` 계약, `institution_code_settings` 운영 설정은 같은 트랜잭션에서 삭제합니다. 이 순서로 같은 code 재생성 시 이전 `제한 없음` 설정·계약 기간·정원·담당자가 되살아나는 것을 막습니다.
  - payload/diff 계약: 생성은 `payload.note`, 수정은 `diff.label/kind/status`와 `payload.reason`, 삭제는 `diff.deleted.from=false/to=true`와 `payload.reason/label/kind/status/note/member_count/deleted_exposure_count/deleted_exposure_mode_count/deleted_contract_count/deleted_settings_count/canceled_invitation_count`를 기록합니다. 성공 피드백은 `감사 로그 확인` 링크(`/system/audit-logs?targetType=InstitutionCode&targetId={code}`)를 노출합니다.
...
- PDF 내보내기 쿼터 정책 조치 로그는 `Target Type = PdfQuotaPolicy`(`admin_audit_logs.target_table='PdfQuotaPolicy'`), `Target ID = policyId(uuid)`를 사용하며, `/operation/pdf-quota` 기준으로 원본 화면을 역추적할 수 있어야 합니다.
  - 액션 사전: `pdf_quota_policy_saved`(설정 저장 — 2026-07-08 설정형 전환 이후에도 action명은 유지해 과거 이력 연속성을 보존).
  - 기록 주체: admin RPC 단일 write 경로(`admin_save_pdf_quota_policy(p_limit_count,p_period_unit,p_period_timezone,p_reason,p_expected_updated_at)`, 20260708150000에서 구 시그니처 drop). 사유 필수, 항상 현재 정책 1행을 갱신/복구(자기치유)하며 한도 0(의도적 중단)을 허용합니다.
  - payload/diff 계약: `diff`에 limit_count/period_unit/period_timezone의 `{from,to}`를 **항상 3필드 모두 기록**합니다(2026-07-08부터 — 변경분만 기록하던 구형 행은 이력 화면에서 결과값 fallback으로 렌더). `payload`에 `reason`/`period_unit_changed`/`export_paused`(한도 0)/`deactivated_ids`(자기치유로 비활성화된 행)를 포함합니다.
  - 이력 조회: `get_admin_pdf_quota_policy_history(p_page,p_page_size)` read RPC가 이 action의 감사 행에서 **비민감 화이트리스트 필드만**(감사 id/KST 시각/처리자/사유/한도·주기·시간대 from→to/결과값) `operation.pdf-quota.manage` 권한자에게 반환합니다. 2026-06-18 diff/payload platform_admin 게이팅의 **범위 예외**입니다 — 쿼터 수치는 PII·정책 본문이 아니고 reason은 이미 전체 admin에게 공개되는 필드라는 근거로, 이 action 1종에 한정합니다. raw diff/payload 전체는 여전히 platform_admin 전용입니다.
- PDF 내보내기 쿼터 초기화 조치 로그는 `Target Type = PdfQuotaReset`(`admin_audit_logs.target_table='PdfQuotaReset'`), `Target ID = resetId(uuid)`를 사용하며, `/operation/pdf-quota?tab=resets` 기준으로 원본 화면을 역추적할 수 있어야 합니다.
  - 액션 사전: `pdf_quota_reset_created`(개인/기관 코드/전체 초기화 생성). 초기화 이력은 수정/삭제하지 않고 보상 초기화로만 정정합니다.
  - 기록 주체: admin RPC 단일 write 경로(`admin_create_pdf_quota_reset(p_scope,p_user_id,p_group_code,p_problem_id,p_reason)`). 사유 필수, user/group/global scope 모두 `pdf_export_quota_reset_targets`에 concrete `user_id`를 실체화합니다. group은 `profiles.affiliation_code`, global은 non-deleted `profiles` 생성 시점 스냅샷이며 0명이면 거부합니다.
  - payload/diff 계약: `payload`에 `reason`/`scope`/`user_id`/`group_code`/`problem_id`/`target_count`/`period_semantics='period_local_snapshot'`을 기록합니다. `pdf_export_quota_resets.created_by`는 admin 계정이 v13 `profiles`에 없어 null일 수 있으므로 처리자 추적은 이 감사 로그가 SoT입니다.
...
- TOPIK 쓰기 문항 조치 로그는 `Target Type = AssessmentQuestion`, `Target ID = questionId`를 사용합니다.
  - 액션 사전은 D-8 개정(2026-06-11 인바운드 전환 — `docs/architecture/metadata-tag-schema-transition-decision-record.md` §0)과 기관 노출 정합화(2026-06-26)를 따릅니다: 유지 = `service_status_changed`/`tag_assigned`/`tag_removed`, 기관 매핑 = `question_institutions_changed`/`question_institutions_cleared`, 수신 = `question_received`(인박스 적재 `AssessmentQuestionImport` + 정식 문항 승격 `AssessmentQuestion`), 폐기 = 검수 4종(`review_completed`/`review_on_hold`/`review_revision_requested`/`review_memo_saved`)·배포 `question_published`.
- `노출 가능`/`노출 제외`/`내부 테스트`(`service_status_changed`) 전환 조치와 태그 부여/제거(`tag_assigned`/`tag_removed`)는 **P4 관리 포인트 개방(2026-06-11)으로 활성**이며, 시스템 감사 로그에서 통합 문항 화면 `/assessment/question-bank` 기준으로 원본 화면을 역추적할 수 있어야 합니다(상세 라우트는 재정의 P3에서 `/assessment/question-bank/{questionId}`로 개명 완료 — `202f905`). 노출 상태 전환은 확인 + 사유 입력(필수)을 유지하고, 태그 부여/제거는 별도 사유 입력 없이 처리합니다.
  ...
  - **정식 버전 승격 계약(마이그레이션 `20260713080015`, 공유 dev DB 적용 2026-07-14·운영 미적용)**: 승격 성공 감사 payload에는 정식 `question_id`와 `canonical_import_id`를 포함해 어떤 인박스 버전이 정식 카탈로그가 됐는지 역추적합니다. 기존 `question_received + AssessmentQuestion + questionId` 액션/Target은 유지하며, 학습자 읽기·제출 guard·service-role 채점 payload 조회는 관리자 조치가 아니므로 별도 감사 액션을 신설하지 않습니다.
...
- 폐기: 운영정책 `POL-017` 구판의 `배포(API 업로드)` 후보 액션(`question_published`, `publishedTaskId` 근거 포함)은 상류 push 폐기(2026-06-11 §0)로 철회됐습니다. `POL-017`은 "TOPIK 쓰기 문항 수신·관리 운영정책"(외부 상세 API 수신 → 인박스 적재 → §7 승격 → 관리 포인트(태그) + 노출(`service_status`) → v13 read-only)으로 재정의됐고, `question_received`는 적재 시 `AssessmentQuestionImport + source_task_id`, 승격 시 `AssessmentQuestion + questionId` 계약으로 기록됩니다.
...
- 액션 사전(D-8 개정): 유지 = `service_status_changed`·`tag_assigned`·`tag_removed`, 수신 = `question_received`(`AssessmentQuestionImport` 적재 + `AssessmentQuestion` 승격), 폐기 = 검수 계열 `review_completed`·`review_on_hold`·`review_revision_requested`·`review_status_changed`·`review_memo_saved` + 배포 `question_published`.

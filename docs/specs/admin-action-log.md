...
- PDF 내보내기 쿼터 초기화 조치 로그는 `Target Type = PdfQuotaReset`(`admin_audit_logs.target_table='PdfQuotaReset'`), `Target ID = resetId(uuid)`를 사용하며, `/operation/pdf-quota?tab=resets` 기준으로 원본 화면을 역추적할 수 있어야 합니다.
  ...
  - 기록 주체: admin RPC 단일 write 경로(`admin_create_pdf_quota_reset(p_scope,p_user_id,p_group_code,p_problem_id,p_reason)`). 사유 필수, group scope는 `profiles.affiliation_code` 생성 시점 스냅샷으로 대상을 실체화하며 0명이면 거부합니다.

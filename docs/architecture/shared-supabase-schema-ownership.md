...
| `private.system_reports`, `public.submit_system_report` | **topik-ai** (`admin_schema_migrations`, `supabase/migrations-admin/20260723170000_system_reports.sql`) | v13 server API의 service role 전용 접수 RPC | `system.reports.read` 보유 admin(읽기 RPC 경유) | RLS enable+force, 정책 0건, `PUBLIC`·`anon`·`authenticated`·`service_role` 직접 테이블 권한 전무. **소유권 인수(2026-07-29): 정본 SQL은 v13이 작성했고 이 저장소가 바이트 그대로 채택해 적용·보관을 소유한다.** 정본 파일은 handoff의 checksum 계약 때문에 수정하지 않으며, 컬럼·트리거·인덱스 추가와 자동 retention 을 금지한다. 접수 항목 집합 변경이 필요하면 v13 협의로 새 forward migration 을 만든다 |
| `admin_list_system_reports`, `admin_delete_system_report` RPC | **topik-ai** (`admin_schema_migrations`, `supabase/migrations-admin/20260729120000_admin_system_reports_console.sql`) | 삭제는 platform_admin RPC(사유 필수) | `system.reports.read` 보유 admin | 정본 테이블을 변형하지 않는 읽기/단건 삭제 전용. `postgres` owner + SECURITY DEFINER 로 force RLS 를 우회하고 `PUBLIC`·`anon` execute 는 회수. 삭제는 `SystemReport`/접수번호로 감사하며 감사 payload 에 제출자 이메일·제목·본문·사용자 식별자를 담지 않는다 |

...
- **글자 크기는 `FONT_SIZE`(TS)와 `var(--admin-font-*)`(CSS)에서만 나온다.** px 리터럴은 값이 규칙을 지켜도 위반이다 — 스케일이 두 벌이 되기 때문(2026-08-21 결정). base 는 `src/app/theme.ts` 의 `BASE_FONT_SIZE_PX`(14)이고 나머지 단계는 antd 가 그 base 에서 파생한 값을 그대로 읽는다. 스케일에 없는 크기가 정말 필요하면 `FONT_SIZE` 에 이름과 근거를 붙여 추가한다(선례: `metric` 28). 게이트 = `check:typography-min-font` + `tests/e2e/typography-scale-audit.spec.ts`.
- **링크와 그 hover 는 모두 WCAG AA(4.5:1)를 넘어야 한다.** hover 가 기준을 못 넘으면 평소엔 읽히는 링크가 마우스를 올리면 덜 읽힌다. antd 기본 파랑 계열(`#1677ff`·`#4096ff`·`#69b1ff`)은 흰 배경에서 전부 미달이므로 링크 색으로 쓰지 않는다 — 기준은 `tests/unit/design-tokens.test.ts` 가 고정한다.

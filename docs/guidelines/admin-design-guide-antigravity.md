...
- **설명 툴팁**: 컬럼 또는 필드 설명이 필요할 때는 heading/label 우측에 정보 아이콘(`i`)을 두고 hover/focus `Tooltip`으로 설명을 제공한다. 설명 문구를 항목 아래 helper text로 상시 펼쳐두는 방식은 기본값으로 사용하지 않는다.

## 테이블/필터 UI 규칙
- **필터**: 테이블 상단에 배치하며, 검색어 입력과 셀렉트 박스, 기간 선택(RangePicker)을 조합한다.
- **컬럼**: `docs/specs/admin-page-tables.md`에 정의된 순서와 명칭을 그대로 유지한다.
- **컬럼 헤더 제어**: 정렬 가능한 컬럼은 헤더에 오름차/내림차 affordance를 노출한다. 코드값·상태값처럼 고정 집합 컬럼은 헤더 필터를 함께 둘 수 있다.
- **정렬/필터 동기화**: 헤더 정렬/필터가 추가되면 URL 또는 store 복원 기준과 함께 설계한다. 새로고침 후에도 같은 목록 상태가 재현되어야 한다.
- **액션 컬럼**: 다중 조치가 있는 테이블은 공용 `더보기` 액션 메뉴를 기본으로 하고, 페이지별 `액션 + 아이콘` 버튼을 임의로 새로 만들지 않는다.

...
| 코드 근거 | `src/features/operation/pages/operation-faq-page.tsx` |
...
- 질문, 답변, 검색 키워드, 카테고리, 공개 상태, 노출 위치, 조회 지표를 관리자 기준으로 추적합니다.
...
| --- | --- | --- | --- | --- | --- |
| FAQ 조회 | FAQ의 목록/상세 또는 예정 데이터 블록을 확인합니다. | 조회 | OperationFaq | 현재 상태 확인 | 불필요 |
| FAQ 관리 | 질문, 답변, 검색 키워드, 카테고리, 공개 상태, 노출 위치, 조회 지표에 대한 등록/수정/상태 변경 또는 예정 계약을 관리합니다. | 수정 | OperationFaq + faqId | 데이터 반영 또는 후속 검증 | 필요 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| OperationFaq | operation_faqs, operation_faq_curations, operation_faq_metrics | Create, Read, Update, Delete 후보 | FAQ 본문/상세/Modal | 질문, 답변, 검색 키워드, 카테고리, 공개 상태, 노출 위치, 조회 지표, id, status, created_at, updated_at | OperationFaq + faqId | 운영상 추정 | 현재 프론트엔드/문서 기준 후보 |

### CRUD 상세

| CRUD | 지원 여부 | 화면 동작 | 저장/서비스 후보 | 성공 후 동기화 대상 | 실패 시 fail-safe |
| --- | --- | --- | --- | --- | --- |
| Create | `지원 또는 후보` | FAQ 등록/생성 후보 | service/store/API 후보 | 목록, 상세, 사용자 화면 후보 | error 표시, 재시도, 마지막 성공 상태 fallback |
| Read | `지원` | FAQ 조회 | service/store/API 후보 | URL/필터/탭 복원 | empty/error 처리 |
| Update | `지원 또는 후보` | FAQ 수정/상태 변경 후보 | service/store/API 후보 | 목록, 상세, 감사 로그 | 실패 시 재조회 또는 rollback |
| Delete | `지원 또는 후보` | FAQ 삭제/숨김/중지 후보 | service/store/API 후보 | 목록, 상세, 감사 로그, 사용자 노출 | 확인 모달, 사유 필수, 실패 안내 |

## 6. 관리자 조치와 감사 로그 계약

| 조치 | 파괴적 여부 | 확인 단계 | 사유/근거 입력 | Target Type | Target ID | 감사 로그 확인 경로 |
| --- | --- | --- | --- | --- | --- | --- |
| FAQ 주요 조치 | 예 | 필수 | 필수 | OperationFaq | 대상 ID | /system/audit-logs?targetType=OperationFaq&targetId={targetId} |

## 7. 사용자 화면 동기화 포인트

| 사용자 화면 후보 | 영향 상태 | 관리자 데이터 | 사용자 화면에 반영되는 방식 | 동기화 필요 시점 | 비고 |
| --- | --- | --- | --- | --- | --- |
| 고객센터 FAQ, 도움말, 홈 추천 FAQ, 결제 도움말 | 운영상 추정 | 질문, 답변, 검색 키워드, 카테고리, 공개 상태, 노출 위치, 조회 지표 | 고객센터 FAQ, 도움말, 홈 추천 FAQ, 결제 도움말에 운영상 추정으로 연결됩니다. | 관리자 변경 후 또는 원본 데이터 갱신 후 | 실제 사용자 화면 저장소 확인 전까지 추정은 추정으로 유지 |
...
| --- | --- | --- | --- | --- |
| 공개/비공개 | 공개/비공개 | page-specific enum candidate | 공개/비공개 | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
| 노출 상태 | 노출 상태 | page-specific enum candidate | 노출 상태 | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
| 설정 방식 | 설정 방식 | page-specific enum candidate | 설정 방식 | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
...
| --- | --- | --- | --- | --- | --- |
| FAQ 최종 계약 | FAQ 노출 위치와 사용자 화면 실제 surface는 B2C 구현 확인 후 승격해야 합니다. | 기획/백엔드/프론트 | 필터/액션/감사 로그 계약 변동 가능 | 고객센터 FAQ, 도움말, 홈 추천 FAQ, 결제 도움말에 운영상 추정으로 연결됩니다. | docs/specs/page-ia/operation-faq-page-ia.md |

...
- 이 문서는 `시스템 로그` 관리자 페이지와 사용자 화면 개발 사이의 동기화 기준을 정리합니다.
...
| 현재 상태 | `구현됨` |
| 페이지 유형 | `목록 운영형` |
| 페이지 목적 한 줄 요약 | 기술 로그를 component, level, timestamp 기준으로 조회해 장애 원인을 좁히는 화면입니다. |
| 주요 운영자 | `SUPER_ADMIN` |
| 주요 권한 | `system.logs.read` |
| 코드 근거 | `src/features/system/pages/system-logs-page.tsx` |
| 연관 SoT 문서 | `docs/specs/page-ia/system-logs-page-ia.md`, `docs/specs/admin-data-contract.md`, `docs/specs/admin-data-usage-map.md`, `docs/specs/admin-page-tables.md` |

## 3. 이 페이지의 목적

### 목적

- 시스템 장애와 기술 이벤트를 조회하고 관련 운영 화면으로 영향 범위를 좁힙니다.
- component, level, message, timestamp, traceId를 관리자 기준으로 추적합니다.
- B2C 직접 노출 없음. 장애 분석용 내부 데이터입니다.
...
| --- | --- | --- | --- | --- | --- |
| 시스템 로그 조회 | 시스템 로그의 목록/상세 또는 예정 데이터 블록을 확인합니다. | 조회 | SystemLog | 현재 상태 확인 | 불필요 |
| 시스템 로그 관리 | component, level, message, timestamp, traceId에 대한 등록/수정/상태 변경 또는 예정 계약을 관리합니다. | 조회 | SystemLog + logId | 조회 결과 확인 | 불필요 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| SystemLog | system_logs | Read | 시스템 로그 본문/상세/Modal | component, level, message, timestamp, traceId, id, status, created_at, updated_at | SystemLog + logId | 내부 전용 | 현재 프론트엔드/문서 기준 후보 |
...
| Read | `지원` | 시스템 로그 조회 | service/store/API 후보 | URL/필터/탭 복원 | empty/error 처리 |
...
| --- | --- | --- | --- | --- | --- | --- |
| 시스템 로그 조회 | 아니오 | 불필요 | 불필요 | SystemLog | 조회 조건 | 조회 전용 화면으로 별도 감사 로그 없음 |

## 7. 사용자 화면 동기화 포인트

| 사용자 화면 후보 | 영향 상태 | 관리자 데이터 | 사용자 화면에 반영되는 방식 | 동기화 필요 시점 | 비고 |
| --- | --- | --- | --- | --- | --- |
| 직접 연관 사용자 화면 없음 | 내부 전용 | component, level, message, timestamp, traceId | B2C 직접 노출 없음. 장애 분석용 내부 데이터입니다. | 관리자 변경 후 또는 원본 데이터 갱신 후 | 실제 사용자 화면 저장소 확인 전까지 추정은 추정으로 유지 |
...
| --- | --- | --- | --- | --- | --- |
| Dashboard | 참고/후속 | 시스템 로그 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 선행 또는 후행 | 운영상 추정 |
| System > 감사 로그 | 필수 후행 | 시스템 로그 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 후행 | 확정 |

### 사용자 화면

| 연관 사용자 화면 후보 | 관계 유형 | 연관 이유 | 관리자 변경 후 예상 영향 | 확정 상태 |
| --- | --- | --- | --- | --- |
| 직접 연관 사용자 화면 없음 | 내부 운영 | component, level, message, timestamp, traceId | 사용자 화면 영향 없음 | 내부 전용 |

## 9. 상태값/용어/키워드 정합성

| 구분 | 표준 값/용어 | 내부 코드 후보 | 사용자 노출 라벨 | 비고 |
| --- | --- | --- | --- | --- |
| info/warn/error | info/warn/error | page-specific enum candidate | info/warn/error | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
| component | component | page-specific enum candidate | component | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
...
- 사용자 화면 동기화에 필요한 식별자: SystemLog + logId
...
- Codex 확인 포인트:
  - `src/features/system/pages/system-logs-page.tsx` 구현과 `docs/specs/page-ia/system-logs-page-ia.md` 문서 일치 확인

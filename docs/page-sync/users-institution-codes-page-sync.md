...
| --- | --- | --- | --- | --- | --- | --- | --- |
| InstitutionCode | institution_codes | Create, Read, Update | 본문 목록/생성/수정 모달 | code, label, kind, status, note, member_count | InstitutionCode + code | 가입/QR 유입 코드 유효성 | 실제 테이블 home은 Supabase migration 기준 확인 |
...
| InstitutionQuestionExposure | topik_writing_question_institution_exposure | Create, Read, Delete | 노출 문항 모달 | institution_code, question_id, is_exposed | InstitutionCode + code | 기관 전용 문항 노출 | v13 사용자 화면 필터 조건 확인 필요 |
...
| 기관 전용 TOPIK 쓰기 문항 | 운영상 추정 | InstitutionQuestionExposure | 기관 코드에 연결된 문항만 노출하는 필터 후보입니다. | 문항 목록 조회 시 | v13 구현 확인 필요 |
...
- v13 사용자 화면의 실제 기관별 문항 필터 조건은 확인 전까지 `운영상 추정`으로 유지합니다.

## 13. 미확정 항목

| 항목 | 미확정 내용 | 필요한 결정 주체 | 관리자 페이지 영향 | 사용자 화면 영향 | 추적 문서 |
| --- | --- | --- | --- | --- | --- |
| 기관별 문항 노출 조건 | v13이 기관 코드별 문항 매핑을 어느 조회에서 적용하는지 확인 필요 | v13/백엔드 | 노출 문항 모달 의미 검증 | 기관 전용 문항 목록 | `docs/specs/page-ia/users-institution-codes-page-ia.md` |

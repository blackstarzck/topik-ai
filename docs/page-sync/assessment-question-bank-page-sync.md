...
| --- | --- | --- | --- | --- | --- |
| TOPIK 쓰기 시험 화면, 문제 풀이 화면, 결과/해설 화면 | 노출 예정 | 문항 본문+메타데이터(신규 4테이블), `service_status`, 태그 | v13 사용자 기능이 read-only로 소비합니다. 노출 on/off는 `service_status`(available/excluded/internal_test, 기본 internal_test)로만 통제합니다(`/manage`, P4 개방 예정). 상류 push(업로드/배포) 경로는 2026-06-11 §0으로 폐기됐습니다. | `service_status` 변경 시 | 인터림: v13 사용자 기능은 현행 `problems`를 읽는 중 — 신규 4테이블 소비 경로 전환은 컷오버 후속(별도 결정) |
...
| service_status ↔ v13 노출 연동 | `service_status`(D-6 유지)가 유일한 물리 노출 상태입니다. 구판의 상류 노출 토글 엔드포인트·보상 정책(구 P6) 미확정은 push 폐기로 소멸했습니다. 남은 미확정은 v13 사용자 기능의 신규 4테이블 read-only 소비 경로 전환(현재는 `problems`를 읽음)과 `/manage` write 개방(P4) 시점입니다. | 기획/백엔드/프론트 | `/manage` 운영 조치 활성화/감사 로그 계약 | 사용자 노출 on/off | docs/specs/page-ia/assessment-question-manage-page-ia.md, docs/architecture/metadata-tag-schema-transition-decision-record.md |

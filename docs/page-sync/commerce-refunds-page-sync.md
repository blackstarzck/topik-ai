...
status: "구현됨"
primary_entity: "CommerceRefund"
primary_table_candidate: "commerce_refunds"
...
- 이 문서는 `환불 관리` 관리자 페이지와 사용자 화면 개발 사이의 동기화 기준을 정리합니다.
...
| 모듈 | `Commerce` |
| 페이지명 | `환불 관리` |
| 라우트 | `/commerce/refunds` |
| 현재 상태 | `구현됨` |
| 페이지 유형 | `목록 운영형` |
| 페이지 목적 한 줄 요약 | 환불 요청을 검수하고 승인/거절하여 결제 원본과 사용자 결제 상태를 동기화하는 화면입니다. |
| 주요 운영자 | `OPS_ADMIN, CS_MANAGER, SUPER_ADMIN` |
| 주요 권한 | `commerce.refunds.manage` |
| 코드 근거 | `src/features/billing/pages/billing-refunds-page.tsx` |
| 연관 SoT 문서 | `docs/specs/page-ia/commerce-refunds-page-ia.md`, `docs/specs/admin-data-contract.md`, `docs/specs/admin-data-usage-map.md`, `docs/specs/admin-page-tables.md` |

## 3. 이 페이지의 목적

### 목적

- 환불 요청 상세를 확인하고 승인/거절 조치를 수행합니다.
- 환불 ID, 결제 ID, 신청자, 사유, 처리 상태, 환불 금액를 관리자 기준으로 추적합니다.
- 마이페이지 주문/결제 환불 상태에 운영상 추정으로 연결됩니다.
...
| --- | --- | --- | --- | --- | --- |
| Create | `지원 또는 후보` | 환불 관리 등록/생성 후보 | service/store/API 후보 | 목록, 상세, 사용자 화면 후보 | error 표시, 재시도, 마지막 성공 상태 fallback |
| Read | `지원` | 환불 관리 조회 | service/store/API 후보 | URL/필터/탭 복원 | empty/error 처리 |
| Update | `지원 또는 후보` | 환불 관리 수정/상태 변경 후보 | service/store/API 후보 | 목록, 상세, 감사 로그 | 실패 시 재조회 또는 rollback |
| Delete | `지원 또는 후보` | 환불 관리 삭제/숨김/중지 후보 | service/store/API 후보 | 목록, 상세, 감사 로그, 사용자 노출 | 확인 모달, 사유 필수, 실패 안내 |

## 6. 관리자 조치와 감사 로그 계약

| 조치 | 파괴적 여부 | 확인 단계 | 사유/근거 입력 | Target Type | Target ID | 감사 로그 확인 경로 |
| --- | --- | --- | --- | --- | --- | --- |
| 환불 관리 주요 조치 | 예 | 필수 | 필수 | CommerceRefund | 대상 ID | /system/audit-logs?targetType=CommerceRefund&targetId={targetId} |

## 7. 사용자 화면 동기화 포인트

| 사용자 화면 후보 | 영향 상태 | 관리자 데이터 | 사용자 화면에 반영되는 방식 | 동기화 필요 시점 | 비고 |
| --- | --- | --- | --- | --- | --- |
| 마이페이지 결제 내역, 환불 상태 안내 | 운영상 추정 | 환불 ID, 결제 ID, 신청자, 사유, 처리 상태, 환불 금액 | 마이페이지 주문/결제 환불 상태에 운영상 추정으로 연결됩니다. | 관리자 변경 후 또는 원본 데이터 갱신 후 | 실제 사용자 화면 저장소 확인 전까지 추정은 추정으로 유지 |
...
| --- | --- | --- | --- | --- | --- |
| Commerce > 결제 내역 | 참고/후속 | 환불 관리 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 선행 또는 후행 | 운영상 추정 |
| Users > 회원 상세 | 참고/후속 | 환불 관리 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 선행 또는 후행 | 운영상 추정 |
| System > 감사 로그 | 필수 후행 | 환불 관리 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 후행 | 확정 |

### 사용자 화면

| 연관 사용자 화면 후보 | 관계 유형 | 연관 이유 | 관리자 변경 후 예상 영향 | 확정 상태 |
| --- | --- | --- | --- | --- |
| 마이페이지 결제 내역 | 데이터 노출 후보 | 환불 ID, 결제 ID, 신청자, 사유, 처리 상태, 환불 금액 | 환불 관리 데이터 변경 시 표시/접근/알림이 달라질 수 있습니다. | 운영상 추정 |
| 환불 상태 안내 | 데이터 노출 후보 | 환불 ID, 결제 ID, 신청자, 사유, 처리 상태, 환불 금액 | 환불 관리 데이터 변경 시 표시/접근/알림이 달라질 수 있습니다. | 운영상 추정 |

## 9. 상태값/용어/키워드 정합성

| 구분 | 표준 값/용어 | 내부 코드 후보 | 사용자 노출 라벨 | 비고 |
| --- | --- | --- | --- | --- |
| 대기/승인/거절/완료 | 대기/승인/거절/완료 | page-specific enum candidate | 대기/승인/거절/완료 | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
| 환불 사유 | 환불 사유 | page-specific enum candidate | 환불 사유 | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |

## 10. URL/검색/복원 규칙

- 기본 라우트: `/commerce/refunds`
- 필수 쿼리/경로 파라미터: 없음
...
- Codex 확인 포인트:
  - `src/features/billing/pages/billing-refunds-page.tsx` 구현과 `docs/specs/page-ia/commerce-refunds-page-ia.md` 문서 일치 확인

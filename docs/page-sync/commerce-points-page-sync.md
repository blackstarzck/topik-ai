# Commerce > 포인트 관리 페이지 동기화 문서

---
doc_type: admin_page_sync
module: "Commerce"
page_name: "포인트 관리"
route: "/commerce/points"
status: "구현됨"
primary_entity: "CommercePointPolicy"
primary_table_candidate: "commerce_point_policies, commerce_point_ledgers, commerce_point_expirations"
...
- 이 문서는 `포인트 관리` 관리자 페이지와 사용자 화면 개발 사이의 동기화 기준을 정리합니다.
...
| 페이지 목적 한 줄 요약 | 포인트 정책, 포인트 원장, 소멸 예정 데이터를 관리하고 수동 조정/보류를 처리하는 화면입니다. |
| 주요 운영자 | `OPS_ADMIN, CS_MANAGER, SUPER_ADMIN` |
| 주요 권한 | `commerce.points.manage` |
| 코드 근거 | `src/features/commerce/pages/commerce-points-page.tsx` |
| 연관 SoT 문서 | `docs/specs/page-ia/commerce-points-page-ia.md`, `docs/specs/admin-data-contract.md`, `docs/specs/admin-data-usage-map.md`, `docs/specs/admin-page-tables.md` |

## 3. 이 페이지의 목적

### 목적

- 포인트 정책/원장/소멸 예정 탭을 운영하고 수동 조정을 기록합니다.
- 포인트 정책, 원장, 소멸 예정일, 수동 조정 사유를 관리자 기준으로 추적합니다.
- 포인트 지갑, 결제 포인트 사용, 추천/미션/이벤트 리워드 내역에 노출 예정으로 연결됩니다.
...
| --- | --- | --- | --- | --- | --- |
| 포인트 지갑, 결제 포인트 사용, 리워드 내역, 소멸 예정 안내 | 노출 예정 | 포인트 정책, 원장, 소멸 예정일, 수동 조정 사유 | 포인트 지갑, 결제 포인트 사용, 추천/미션/이벤트 리워드 내역에 노출 예정으로 연결됩니다. | 관리자 변경 후 또는 원본 데이터 갱신 후 | 실제 사용자 화면 저장소 확인 전까지 추정은 추정으로 유지 |
...
| --- | --- | --- | --- | --- | --- |
| Users > 추천인 관리 | 참고/후속 | 포인트 관리 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 선행 또는 후행 | 운영상 추정 |
| Commerce > 결제 내역 | 참고/후속 | 포인트 관리 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 선행 또는 후행 | 운영상 추정 |
| System > 감사 로그 | 필수 후행 | 포인트 관리 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 후행 | 확정 |

### 사용자 화면

| 연관 사용자 화면 후보 | 관계 유형 | 연관 이유 | 관리자 변경 후 예상 영향 | 확정 상태 |
| --- | --- | --- | --- | --- |
| 포인트 지갑 | 데이터 노출 후보 | 포인트 정책, 원장, 소멸 예정일, 수동 조정 사유 | 포인트 관리 데이터 변경 시 표시/접근/알림이 달라질 수 있습니다. | 노출 예정 |
| 결제 포인트 사용 | 데이터 노출 후보 | 포인트 정책, 원장, 소멸 예정일, 수동 조정 사유 | 포인트 관리 데이터 변경 시 표시/접근/알림이 달라질 수 있습니다. | 노출 예정 |
| 리워드 내역 | 데이터 노출 후보 | 포인트 정책, 원장, 소멸 예정일, 수동 조정 사유 | 포인트 관리 데이터 변경 시 표시/접근/알림이 달라질 수 있습니다. | 노출 예정 |
| 소멸 예정 안내 | 데이터 노출 후보 | 포인트 정책, 원장, 소멸 예정일, 수동 조정 사유 | 포인트 관리 데이터 변경 시 표시/접근/알림이 달라질 수 있습니다. | 노출 예정 |

## 9. 상태값/용어/키워드 정합성

| 구분 | 표준 값/용어 | 내부 코드 후보 | 사용자 노출 라벨 | 비고 |
| --- | --- | --- | --- | --- |
| 정책 상태 | 정책 상태 | page-specific enum candidate | 정책 상태 | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
| 원장 유형 | 원장 유형 | page-specific enum candidate | 원장 유형 | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
| 소멸 상태 | 소멸 상태 | page-specific enum candidate | 소멸 상태 | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
...
- 사용자 화면 동기화에 필요한 식별자: CommercePointPolicy 또는 CommercePointLedger + id
...
- Codex 확인 포인트:
  - `src/features/commerce/pages/commerce-points-page.tsx` 구현과 `docs/specs/page-ia/commerce-points-page-ia.md` 문서 일치 확인

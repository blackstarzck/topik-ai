...
status: "구현됨"
primary_entity: "AnalyticsMetric"
primary_table_candidate: "analytics_metrics 또는 원본 도메인 집계 view"
...
- 이 문서는 `통계 개요` 관리자 페이지와 사용자 화면 개발 사이의 동기화 기준을 정리합니다.
...
- 운영 지표를 조회하고 원본 도메인 개선 포인트를 찾습니다.
- 활성 사용자, 발송 성공률, 매출, 환불, 콘텐츠/학습 지표를 관리자 기준으로 추적합니다.
- 일부 B2C 직접 노출 가능성은 낮고 현재는 내부 운영 지표입니다.
...
| --- | --- | --- | --- | --- | --- |
| 통계 개요 조회 | 통계 개요의 목록/상세 또는 예정 데이터 블록을 확인합니다. | 조회 | AnalyticsMetric | 현재 상태 확인 | 불필요 |
| 통계 개요 관리 | 활성 사용자, 발송 성공률, 매출, 환불, 콘텐츠/학습 지표에 대한 등록/수정/상태 변경 또는 예정 계약을 관리합니다. | 조회 | AnalyticsMetric + metricKey | 조회 결과 확인 | 불필요 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| AnalyticsMetric | analytics_metrics 또는 원본 도메인 집계 view | Read | 통계 개요 본문/상세/Modal | 활성 사용자, 발송 성공률, 매출, 환불, 콘텐츠/학습 지표, id, status, created_at, updated_at | AnalyticsMetric + metricKey | 운영상 추정 | 현재 프론트엔드/문서 기준 후보 |
...
| Read | `지원` | 통계 개요 조회 | service/store/API 후보 | URL/필터/탭 복원 | empty/error 처리 |
...
| --- | --- | --- | --- | --- | --- | --- |
| 통계 개요 조회 | 아니오 | 불필요 | 불필요 | AnalyticsMetric | 조회 조건 | 조회 전용 화면으로 별도 감사 로그 없음 |

## 7. 사용자 화면 동기화 포인트

| 사용자 화면 후보 | 영향 상태 | 관리자 데이터 | 사용자 화면에 반영되는 방식 | 동기화 필요 시점 | 비고 |
| --- | --- | --- | --- | --- | --- |
| 직접 연관 사용자 화면 없음 | 운영상 추정 | 활성 사용자, 발송 성공률, 매출, 환불, 콘텐츠/학습 지표 | 일부 B2C 직접 노출 가능성은 낮고 현재는 내부 운영 지표입니다. | 관리자 변경 후 또는 원본 데이터 갱신 후 | 실제 사용자 화면 저장소 확인 전까지 추정은 추정으로 유지 |
...
| --- | --- | --- | --- | --- | --- |
| Dashboard | 참고/후속 | 통계 개요 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 선행 또는 후행 | 운영상 추정 |
| Commerce > 결제 내역 | 참고/후속 | 통계 개요 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 선행 또는 후행 | 운영상 추정 |
| Message > 발송 이력 | 참고/후속 | 통계 개요 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 선행 또는 후행 | 운영상 추정 |

### 사용자 화면

| 연관 사용자 화면 후보 | 관계 유형 | 연관 이유 | 관리자 변경 후 예상 영향 | 확정 상태 |
| --- | --- | --- | --- | --- |
| 직접 연관 사용자 화면 없음 | 데이터 노출 후보 | 활성 사용자, 발송 성공률, 매출, 환불, 콘텐츠/학습 지표 | 통계 개요 데이터 변경 시 표시/접근/알림이 달라질 수 있습니다. | 운영상 추정 |

## 9. 상태값/용어/키워드 정합성

| 구분 | 표준 값/용어 | 내부 코드 후보 | 사용자 노출 라벨 | 비고 |
| --- | --- | --- | --- | --- |
| 기간 필터 | 기간 필터 | page-specific enum candidate | 기간 필터 | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
| 지표 집계 상태 | 지표 집계 상태 | page-specific enum candidate | 지표 집계 상태 | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |

## 10. URL/검색/복원 규칙

- 기본 라우트: `/analytics/overview`
- 필수 쿼리/경로 파라미터: 없음
...
- 사용자 화면 동기화에 필요한 식별자: AnalyticsMetric + metricKey
...
- Codex 확인 포인트:
  - `src/features/analytics/pages/analytics-overview-page.tsx` 구현과 `docs/specs/page-ia/analytics-overview-page-ia.md` 문서 일치 확인
...
| --- | --- | --- | --- | --- | --- |
| 통계 개요 최종 계약 | 집계 지표별 원본 테이블과 갱신 주기는 백엔드/API 계약 확정이 필요합니다. | 기획/백엔드/프론트 | 필터/액션/감사 로그 계약 변동 가능 | 일부 B2C 직접 노출 가능성은 낮고 현재는 내부 운영 지표입니다. | docs/specs/page-ia/analytics-overview-page-ia.md |

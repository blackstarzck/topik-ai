...
| 통계 개요 관리 | 활성 사용자·매출·발송 성공률 등 운영 지표를 조회·교차 분석한다. | 조회 | AnalyticsMetric + metricKey | 조회 결과 확인 | 불필요 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| AnalyticsMetric | analytics_metrics 또는 원본 도메인 집계 view | Read | 통계 개요 본문/상세/Modal | 활성 사용자, 발송 성공률, 매출, 환불, 콘텐츠/학습 지표, id, status, created_at, updated_at | AnalyticsMetric + metricKey | 내부 전용 | 현재 프론트엔드/문서 기준 후보 |
...
| --- | --- | --- | --- | --- | --- |
| 직접 연관 사용자 화면 없음 | 내부 전용 | 활성 사용자, 발송 성공률, 매출, 환불, 콘텐츠/학습 지표 | 일부 B2C 직접 노출 가능성은 낮고 현재는 내부 운영 지표입니다. | 관리자 변경 후 또는 원본 데이터 갱신 후 | 실제 사용자 화면 저장소 확인 전까지 추정은 추정으로 유지 |

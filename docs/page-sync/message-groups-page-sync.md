...
status: "구현됨"
primary_entity: "MessageGroup"
primary_table_candidate: "message_groups, message_group_rules"
...
- 이 문서는 `대상 그룹` 관리자 페이지와 사용자 화면 개발 사이의 동기화 기준을 정리합니다.
...
| 현재 상태 | `구현됨` |
| 페이지 유형 | `목록 운영형` |
| 페이지 목적 한 줄 요약 | 메일/푸시 발송에 사용할 사용자 세그먼트와 정적 대상 그룹을 관리하는 화면입니다. |
| 주요 운영자 | `OPS_ADMIN, SUPER_ADMIN` |
| 주요 권한 | `messages.groups.manage` |
| 코드 근거 | `src/features/message/pages/message-groups-page.tsx` |
| 연관 SoT 문서 | `docs/specs/page-ia/message-groups-page-ia.md`, `docs/specs/admin-data-contract.md`, `docs/specs/admin-data-usage-map.md`, `docs/specs/admin-page-tables.md` |

## 3. 이 페이지의 목적

### 목적

- 발송 대상 조건을 저장하고 예상 대상 수를 재계산합니다.
- 그룹명, 설명, 정의 방식, 조건 요약, 예상 대상 수, 상태를 관리자 기준으로 추적합니다.
- 직접 노출은 없지만 발송/혜택 타게팅에 내부 전용으로 사용됩니다.
...
| --- | --- | --- | --- | --- | --- |
| Create | `지원 또는 후보` | 대상 그룹 등록/생성 후보 | service/store/API 후보 | 목록, 상세, 사용자 화면 후보 | error 표시, 재시도, 마지막 성공 상태 fallback |
| Read | `지원` | 대상 그룹 조회 | service/store/API 후보 | URL/필터/탭 복원 | empty/error 처리 |
| Update | `지원 또는 후보` | 대상 그룹 수정/상태 변경 후보 | service/store/API 후보 | 목록, 상세, 감사 로그 | 실패 시 재조회 또는 rollback |
| Delete | `지원 또는 후보` | 대상 그룹 삭제/숨김/중지 후보 | service/store/API 후보 | 목록, 상세, 감사 로그, 사용자 노출 | 확인 모달, 사유 필수, 실패 안내 |
...
| --- | --- | --- | --- | --- | --- |
| Message > 메일 | 참고/후속 | 대상 그룹 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 선행 또는 후행 | 운영상 추정 |
| Message > 푸시 | 참고/후속 | 대상 그룹 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 선행 또는 후행 | 운영상 추정 |
| Message > 발송 이력 | 참고/후속 | 대상 그룹 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 선행 또는 후행 | 운영상 추정 |
| System > 감사 로그 | 필수 후행 | 대상 그룹 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 후행 | 확정 |

### 사용자 화면

| 연관 사용자 화면 후보 | 관계 유형 | 연관 이유 | 관리자 변경 후 예상 영향 | 확정 상태 |
| --- | --- | --- | --- | --- |
| 메일/푸시 수신 대상 | 데이터 노출 후보 | 그룹명, 설명, 정의 방식, 조건 요약, 예상 대상 수, 상태 | 대상 그룹 데이터 변경 시 표시/접근/알림이 달라질 수 있습니다. | 운영상 추정 |

## 9. 상태값/용어/키워드 정합성

| 구분 | 표준 값/용어 | 내부 코드 후보 | 사용자 노출 라벨 | 비고 |
| --- | --- | --- | --- | --- |
| 사용중/초안 | 사용중/초안 | page-specific enum candidate | 사용중/초안 | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
| simple/query-builder | simple/query-builder | page-specific enum candidate | simple/query-builder | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
...
- 사용자 화면 동기화에 필요한 식별자: MessageGroup + groupId
...
- Codex 확인 포인트:
  - `src/features/message/pages/message-groups-page.tsx` 구현과 `docs/specs/page-ia/message-groups-page-ia.md` 문서 일치 확인
...
| --- | --- | --- | --- | --- | --- |
| 대상 그룹 최종 계약 | Query Builder SQL/JSON 구조와 대상 수 계산 API 계약은 API 전환 시 확인해야 합니다. | 기획/백엔드/프론트 | 필터/액션/감사 로그 계약 변동 가능 | 직접 노출은 없지만 발송/혜택 타게팅에 내부 전용으로 사용됩니다. | docs/specs/page-ia/message-groups-page-ia.md |

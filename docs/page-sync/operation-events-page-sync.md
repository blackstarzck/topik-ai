...
| 코드 근거 | `src/features/operation/pages/operation-events-page.tsx, src/features/operation/pages/operation-event-create-page.tsx` |
...
| --- | --- | --- | --- | --- | --- |
| Create | `지원 또는 후보` | 이벤트 등록/생성 후보 | service/store/API 후보 | 목록, 상세, 사용자 화면 후보 | error 표시, 재시도, 마지막 성공 상태 fallback |
| Read | `지원` | 이벤트 조회 | service/store/API 후보 | URL/필터/탭 복원 | empty/error 처리 |
| Update | `지원 또는 후보` | 이벤트 수정/상태 변경 후보 | service/store/API 후보 | 목록, 상세, 감사 로그 | 실패 시 재조회 또는 rollback |
| Delete | `지원 또는 후보` | 이벤트 삭제/숨김/중지 후보 | service/store/API 후보 | 목록, 상세, 감사 로그, 사용자 노출 | 확인 모달, 사유 필수, 실패 안내 |
...
| --- | --- | --- | --- | --- | --- |
| 이벤트 목록, 이벤트 상세, 프로모션 랜딩 | 노출 예정 | 이벤트명, 요약, HTML 본문, 기간, 노출 상태, 배너, 공유/SEO 메타 | 이벤트 목록, 상세, 프로모션 랜딩에 노출 예정으로 연결됩니다. | 관리자 변경 후 또는 원본 데이터 갱신 후 | 실제 사용자 화면 저장소 확인 전까지 추정은 추정으로 유지 |
...
| --- | --- | --- | --- | --- | --- |
| Commerce > 쿠폰 관리 | 참고/후속 | 이벤트 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 선행 또는 후행 | 운영상 추정 |
| Commerce > 포인트 관리 | 참고/후속 | 이벤트 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 선행 또는 후행 | 운영상 추정 |
| System > 감사 로그 | 필수 후행 | 이벤트 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 후행 | 확정 |
...
| --- | --- | --- | --- | --- |
| 노출/예약/숨김 | 노출/예약/숨김 | page-specific enum candidate | 노출/예약/숨김 | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
| 진행 상태 | 진행 상태 | page-specific enum candidate | 진행 상태 | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
| indexingPolicy | indexingPolicy | page-specific enum candidate | indexingPolicy | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
...
| --- | --- | --- | --- | --- | --- |
| 이벤트 최종 계약 | 이벤트 보상 지급 방식과 사용자 랜딩 URL 정책은 추가 확정이 필요합니다. | 기획/백엔드/프론트 | 필터/액션/감사 로그 계약 변동 가능 | 이벤트 목록, 상세, 프로모션 랜딩에 노출 예정으로 연결됩니다. | docs/specs/page-ia/operation-events-page-ia.md |

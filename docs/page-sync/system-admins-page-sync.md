...
primary_table_candidate: "admin_accounts"
...
| 현재 상태 | `구현됨` |
| 페이지 유형 | `목록 운영형` |
| 페이지 목적 한 줄 요약 | 관리자 계정, role, 상태를 관리하는 시스템 운영 화면입니다. |
| 주요 운영자 | `SUPER_ADMIN` |
...
- 관리자 계정을 생성/수정/정지하고 권한 그룹과 상태를 관리합니다.
- 관리자 ID, 이름, 이메일, role, 상태, 최근 접속를 관리자 기준으로 추적합니다.
...
| --- | --- | --- | --- | --- | --- |
| 관리자 계정 조회 | 관리자 계정의 목록/상세 또는 예정 데이터 블록을 확인합니다. | 조회 | AdminAccount | 현재 상태 확인 | 불필요 |
| 관리자 계정 관리 | 관리자 ID, 이름, 이메일, role, 상태, 최근 접속에 대한 등록/수정/상태 변경 또는 예정 계약을 관리합니다. | 수정 | AdminAccount + adminId | 데이터 반영 또는 후속 검증 | 필요 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| AdminAccount | admin_accounts | Create, Read, Update, Delete 후보 | 관리자 계정 본문/상세/Modal | 관리자 ID, 이름, 이메일, role, 상태, 최근 접속, id, status, created_at, updated_at | AdminAccount + adminId | 내부 전용 | 현재 프론트엔드/문서 기준 후보 |

### CRUD 상세

| CRUD | 지원 여부 | 화면 동작 | 저장/서비스 후보 | 성공 후 동기화 대상 | 실패 시 fail-safe |
| --- | --- | --- | --- | --- | --- |
| Create | `지원 또는 후보` | 관리자 계정 등록/생성 후보 | service/store/API 후보 | 목록, 상세, 사용자 화면 후보 | error 표시, 재시도, 마지막 성공 상태 fallback |
| Read | `지원` | 관리자 계정 조회 | service/store/API 후보 | URL/필터/탭 복원 | empty/error 처리 |
| Update | `지원 또는 후보` | 관리자 계정 수정/상태 변경 후보 | service/store/API 후보 | 목록, 상세, 감사 로그 | 실패 시 재조회 또는 rollback |
| Delete | `지원 또는 후보` | 관리자 계정 삭제/숨김/중지 후보 | service/store/API 후보 | 목록, 상세, 감사 로그, 사용자 노출 | 확인 모달, 사유 필수, 실패 안내 |

## 6. 관리자 조치와 감사 로그 계약

| 조치 | 파괴적 여부 | 확인 단계 | 사유/근거 입력 | Target Type | Target ID | 감사 로그 확인 경로 |
| --- | --- | --- | --- | --- | --- | --- |
| 관리자 계정 주요 조치 | 예 | 필수 | 필수 | AdminAccount | 대상 ID | /system/audit-logs?targetType=AdminAccount&targetId={targetId} |

## 7. 사용자 화면 동기화 포인트

| 사용자 화면 후보 | 영향 상태 | 관리자 데이터 | 사용자 화면에 반영되는 방식 | 동기화 필요 시점 | 비고 |
| --- | --- | --- | --- | --- | --- |
| 직접 연관 사용자 화면 없음 | 내부 전용 | 관리자 ID, 이름, 이메일, role, 상태, 최근 접속 | B2C 직접 노출 없음. 관리자 콘솔 접근 제어에만 사용됩니다. | 관리자 변경 후 또는 원본 데이터 갱신 후 | 실제 사용자 화면 저장소 확인 전까지 추정은 추정으로 유지 |
...
| --- | --- | --- | --- | --- | --- |
| System > 권한 관리 | 참고/후속 | 관리자 계정 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 선행 또는 후행 | 운영상 추정 |
...
| --- | --- | --- | --- | --- |
| 활성/정지 | 활성/정지 | page-specific enum candidate | 활성/정지 | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
| role | role | page-specific enum candidate | role | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
...
- 사용자 화면 동기화에 필요한 식별자: AdminAccount + adminId
...
| --- | --- | --- | --- | --- | --- |
| 관리자 계정 최종 계약 | 실제 로그인 관리자 식별자와 계정 초대/회수 정책은 인증 연동 시 확정해야 합니다. | 기획/백엔드/프론트 | 필터/액션/감사 로그 계약 변동 가능 | B2C 직접 노출 없음. 관리자 콘솔 접근 제어에만 사용됩니다. | docs/specs/page-ia/system-admins-page-ia.md |

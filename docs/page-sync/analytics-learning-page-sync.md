...
| KPI 설명 툴팁 | 조회 | 카드 정보 아이콘 클릭·hover·focus로 분류·지표명·정의·세부 기준을 계층적으로 확인 | 불필요 |
...
| --- | --- | --- | --- | --- | --- |
| LearningAnalyticsAggregate | `get_admin_learning_analytics_filtered(...)` | Read | 8 KPI, 유형·점수·주제·PDF 집계(`pdf_usage.perTopic`: 문제 유형×대주제×세부 주제별 직접 귀속 건수) | 내부 전용 | 제출·이벤트의 `problem_id`를 현재 canonical identity 또는 전환 시 이관한 private historical identity snapshot에 연결한다. 개인 식별자·민감 본문은 반환하지 않고 혼합·미분류는 주제에 배분하지 않음 |
...
- 조회·클라이언트 CSV 생성만 제공하므로 파괴적 조치, Target Type/ID, 감사 로그가 없습니다.

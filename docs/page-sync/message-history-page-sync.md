...
- 이 문서는 `발송 이력` 관리자 페이지와 사용자 화면 개발 사이의 동기화 기준을 정리합니다.
...
| 현재 상태 | `구현됨` |
| 페이지 유형 | `목록 운영형` |
| 페이지 목적 한 줄 요약 | 메일/푸시 발송 실행 결과와 수신자별 성공/실패 상태를 확인하고 재시도하는 화면입니다. |
| 주요 운영자 | `OPS_ADMIN, CS_MANAGER, SUPER_ADMIN` |
| 주요 권한 | `messages.history.read` |
| 코드 근거 | `src/features/message/pages/message-history-page.tsx` |
| 연관 SoT 문서 | `docs/specs/page-ia/message-history-page-ia.md`, `docs/specs/admin-data-contract.md`, `docs/specs/admin-data-usage-map.md`, `docs/specs/admin-page-tables.md` |

## 3. 이 페이지의 목적

### 목적

- 발송 결과를 검수하고 실패 건을 재시도하거나 CSV로 내보냅니다.
- 템플릿명, 발송 그룹, 수신자 수, 성공/실패 수, 수신자별 상태를 관리자 기준으로 추적합니다.
- 사용자 알림/메일 수신 결과에 운영상 추정으로 연결됩니다.
...
| --- | --- | --- | --- | --- | --- |
| 발송 이력 조회 | 발송 이력의 목록/상세 또는 예정 데이터 블록을 확인합니다. | 조회 | MessageHistory | 현재 상태 확인 | 불필요 |
| 발송 이력 관리 | 템플릿명, 발송 그룹, 수신자 수, 성공/실패 수, 수신자별 상태에 대한 등록/수정/상태 변경 또는 예정 계약을 관리합니다. | 수정 | MessageHistory + historyId | 데이터 반영 또는 후속 검증 | 필요 |
...
| --- | --- | --- | --- | --- | --- |
| Create | `지원 또는 후보` | 발송 이력 등록/생성 후보 | service/store/API 후보 | 목록, 상세, 사용자 화면 후보 | error 표시, 재시도, 마지막 성공 상태 fallback |
| Read | `지원` | 발송 이력 조회 | service/store/API 후보 | URL/필터/탭 복원 | empty/error 처리 |
| Update | `지원 또는 후보` | 발송 이력 수정/상태 변경 후보 | service/store/API 후보 | 목록, 상세, 감사 로그 | 실패 시 재조회 또는 rollback |
| Delete | `지원 또는 후보` | 발송 이력 삭제/숨김/중지 후보 | service/store/API 후보 | 목록, 상세, 감사 로그, 사용자 노출 | 확인 모달, 사유 필수, 실패 안내 |
...
| --- | --- | --- | --- | --- | --- |
| 이메일 수신함, 앱/웹 푸시 알림 | 운영상 추정 | 템플릿명, 발송 그룹, 수신자 수, 성공/실패 수, 수신자별 상태 | 사용자 알림/메일 수신 결과에 운영상 추정으로 연결됩니다. | 관리자 변경 후 또는 원본 데이터 갱신 후 | 실제 사용자 화면 저장소 확인 전까지 추정은 추정으로 유지 |
...
| --- | --- | --- | --- | --- | --- |
| Message > 메일 | 참고/후속 | 발송 이력 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 선행 또는 후행 | 운영상 추정 |
| Message > 푸시 | 참고/후속 | 발송 이력 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 선행 또는 후행 | 운영상 추정 |
| Message > 대상 그룹 | 참고/후속 | 발송 이력 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 선행 또는 후행 | 운영상 추정 |
| System > 감사 로그 | 필수 후행 | 발송 이력 데이터의 원본 확인 또는 후속 검증 | 식별자 또는 필터 기반 이동 | 후행 | 확정 |

### 사용자 화면

| 연관 사용자 화면 후보 | 관계 유형 | 연관 이유 | 관리자 변경 후 예상 영향 | 확정 상태 |
| --- | --- | --- | --- | --- |
| 이메일 수신함 | 데이터 노출 후보 | 템플릿명, 발송 그룹, 수신자 수, 성공/실패 수, 수신자별 상태 | 발송 이력 데이터 변경 시 표시/접근/알림이 달라질 수 있습니다. | 운영상 추정 |
| 앱/웹 푸시 알림 | 데이터 노출 후보 | 템플릿명, 발송 그룹, 수신자 수, 성공/실패 수, 수신자별 상태 | 발송 이력 데이터 변경 시 표시/접근/알림이 달라질 수 있습니다. | 운영상 추정 |

## 9. 상태값/용어/키워드 정합성

| 구분 | 표준 값/용어 | 내부 코드 후보 | 사용자 노출 라벨 | 비고 |
| --- | --- | --- | --- | --- |
| 성공/실패/예약 | 성공/실패/예약 | page-specific enum candidate | 성공/실패/예약 | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
| 자동/수동 | 자동/수동 | page-specific enum candidate | 자동/수동 | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |
| 채널 | 채널 | page-specific enum candidate | 채널 | 정확한 상태 세트는 IA와 데이터 계약 문서를 우선합니다. |

## 10. URL/검색/복원 규칙

- 기본 라우트: `/messages/history`
- 필수 쿼리/경로 파라미터: 없음
...
- 사용자 화면 동기화에 필요한 식별자: MessageHistory + historyId
...
- Codex 확인 포인트:
  - `src/features/message/pages/message-history-page.tsx` 구현과 `docs/specs/page-ia/message-history-page-ia.md` 문서 일치 확인

...
- 프로필, 성별, 전화번호, 회원 상태, 이메일 인증, 약관 동의, 활동, 결제, 커뮤니티, 로그, 관리자 메모를 관리자 기준으로 추적합니다.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| User | v13 `profiles`/`auth.users` + 상세 RPC/mock 폴백 | Read, 상태 Update | 회원 상세 본문/상세/Modal | 프로필, 성별, 전화번호, 회원 상태(파생), `profiles.status` 원천 운영 상태, 이메일 인증, 약관 동의, 활동, 결제, 커뮤니티, 로그, 관리자 메모, id, status, created_at, updated_at | User + userId | 운영상 추정 | `회원 상태`는 `profiles.status` + 이메일 인증 + 약관 동의 파생값, `이메일 인증`은 `auth.users.email_confirmed_at` 기준. 상세 프로필은 성별과 `get_admin_user` 단건 조회 원문 `phone`을 우선 표시하고 목록은 마스킹 값을 사용 |
...
| --- | --- | --- | --- | --- | --- |
| 마이페이지 > 프로필, 마이페이지 > 결제 내역, 커뮤니티 > 내 활동 | 운영상 추정 | 프로필, 성별, 전화번호, 활동, 결제, 커뮤니티, 로그, 관리자 메모 | 마이페이지 프로필, 결제 내역, 커뮤니티 활동 화면에 운영상 추정으로 연결됩니다. | 관리자 변경 후 또는 원본 데이터 갱신 후 | 실제 사용자 화면 저장소 확인 전까지 추정은 추정으로 유지 |
...
| --- | --- | --- | --- | --- |
| 마이페이지 > 프로필 | 데이터 노출 후보 | 프로필, 성별, 전화번호, 활동, 결제, 커뮤니티, 로그, 관리자 메모 | 회원 상세 데이터 변경 시 표시/접근/알림이 달라질 수 있습니다. | 운영상 추정 |

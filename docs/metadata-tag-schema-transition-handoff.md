...
| P3~P6 | 미착수 | P3 코드 선행 개발은 지금 착수 가능(컷오버 배포만 P2 PASS 전환 후) |
...
2. **P3 선행 개발 (병행 가능 — 컷오버 배포 제외)**: §7.2 변경 파일 11파일(읽기 전환 + 검수 쓰기 신규 RPC 결선), D-12 모크 모드 결선, RT-3/RT-4 시나리오 준비. 실행계획안 §7 + P3 채점표(§12.3) 참조. 신규 4테이블은 적재 완료 상태라 실데이터 기반 개발 가능(전 행 `internal_test`).
3. **P3 컷오버 배포**: P2 PASS 전환 후에만. freeze 윈도 → 델타 재적재(리허설 경로 재사용) → 발산 0건 대사 → 배포.

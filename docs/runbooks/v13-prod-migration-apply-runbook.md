...
| 순서 | 운영 배치 | 파일 | 주의 |
| ---:| --- | --- | --- |
| 0 | — | `20260527113000` | **조건부** — §2-2 `pre_email_confirmed=false` 일 때만. 운영 장부에 false record 가 있으면 러너 provenance upsert 가 덮어쓴다 |
| 1 | `P1` | `20260718120000` | `is_email_confirmed` + `is_supported_country_code` 선행 필요. 적용 즉시 온보딩이 jsonb 경로로 바뀌고 구버전 앱은 완료 화면에서 막힌다 → **앱 배포와 같은 창에서만 실행**한다 |
| 2 | `P2_P6` | `20260722120000` · `20260723234527` · `20260724120000` · `20260724140000` · `20260729120000` | **5파일 1트랜잭션 필수**. **유지보수 창**: PDF 요청 quiesce → 적용 → 앱 동시 배포 |

- `20260724130000`(구 `P5`)은 **2026-08-13 에 단독 적용 완료**라 위 순서에 없다. 남은 백로그는 `P1` 과 `P2_P6` 두 번의 실행이 전부다.
- dev 의 B2(`20260710093000`)·B3(`20260701140000`)·B10(`20260710095000`)은 **운영에 이미 기록돼 있어 백로그가 아니다**(단 B3·B10 은 §2-2 선행조건 프로브 대상).
- **`P2_P6` 을 쪼갤 수 없는 이유는 두 겹이다.** `20260729120000` 은 `20260722120000` 의 수리본이라 같은 트랜잭션이어야 하는데(분리하면 42883 결함 정의가 커밋된다), 동시에 이 매니페스트에서 타임스탬프가 가장 높다. 그래서 그 둘만 묶어 먼저 적용하면 `20260723234527`·`20260724120000`·`20260724140000` 보다 앞서 기록되고, 러너의 순서 역전 가드가 정당하게 거부한다. 다섯 파일을 한 트랜잭션에 넣는 배열만이 수리본을 원본 옆에 두면서 장부를 정확한 타임스탬프순으로 남긴다.
- **비멱등 2건이 이 트랜잭션 안에 있다**(`set schema`+`rename` 7쌍, `request_id` NOT NULL + unique 인덱스). 배치가 하나라 실패는 원자적이다 — 부분 커밋이 없으므로 구 `P3`·`P6` 이 적어 둔 수동 역전 절차는 더 이상 해당하지 않는다. 실패 후 `private.*_unchecked` 개수는 반드시 0 이어야 하며, 0 이 아니면 재시도하지 말고 보고한다.
- 러너가 생성하는 SQL 은 `lock_timeout=5s` · `statement_timeout=180s` 를 트랜잭션 로컬로 설정한다. 잠금을 못 잡으면 커밋 없이 중단되므로, 실패해도 운영 스키마는 실행 전 상태 그대로다.
...
| 2026-08-14 (정정) | **6배치 순서표는 적용 불가능한 계획이었다.** `20260729120000` 을 `20260722120000` 과만 묶으면 나머지 세 파일보다 앞서 기록돼 순서 역전 가드가 거부하고, 그 세 파일을 먼저 넣으려 하면 `20260723234527` 이 `P2` 를 요구해 순환한다. 어느 순서로도 실행되지 않는다는 사실은 실제 적용 시도 전까지 드러나지 않았다. 남은 `P2`·`P3`·`P4`·`P6` 을 단일 배치 `P2_P6`(5파일 1트랜잭션, 오름차순)으로 병합해 해소했다. 아울러 가드가 **같은 배치 안의 앞선 파일까지 차단 사유로 세던 결함**을 고쳤다 — 배치는 한 트랜잭션이고 파일 순서는 이미 오름차순이 강제되므로 형제는 장부 재생을 깨뜨릴 수 없다. 실행 이력: `20260724130000` 2026-08-13 적용, `20260718120000` 2026-08-14 적용 후 같은 날 down 으로 되돌림(구버전 앱 온보딩이 막혀 창을 닫음, 장부 행 삭제까지 포함) |

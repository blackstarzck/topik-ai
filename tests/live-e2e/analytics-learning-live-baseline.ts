...
// live 분석 검증의 기준값 계산 계층 — 분해로 analytics-learning-live.pw.ts 에서
// 이동(동작 동일). Management API 로 dev DB 에 직접 SQL 을 던져 화면과 독립적인
// 기준값을 만들고, 상세 필터 옵션도 여기서 조회한다.
// 이 파일은 playwright testMatch(analytics-learning-live.pw.ts) 밖이라 스펙으로
// 수집되지 않는다.
...
export const projectRef = process.env.SUPABASE_PROJECT_REF;
export const expectedProjectRef = process.env.EXPECTED_SUPABASE_PROJECT_REF;

type LiveWeakDimension = {
  questionNo: number;
  dimension: string;
  submissions: number;
...
export type LiveRpcRow = {
  ...
  weak_dimensions: LiveWeakDimension[];
...
export async function runSql<T extends Record<string, unknown>>(sql: string): Promise<T[]> {
...
export const metadataCte = `resolved_problem_map as (
...
export function resolveCaseRange(
...
export async function baselineAnalytics(
  filterCase: FilterCase,
  boundaries: SqlDateBoundaries
...
export async function optionValues(questionNo: LearningQuestionNo, key: string, limit = 1): Promise<string[]> {

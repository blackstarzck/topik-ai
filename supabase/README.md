...
- **적용 순서는 파일 이름 순이 유일하게 정의된 순서다.** 이미 더 나중 파일이 적용된 환경에
  앞선 파일을 적용하면 그 파일의 `create or replace` 본문이 새 정의를 덮어 되돌린다. checksum
  은 파일별로만 검증하므로 이 되돌림을 잡지 못하고, 클린 재생은 이름 순이라 정상 상태를
  만들기 때문에 shadow 계약으로도 볼 수 없다. 2026-08-06 dev 실측이 그 결과였다 —
  `20260617211000_system_metadata.sql` 등 6/17~6/22 자 파일이 대기 상태로 남아 있다가 phase8
  강제 블록(6/24 적용) 뒤인 7/8~7/16 에 적용돼, 관리자 RPC **47개**가 권한 키 검사를 잃은 채
  한 달을 돌았다(파일 세트·운영 DB 는 처음부터 정상). 러너가 이제 역순 적용을 거부하고
  `--allow-out-of-order-apply` 로만 우회하며, 우회했다면 겹치는 파일을 이름 순으로 재적용한다.
- 게이트 실측·복구: `npm run db:permission-gate-parity`(적용된 파일의 이름 순 최종 정의가
  요구하는 권한 키가 라이브에 있는지 환경별 확인) · `npm run db:permission-gate-repair --
  --target <ref> --reference <parity 통과 ref>`(참조 환경이 스스로 통과하지 않거나 시그니처가
  다르면 거부 — 오버로드 추가로 게이트 없는 본문이 남는 것을 막는다).

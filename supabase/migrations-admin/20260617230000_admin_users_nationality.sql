-- =====================================================================
-- topik-ai admin · Users directory · admin-0022
-- 회원 목록·상세에 국적(nationality_country_code) 노출 — get_admin_users 확장.
--   v13 가입(일반·SNS) 시 수집되는 ISO 3166-1 alpha-2 국가 코드.
--
-- 원천(v13 소유, 읽기 전용): public.profiles.nationality_country_code
--   (v13 마이그레이션 20260617195000_profiles_nationality — nullable, alpha-2 CHECK,
--    handle_new_user 가 raw_user_meta_data 에서 시드). admin 은 읽기만 한다.
-- RETURNS TABLE 변경 → drop 후 재생성. 인자명(search/sort/page/page_size) 유지.
-- 화면 표시(코드→국가명 한글)는 프론트 formatNationality 에서 처리(여기선 코드 원본 반환).
-- down: supabase/migrations-admin/down/20260617230000_admin_users_nationality.sql
-- =====================================================================
...
comment on function public.get_admin_users(text, text, integer, integer) is
  'Users > 회원 목록 read. platform_admin 전용. profiles(+nationality_country_code)+auth.users 조인, writing_submissions 집계, legal_documents/user_consents 약관 동의 상태 포함. 인자명 search/sort/page/page_size 고정(PostgREST 매칭).';

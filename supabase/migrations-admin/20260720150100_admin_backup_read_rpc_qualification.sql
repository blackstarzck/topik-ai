...
-- Qualify projected component columns so PL/pgSQL does not confuse them with
-- RETURNS TABLE output variables when the detail RPC executes.
...
begin
  ...
  with rows as (
    ...
    select candidate.*
    from rows as candidate
    where p_result is null or case
      when p_target is null then candidate.effective_status
      when p_target = 'database' and candidate.database_status = 'pending' then
        case when candidate.effective_status = 'delayed' then 'delayed' else 'running' end
      when p_target = 'database' and candidate.database_status = 'not_run' then 'delayed'
      when p_target = 'database' then candidate.database_status
      when p_target = 'storage' and candidate.storage_status = 'pending' then
        case when candidate.effective_status = 'delayed' then 'delayed' else 'running' end
      when p_target = 'storage' and candidate.storage_status = 'not_run' then 'delayed'
      else candidate.storage_status
    end = p_result
  )
  select
    f.id,

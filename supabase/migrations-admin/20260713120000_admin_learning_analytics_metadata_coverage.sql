-- =====================================================================
-- Analytics > 학습 분석: 결정적 problem alias + metadata coverage
--
-- canonical legacy_problem_id와 환경별 결정적 problem_id alias를 하나의 읽기
-- 관계로 합치고, 기간별 제출/이벤트 metadata coverage를 별도 계약으로 반환한다.
-- 주제/세부 조건은 TOPIK 쓰기 metadata SoT로만 판정하며 v13 원본은 변경하지 않는다.
--
-- 기간 경계: date 입력을 Asia/Seoul 달력일로 해석한다. 시작일 00:00 이상,
-- 종료일 다음 날 00:00 미만이다. 양쪽 날짜가 모두 NULL이면 전체 기간이다.
-- down: supabase/migrations-admin/down/20260713120000_admin_learning_analytics_metadata_coverage.sql
-- =====================================================================

drop function if exists public.get_admin_learning_analytics_filter_options();

create function public.get_admin_learning_analytics_filter_options()
...
create function public.get_admin_learning_analytics_filtered(

-- =====================================================================
-- Analytics > 학습 분석: 미매핑 제출 집계 fallback
--
-- 신규 메타데이터와 연결되지 않은 writing_submissions도 문제 번호 기준의
-- 기본 통계에서 제외하지 않는다. 주제/세부 특성 조건을 적용한 경우에만
-- source_map 연결을 필수로 유지한다. v13 소유 테이블 DDL은 변경하지 않는다.
--
-- 기간 경계: date 입력을 Asia/Seoul 달력일로 해석한다. 시작일 00:00 이상,
-- 종료일 다음 날 00:00 미만이다. 양쪽 날짜가 모두 NULL이면 전체 기간이다.
-- down: supabase/migrations-admin/down/20260713103000_admin_learning_analytics_unmapped_fallback.sql
-- =====================================================================
...
begin
  ...
  submission_facts as (
    select
      ...
      ws.user_id,
      ws.problem_id,
      coalesce(m.item_number, problem.question_no) as question_no,
      m.topic_main,
      m.topic_detail,
      ws.submitted_at,
      ws.feedback_status,
    ...
    where coalesce(m.item_number, problem.question_no) = any(v_question_nos)
      and (
  ...
  current_active_events as (
    select se.id, se.user_id, coalesce(m.item_number, problem.question_no) as question_no
    ...
    where se.event_type <> 'export_downloaded'
      and coalesce(m.item_number, problem.question_no) = any(v_question_nos)
      and (
        (p_topic_main is null and p_topic_detail is null and v_detail_filters = '{}'::jsonb)
        or m.problem_id is not null
      )
...
comment on function public.get_admin_learning_analytics_filtered(
  ...
  'KST 날짜·51~54는 problems.question_no fallback을 포함하고, 신규 topic_main/topic_detail·세부 조건은 매핑 제출에 적용한다. '

-- =====================================================================
-- Analytics > 학습 분석: metadata coverage 계약 회귀 복구
--
-- 문제별 주제 통계를 유지하면서 기간별 제출/이벤트 metadata coverage를 복구한다.
-- canonical identity projection이 이미 설치된 환경에서는 public mirror 대신 private
-- identity 관계를 사용하고, 구형 환경에서는 기존 public 관계를 그대로 사용한다.
--
-- 기간 경계: date 입력을 Asia/Seoul 달력일로 해석한다. 시작일 00:00 이상,
-- 종료일 다음 날 00:00 미만이다. 양쪽 날짜가 모두 NULL이면 전체 기간이다.
-- down: supabase/migrations-admin/down/20260715173826_restore_learning_analytics_metadata_contract.sql
-- =====================================================================

create or replace function public.get_admin_learning_analytics_filter_options()
...
begin
  ...
  return query
  with topic_pairs as (
    select distinct v.topic_main, v.topic_detail
    from public.topik_writing_question_recommendation_view v
    where v.item_number in (51, 52, 53, 54)
  ),
  topic_rows as (
    select
      t.topic_main,
      jsonb_agg(to_jsonb(t.topic_detail) order by t.topic_detail) as topic_details
    from topic_pairs t
    group by t.topic_main
  ),
  q51_values as (
    select
      coalesce(
        (select jsonb_agg(to_jsonb(x.value) order by x.value)
           from (select distinct value
                   from public.topik_writing_51_questions q
                   cross join lateral unnest(array[q.blank_1_role, q.blank_2_role]) item(value)
                  where value is not null and value <> '') x),
        '[]'::jsonb
      ) as blank_role,
      coalesce(
        (select jsonb_agg(to_jsonb(x.value) order by x.value)
           from (select distinct value
                   from public.topik_writing_51_questions q
                   cross join lateral unnest(array[q.blank_1_function, q.blank_2_function]) item(value)
                  where value is not null and value <> '') x),
        '[]'::jsonb
      ) as blank_function,
      coalesce(
        (select jsonb_agg(to_jsonb(x.value) order by x.value)
           from (select distinct value
                   from public.topik_writing_51_questions q
                   cross join lateral unnest(array[q.blank_1_answer_type, q.blank_2_answer_type]) item(value)
                  where value is not null and value <> '') x),
        '[]'::jsonb
      ) as answer_type
  ),
  q52_values as (
    select
      coalesce(
        (select jsonb_agg(to_jsonb(x.value) order by x.value)
           from (select distinct q.connection_function as value
                   from public.topik_writing_52_questions q
                  where q.connection_function is not null and q.connection_function <> '') x),
        '[]'::jsonb
      ) as connection_function,
      coalesce(
        (select jsonb_agg(to_jsonb(x.value) order by x.value)
           from (select distinct q.answer_scope_type as value
                   from public.topik_writing_52_questions q
                  where q.answer_scope_type is not null and q.answer_scope_type <> '') x),
        '[]'::jsonb
      ) as answer_scope
  ),
  q53_values as (
    select
      coalesce(
        (select jsonb_agg(to_jsonb(x.value) order by x.value)
           from (select distinct q.data_type as value
                   from public.topik_writing_53_questions q
                  where q.data_type is not null and q.data_type <> '') x),
        '[]'::jsonb
      ) as data_type,
      coalesce(
        (select jsonb_agg(to_jsonb(x.value) order by x.value)
           from (
             ...
             cross join lateral jsonb_array_elements_text(
               case when jsonb_typeof(q.required_structure) = 'array'
                 then q.required_structure else '[]'::jsonb end
             ) item(value)
             where value <> ''
           ) x),
        '[]'::jsonb
  ...
  q54_values as (
    select
      coalesce(
        (select jsonb_agg(to_jsonb(x.value) order by x.value)
           from (select distinct q.essay_type as value
                   from public.topik_writing_54_questions q
                  where q.essay_type is not null and q.essay_type <> '') x),
        '[]'::jsonb
      ) as essay_type,
      coalesce(
        (select jsonb_agg(to_jsonb(x.value) order by x.value)
           from (select distinct q.stance_requirement as value
                   from public.topik_writing_54_questions q
                  where q.stance_requirement is not null and q.stance_requirement <> '') x),
        '[]'::jsonb
      ) as stance,
      coalesce(
        (select jsonb_agg(to_jsonb(x.value) order by x.value)
  ...
  select jsonb_build_object(
    'topics', coalesce(
      (select jsonb_agg(jsonb_build_object(
        'topicMain', tr.topic_main,
        'topicDetails', tr.topic_details
      ) order by tr.topic_main) from topic_rows tr),
      '[]'::jsonb
    ),
    'detailFilters', jsonb_build_object(
      '51', jsonb_build_object(
        'blankRole', q51.blank_role,
        'blankFunction', q51.blank_function,
        'answerType', q51.answer_type
      ),
      '52', jsonb_build_object(
        'connectionFunction', q52.connection_function,
        'answerScope', q52.answer_scope
      ),
      '53', jsonb_build_object(
        'dataType', q53.data_type,
        'requiredStructure', q53.required_structure
      ),
      '54', jsonb_build_object(
        'essayType', q54.essay_type,
        'stance', q54.stance,
        'requiredStructure', q54.required_structure
      )
    )
  )
  from q51_values q51
  cross join q52_values q52
  cross join q53_values q53
  cross join q54_values q54;
end;
$$;

revoke all on function public.get_admin_learning_analytics_filter_options() from public;
revoke all on function public.get_admin_learning_analytics_filter_options() from anon;
...
begin
  if v_identity is null then
    raise exception 'learning analytics RPC is missing; refusing metadata contract restore';
  end if;
  ...
  if position('topic_total' in v_definition) = 0
    ...
    raise exception 'unexpected learning analytics definition; per-question topic stats must exist before restore';
  end if;
end
$$;
...
begin
  ...
  question_metadata as (
    ...
    join public.problems mapped_problem
      on mapped_problem.id = pm.problem_id
     and mapped_problem.question_no = pm.item_number
    join public.topik_writing_question_recommendation_view v
      on v.question_id = pm.question_id
     and v.item_number = pm.item_number
    ...
    where v.topic_main is not null
      and v.topic_detail is not null
      and (
        (v.item_number = 51
          and q51.blank_1_role is not null
          and q51.blank_1_function is not null
          and q51.blank_1_answer_type is not null
          and q51.blank_2_role is not null
          and q51.blank_2_function is not null
          and q51.blank_2_answer_type is not null)
        or (v.item_number = 52
          and q52.connection_function is not null
          and q52.answer_scope_type is not null)
        or (v.item_number = 53
          and q53.data_type is not null
          and q53.required_structure is not null)
        or (v.item_number = 54
          and q54.essay_type is not null
          and q54.stance_requirement is not null
  ...
  submission_facts as (
    select
      ...
      ws.user_id,
      ws.problem_id,
      problem.question_no as question_no,
      m.topic_main,
      m.topic_detail,
      ws.submitted_at,
      ws.feedback_status,
    ...
    left join metadata_filtered m on m.problem_id = ws.problem_id
  ...
  submission_metadata_facts as (
    select
      ws.problem_id,
      ws.submitted_at,
      (m.problem_id is not null and m.item_number = problem.question_no) as mapped
    from public.writing_submissions ws
    join public.problems problem on problem.id = ws.problem_id
    left join question_metadata m on m.problem_id = ws.problem_id
    where problem.question_no = any(v_question_nos)
  ),
  event_metadata_facts as (
    select
      coalesce(se.problem_id, event_submission.problem_id) as problem_id,
      se.occurred_at,
      (m.problem_id is not null and m.item_number = problem.question_no) as mapped
    ...
    join public.problems problem
      on problem.id = coalesce(se.problem_id, event_submission.problem_id)
    left join question_metadata m on m.problem_id = problem.id
    where se.event_type <> 'export_downloaded'
      and problem.question_no = any(v_question_nos)
  ),
  metadata_coverage as (
    select
      count(*) filter (
        where v_start is null
           or (submitted_at >= v_start and submitted_at < v_end_exclusive)
      )::integer as eligible_submissions,
      count(*) filter (
        where mapped and (
          v_start is null
          or (submitted_at >= v_start and submitted_at < v_end_exclusive)
        )
      )::integer as mapped_submissions,
      count(*) filter (
        where v_compare
          and submitted_at >= v_prev_start
          and submitted_at < v_prev_end_exclusive
      )::integer as eligible_submissions_prev,
      count(*) filter (
        where v_compare and mapped
          and submitted_at >= v_prev_start
          and submitted_at < v_prev_end_exclusive
      )::integer as mapped_submissions_prev,
      count(distinct problem_id) filter (
        where v_start is null
           or (submitted_at >= v_start and submitted_at < v_end_exclusive)
      )::integer as eligible_problems,
      count(distinct problem_id) filter (
        where mapped and (
          v_start is null
          or (submitted_at >= v_start and submitted_at < v_end_exclusive)
        )
      )::integer as mapped_problems
    from submission_metadata_facts
  ),
  event_metadata_coverage as (
    select
      count(*) filter (
        where v_start is null
           or (occurred_at >= v_start and occurred_at < v_end_exclusive)
      )::integer as eligible_events,
      count(*) filter (
        where mapped and (
          v_start is null
          or (occurred_at >= v_start and occurred_at < v_end_exclusive)
        )
      )::integer as mapped_events,
      count(*) filter (
        where v_compare
          and occurred_at >= v_prev_start
          and occurred_at < v_prev_end_exclusive
      )::integer as eligible_events_prev,
      count(*) filter (
        where v_compare and mapped
          and occurred_at >= v_prev_start
          and occurred_at < v_prev_end_exclusive
      )::integer as mapped_events_prev
    from event_metadata_facts
  ),
  current_active_events as (
    select se.id, se.user_id, problem.question_no as question_no
    from public.study_events se
    left join public.writing_submissions event_submission
      on event_submission.id = se.submission_id
    join public.problems problem
      on problem.id = coalesce(se.problem_id, event_submission.problem_id)
    left join metadata_filtered m
      on m.problem_id = problem.id
    where se.event_type <> 'export_downloaded'
      and problem.question_no = any(v_question_nos)
      and (
        (p_topic_main is null and p_topic_detail is null and v_detail_filters = '{}'::jsonb)
  ...
  pdf_classified as (
    select
      ...
      case
        ...
        when pe.source_type = 'submission'
         and coalesce(all_meta.problem_id, source_problem.id) is not null then 'attributable'
        else 'unclassified'
      end as attribution,
      coalesce(all_meta.item_number, source_problem.question_no) as item_number,
      case
        when p_topic_main is null and p_topic_detail is null and v_detail_filters = '{}'::jsonb
          then coalesce(all_meta.item_number, source_problem.question_no) = any(v_question_nos)
        else filtered_meta.problem_id is not null
      end as matches_scope
    from pdf_events pe
    left join public.writing_submissions source_submission
      on pe.source_type = 'submission' and source_submission.id = pe.source_id
    left join public.problems source_problem
      on source_problem.id = source_submission.problem_id
  ...
  select
    jsonb_build_object(
      ...
      'metadataEligibleSubmissions', mc.eligible_submissions,
      'metadataMappedSubmissions', mc.mapped_submissions,
      'metadataUnmappedSubmissions', mc.eligible_submissions - mc.mapped_submissions,
      'metadataCoverageRate', case when mc.eligible_submissions > 0 then round(
        mc.mapped_submissions::numeric / mc.eligible_submissions * 100, 1
      ) end,
      'metadataEligibleSubmissionsPrev', case when v_compare then mc.eligible_submissions_prev end,
      'metadataMappedSubmissionsPrev', case when v_compare then mc.mapped_submissions_prev end,
      'metadataUnmappedSubmissionsPrev', case when v_compare
        then mc.eligible_submissions_prev - mc.mapped_submissions_prev end,
      'metadataCoverageRatePrev', case when v_compare and mc.eligible_submissions_prev > 0 then round(
        mc.mapped_submissions_prev::numeric / mc.eligible_submissions_prev * 100, 1
      ) end,
      'metadataEligibleEvents', emc.eligible_events,
      'metadataMappedEvents', emc.mapped_events,
      'metadataEventCoverageRate', case when emc.eligible_events > 0 then round(
        emc.mapped_events::numeric / emc.eligible_events * 100, 1
      ) end,
      'metadataEligibleEventsPrev', case when v_compare then emc.eligible_events_prev end,
      'metadataMappedEventsPrev', case when v_compare then emc.mapped_events_prev end,
      'metadataEventCoverageRatePrev', case when v_compare and emc.eligible_events_prev > 0 then round(
        emc.mapped_events_prev::numeric / emc.eligible_events_prev * 100, 1
      ) end,
      'metadataEligibleProblems', mc.eligible_problems,
      'metadataMappedProblems', mc.mapped_problems
    ) as summary,
  ...
  cross join metadata_coverage mc
  cross join event_metadata_coverage emc;
end;
$$;
...
declare
  ...
  v_has_identity_map boolean := to_regclass(
    'private.admin_writing_question_identity_map'
  ) is not null;
  v_has_identity_projection boolean := to_regclass(
    'private.admin_writing_problem_identity_projection'
  ) is not null;
  v_definition text;
begin
  if v_identity is null then
    raise exception 'learning analytics RPC restore failed';
  end if;

  if v_has_identity_map <> v_has_identity_projection then
    raise exception 'canonical writing identity objects are partially installed; refusing analytics rewrite';
  end if;
  ...
  if position('submission_metadata_facts as' in v_definition) = 0
     ...
     or position('''questionNo'', t.question_no' in v_definition) = 0 then
    raise exception 'learning analytics metadata/topic contract restore incomplete';
  end if;

  if not v_has_identity_map then
    return;
  end if;
  ...
  if position(
    ...
    raise exception 'unexpected analytics identity definition; refusing canonical rewrite';
  end if;
  ...
  if position('public.problems' in v_definition) > 0
     ...
     or position('event_metadata_coverage as' in v_definition) = 0
     or position('topic_total' in v_definition) = 0 then
    raise exception 'learning analytics canonical rewrite incomplete';
  end if;
...
comment on function public.get_admin_learning_analytics_filtered(
  ...
  'KST 날짜·51~54는 설치된 identity 관계의 문항 번호 fallback을 포함하고, topic_main/topic_detail·세부 조건은 canonical+alias 통합 매핑 제출에 적용한다. '

-- =====================================================================
-- topik-ai admin · Users > 회원 목록 기관 소속 필터 — get_admin_users 확장.
--   기존 affiliation_code/affiliation_label 노출(20260619150000) + email_confirmed
--   (20260623100000)에 더해, 서버사이드 필터 파라미터 affiliation 을 추가한다.
--   목록 page_size 캡(클라이언트 필터로는 첫 페이지만) 문제를 피하려면 필터는 서버에서 적용되어야 한다.
--
-- 파라미터 affiliation (text, default null):
--   null/''        → 전체(필터 없음)
--   '@affiliated'  → 기관 회원만(affiliation_code 보유)
--   '@general'     → 일반 회원만(affiliation_code 없음)
--   그 외 문자열   → 해당 코드 정확 일치
--   * '@' 는 institution_codes.code 정규식([A-Za-z0-9_-])에 들어갈 수 없어 코드 값과 충돌 불가.
--
-- 계약: 인자명 search/sort/page/page_size 유지 + affiliation 추가 → drop 후 재생성.
--   RETURNS TABLE 컬럼은 20260623100000(email_confirmed 포함)과 동일(변경 없음).
-- down: supabase/migrations-admin/down/20260623120000_admin_users_affiliation_filter.sql
-- =====================================================================
...
comment on function public.get_admin_users(text, text, integer, integer, text) is
  'Users > 회원 목록 read. platform_admin 전용. profiles+auth.users+social_providers+약관동의+기관유입(affiliation_code/label)+이메일 인증(email_confirmed) 포함. affiliation 필터: null=전체 / @affiliated=기관회원만 / @general=일반회원만 / 그 외=해당 코드. 인자명 search/sort/page/page_size/affiliation 고정(PostgREST 매칭).';

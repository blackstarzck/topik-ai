-- Keep the persisted audit target contract (`User`) while exposing the
-- user-facing admin route contract (`Users`) from the read RPC.
...
begin
  if lower(v_target_type) in ('user', 'users') then
    v_target_type := 'User';
  end if;
  ...
  with filtered as (
    ...
    where (
        ...
        or l.target_table = v_target_type
        or (v_target_type = 'User' and lower(l.target_table) = 'users')
      )

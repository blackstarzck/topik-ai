-- =====================================================================
-- Analytics > 학습 분석: 전역 조건 기반 집계 RPC
--
-- 기존 get_admin_learning_analytics(integer)는 호환성을 위해 유지한다.
-- 이 마이그레이션은 admin 소유 RPC만 추가하며 v13 / topik_writing 테이블의
-- DDL을 변경하지 않는다. 응답은 순수 집계이며 사용자 식별자와 답안 원문을
-- 반환하지 않는다.
--
-- 기간 경계: date 입력을 Asia/Seoul 달력일로 해석한다. 시작일 00:00 이상,
-- 종료일 다음 날 00:00 미만이다. 양쪽 날짜가 모두 NULL이면 전체 기간이다.
-- down: supabase/migrations-admin/down/20260710120000_admin_learning_analytics_filtered.sql
-- =====================================================================

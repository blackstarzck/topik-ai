-- Phase 8 enforcement (commerce (refunds/coupons/points)): admin_has_permission gates after is_admin.
-- Generated from live bodies by scripts/db/gen-phase8-enforcement.mjs.
-- down: supabase/migrations-admin/down/20260623281000_phase8_commerce.sql
...
begin
  ...
  if not public.admin_has_permission(caller_id, 'commerce.refunds.approve') then raise exception 'forbidden: missing permission commerce.refunds.approve'; end if;
...
begin
  if caller_id is null then raise exception 'unauthenticated'; end if;
  if not private.is_admin(caller_id) then raise exception 'forbidden: admin required'; end if;
  if not public.admin_has_permission(caller_id, 'commerce.coupons.manage') then raise exception 'forbidden: missing permission commerce.coupons.manage'; end if;
  if nullif(btrim(coalesce(p_reason, '')), '') is null then raise exception 'reason required (operational reason)'; end if;
...
begin
  if caller_id is null then raise exception 'unauthenticated'; end if;
  if not private.is_admin(caller_id) then raise exception 'forbidden: admin required'; end if;
  if not public.admin_has_permission(caller_id, 'commerce.points.manage') then raise exception 'forbidden: missing permission commerce.points.manage'; end if;
  if nullif(btrim(coalesce(p_reason, '')), '') is null then raise exception 'reason required (operational reason)'; end if;

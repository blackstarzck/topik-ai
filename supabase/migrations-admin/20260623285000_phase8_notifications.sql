-- Phase 8 enforcement (message (notification templates/dispatch/groups)): admin_has_permission gates after is_admin.
-- Generated from live bodies by scripts/db/gen-phase8-enforcement.mjs.
-- down: supabase/migrations-admin/down/20260623285000_phase8_notifications.sql
...
begin
  ...
  if not public.admin_has_permission(caller_id, 'message.groups.manage') then raise exception 'forbidden: missing permission message.groups.manage'; end if;
...
begin
  if caller_id is null then raise exception 'unauthenticated'; end if;
  if not private.is_admin(caller_id) then raise exception 'forbidden: admin required'; end if;
  if not (public.admin_has_permission(caller_id, 'message.mail.manage') or public.admin_has_permission(caller_id, 'message.push.manage') or public.admin_has_permission(caller_id, 'message.inapp.manage')) then raise exception 'forbidden: missing permission (message.mail.manage/message.push.manage/message.inapp.manage)'; end if;
  if nullif(btrim(coalesce(p_reason, '')), '') is null then
    raise exception 'reason required (operational reason)';
  end if;

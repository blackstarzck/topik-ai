-- Restore the immediately previous promotion function definition.
--
-- This rollback changes only the function interface dependency. It performs no
-- data DML and never recreates deleted writing content.
...
comment on function public.admin_promote_writing_questions(uuid, text[]) is
  'Service-role promotion. Same-import identity/item/hash matches recover bookkeeping idempotently; a later import may replace current canonical content while learner history remains pinned to immutable row snapshots.';

-- Timestampless content versioning: classify receipts whose source updated_at is
-- absent by canonical content comparison instead of holding them.
--
-- Owner decision (2026-08-24): the supply API is a pull of the supplier's current
-- catalog state, and updated_at may be absent. When updated_at is absent, the
-- fetched payload is still the supplier's current truth, so promotion eligibility
-- is decided by the canonical content_hash alone (review_status gate unchanged).
-- When updated_at is present, the strict revision-ordering ladder from
-- 20260716052957 applies unchanged. Once a question family has established
-- revision ordering (any valid receipt carried a parseable updated_at), a later
-- timestampless receipt is a contract regression and is held.
--
-- Measured context (2026-08-24 live probe): the upstream rewrote situation_summary
-- on 699/701 items without setting updated_at, so those receipts were held as
-- invalid_timestamp. This migration also reclassifies exactly those held rows
-- (latest receipt, valid immutable created_at matching the family baseline, a
-- promoted canonical reference present) under the new rule so the pending
-- rewrites become promote candidates without re-ingesting.
...
begin
  ...
  v_timestampless := v_updated_raw is null;
  ...
  select
    ...
    coalesce(
      bool_or(
        private.try_parse_writing_source_time(raw_payload->>'updated_at') is not null
      ) filter (
        where version_decision not in ('invalid_timestamp', 'identity_conflict')
      ),
      false
    ),
  ...
  if v_source_created_at is null
     or (not v_timestampless and v_source_updated_at is null)
     or (v_source_updated_at is not null and v_source_updated_at < v_source_created_at) then
    v_version_decision := 'invalid_timestamp';
    v_hold_reason := 'invalid_timestamp: created_at must be valid and a present updated_at must parse and not precede created_at';
  ...
  elsif v_timestampless and v_family_has_ordering then
    v_version_decision := 'invalid_timestamp';
    v_hold_reason := 'invalid_timestamp: updated_at is missing although revision ordering was already established for this question';
  elsif not v_has_family then
    v_version_decision := 'initial';
  elsif v_timestampless then
    -- Content-only mode: the supply API is a pull of current state, so without a
    -- revision timestamp the changed canonical content itself is the promotion
    -- signal. Same content stays a metadata-only receipt.
  ...
  -- A timestampless receipt records created_at as its effective revision time so
  -- promotion ordering and the version history stay non-null (same convention as
  -- the 20260716052957 legacy backfill). raw_payload keeps the original null.
  if v_timestampless then
    v_source_updated_at := v_source_created_at;
  end if;
...
comment on function public.admin_ingest_writing_task(uuid, text, jsonb, text, smallint, text) is
  'Service-role ingest. Serializes by question_id and preserves exact payload_hash rows. With a source updated_at it promotes only strictly newer revisions with changed canonical content_hash; without one (timestampless pull) it promotes on changed canonical content_hash alone until the family establishes revision ordering.';

-- One-time reclassification of receipts held only because updated_at was absent.
-- Guards: latest receipt of its family, valid created_at that matches the
-- family baseline, canonical reference present via the source map. Content equal
-- to the canonical reference stays a held metadata-only receipt; changed content
-- becomes a promote candidate. Rows without a canonical reference stay held.
with candidate as (
  select
    imp.import_id,
    imp.content_hash,
    ref.content_hash as reference_content_hash
  from public.topik_writing_question_import as imp
  join public.topik_writing_question_source_map as source_map
    on source_map.question_id = imp.source_task_id
  join public.topik_writing_question_import as ref
    on ref.import_id = source_map.canonical_import_id
   and ref.content_hash is not null
  where imp.is_latest
    and imp.version_decision = 'invalid_timestamp'
    and imp.mapping_status = 'held'
    and imp.content_hash is not null
    and imp.source_created_at is not null
    and nullif(btrim(imp.raw_payload->>'updated_at'), '') is null
    and private.try_parse_writing_source_time(imp.raw_payload->>'created_at') = imp.source_created_at
    and exists (
      select 1
        from public.topik_writing_question_import as family
       where family.source_task_id = imp.source_task_id
         and family.import_id <> imp.import_id
         and family.version_decision in ('legacy', 'initial', 'content_changed', 'metadata_only')
         and family.source_created_at = imp.source_created_at
    )
)
update public.topik_writing_question_import as imp
   set version_decision = case
         when imp.content_hash = candidate.reference_content_hash then 'metadata_only'
         else 'content_changed'
       end,
       mapping_status = case
         when imp.content_hash = candidate.reference_content_hash then 'held'
         else 'raw'
       end,
       hold_reason = case
         when imp.content_hash = candidate.reference_content_hash
           then 'metadata_only: canonical learner/grading content is unchanged'
         else null
       end,
       source_updated_at = imp.source_created_at
  from candidate
 where imp.import_id = candidate.import_id;

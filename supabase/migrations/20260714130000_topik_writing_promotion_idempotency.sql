-- Corrective promotion idempotency after canonical cutover.
--
-- 20260713080015 is already applied and must remain immutable. This migration
-- preserves canonical content when the exact pinned import is retried, while
-- keeping different hashes and identity mismatches fail-closed.
...
comment on function public.admin_promote_writing_questions(uuid, text[]) is
  'Service-role promotion. Same-import identity/item/hash matches recover bookkeeping idempotently without replacing canonical content; different hashes and invalid identities remain fail-closed.';
